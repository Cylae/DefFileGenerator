...
```bash
# Install required dependencies (Core)
...
- `--pages PAGES` - PDF pages to process (e.g., "1,2,5")
- `--mapping JSON` - JSON mapping file for custom column detection
- `--address-offset OFFSET` - Shift all register addresses
- `--template` - Generate a sample register map CSV

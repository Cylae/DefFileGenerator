...
*(Required unless `--template` is used)*
...
- `--pages LIST` - Comma-separated PDF pages (e.g., 1,2,5)
- `--mapping JSON` - JSON file for custom column mapping
- `--address-offset INT` - Shift all addresses (default: 0)
- `--forced-write VAL` - Webdyn forced write value
- `--template` - Generate a sample definition CSV

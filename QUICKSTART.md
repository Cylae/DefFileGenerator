...
# Optional dependencies for testing and performance benchmarking
...
- Validates Modbus addresses are within the 0-65535 range.
...
### Required Arguments (unless using --template)
...
- `--pages LIST` - PDF pages (comma-separated integers)
- `--mapping FILE` - JSON mapping file
- `--address-offset INT` - Shift addresses by a value
- `--forced-write STR` - Value for the header's forced-write field
...
- The tool will guess U16 if not specified.
...
- Check the logs for address range or overlap warnings.

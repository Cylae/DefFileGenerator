...
class Extractor:
    COLUMN_MAPPING = {
        ...
        'StartBit': ['start bit', 'startbit', 'bit']
    }
    ...
    def extract_from_xml(self, filepath):
        ...
        try:
            with open(filepath, 'rb') as f:
                content = f.read()
            ET.fromstring(content) # XXE validation
    ...
    def map_and_clean(self, tables, address_offset=0):
        ...
        for table in tables:
            ...
            detection_order = ['RegisterType', 'Address', 'Offset', 'Name', 'Type', 'Unit', 'Action', 'Tag', 'Factor', 'ScaleFactor', 'Length', 'StartBit']
            ...
            for row in table:
                ...
                # Use 'Offset' as fallback for 'Address'
                ...
                # Support complex address construction if Length/StartBit are columns
...
def main():
    ...
    out = (open(args.output, 'w', newline='', encoding='utf-8') if args.output and args.output.strip()
           else sys.stdout)
    ...
    if args.output and args.output.strip(): out.close()

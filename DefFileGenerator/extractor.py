...
class Extractor:
    ...
    def map_and_clean(self, tables, address_offset=0):
        ...
        for table in tables:
            ...
            # Identify target columns by scanning first few rows
            all_src_cols = set()
            for r in table[:5]:
            ...
            for row in table:
                ...
                if length and start_bit:
                    addr_val = f"{addr_base}_{start_bit}_{length}"
                elif length:
                    addr_val = f"{addr_base}_{length}"
                else:
                    addr_val = addr_base
                ...
                # CoefA calculation factor handling

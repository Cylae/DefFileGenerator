...
class Extractor:
    COLUMN_MAPPING = {
        ...
        'Length': ['length', 'len', 'size', 'number of registers', 'quantity']
    }
        ...
        try:
            with pdfplumber.open(filepath) as pdf:
                ...
                for page in target_pages:
                    ...
                    logging.debug(f"Found {len(page_tables)} tables on page {page.page_number}")
    ...
    def map_and_clean(self, tables, address_offset=0):
        ...
        for table in tables:
            ...
            detection_order = ['RegisterType', 'Address', 'Name', 'Type', 'Unit', 'Action', 'Tag', 'Factor', 'ScaleFactor', 'StartBit', 'Length']
            ...
            for row in table:
                ...
                if '_' in addr:
                    normalized_addr = '_'.join(self.generator.normalize_address_val(p) for p in addr.split('_'))
                else:
                    normalized_addr = self.generator.normalize_address_val(addr)

                # Apply Offset
                new_row['Address'] = self.generator.apply_address_offset(normalized_addr, address_offset)

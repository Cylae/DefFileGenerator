        ...
        try:
            with pdfplumber.open(filepath) as pdf:
                ...
                if pages is None:
                    ...
                    requested = pages if isinstance(pages, list) else ([pages] if not isinstance(pages, str) else pages.split(','))
                    for p in requested:
                        try:
                            idx = int(str(p).strip()) - 1
                ...
                for page in target_pages:
                    ...
                    for table in tables:
                        ...
                        # Exhausting the generator into a list while the file is open
                        # to avoid issues with closing the context manager.
        ...
        def csv_table_generator() -> Iterator[Dict[str, Any]]:
            try:
                ...
                with open(filepath, 'r', encoding=encoding) as f:
                    ...
                    for row in reader:
                        if any(val and str(val).strip() for val in row.values()):
                            yield dict(row)

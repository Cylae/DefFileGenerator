...
class Extractor:
    ...
    def extract_from_xml(self, filepath):
        ...
        try:
            with open(filepath, 'rb') as f:
                content = f.read()
            ET.fromstring(content) # Pre-validate with defusedxml
    ...
    def map_and_clean(self, tables, address_offset=0):
        ...
        # Establish mapping from the first row of the first table
        ...
        for table in tables:
            for row in table:
                ...
                # Complex address construction: Address_Length or Address_StartBit_Length
                addr = str(new_row.get('Address', '')).strip()
                length = new_row.get('Length')
                start_bit = new_row.get('StartBit')

                if addr:
                    length_str = str(length).strip() if length is not None and str(length).strip() != "" else ""
                    start_bit_str = str(start_bit).strip() if start_bit is not None and str(start_bit).strip() != "" else ""
                    if length_str != "" and start_bit_str != "":
                        # Address_StartBit_Length
                        addr = f"{addr}_{start_bit_str}_{length_str}"
                    elif length_str:
                        addr = f"{addr}_{length_str}"

        ...
        def pdf_tables_generator():
            try:
                with pdfplumber.open(filepath) as pdf:
                    requested = pages if isinstance(pages, list) else ([pages] if pages is not None else None)
                    if requested is None:
                        ...
                        for p_num in requested:
                            if 1 <= p_num <= len(pdf.pages):
                                ...
                                logging.warning(f"Page {p_num} out of range (1-{len(pdf.pages)}) in {filepath}. Skipping.")

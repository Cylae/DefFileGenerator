...
class Extractor:
    ...
    def extract_from_pdf(self, filepath: str, pages: Optional[Union[str, int, List[int]]] = None) -> Iterator[Iterator[Dict[str, Any]]]:
        ...
        # Handle pages as comma-separated string, single integer, or list of integers
        if isinstance(pages, str):
            try:
                pages = [int(p.strip()) for p in pages.split(',')]
            except ValueError:
                logging.error(f"Invalid pages format: {pages}. Expected comma-separated integers.")
                return iter([])
                    ...
                    if pages is None:
                        ...
                        for i in page_indices:
                            if 1 <= i <= len(pdf.pages):
                                ...
                                logging.warning(f"Page {i} is out of range for PDF with {len(pdf.pages)} pages. Skipping.")

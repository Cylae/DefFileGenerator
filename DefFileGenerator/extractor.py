...
class Extractor:
    ...
    def map_and_clean(self, tables, address_offset=0):
        ...
        for table in tables:
            ...
            # 1. Explicit mapping
            # Ensure mapping is handled even if args object is passed (backwards compatibility)
            mapping_to_use = self.mapping
            if hasattr(self.mapping, 'mapping'): mapping_to_use = self.mapping.mapping

            for target, source in mapping_to_use.items():
            ...
            for row in table:
                ...
                if '_' in addr:
                    # Complex address like Addr_Len or Addr_Bit_Len
                    new_row['Address'] = self.generator.apply_address_offset(addr, address_offset)
                else:
                    new_row['Address'] = self.generator.apply_address_offset(self.generator.normalize_address_val(addr), address_offset)

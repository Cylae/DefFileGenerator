    ...
    def map_and_clean(self, tables):
        ...
        for table in tables:
            ...
            for row in table:
                ...
                # If values are None, map_and_clean uses empty strings for construction
                sb = str(start_bit).strip() if start_bit is not None else ""
                ln = str(length).strip() if length is not None else ""
                ...
                if '_' in addr:
                    ...
                    # Handle separate StartBit/Length columns
                    if sb or ln:
                        dtype_raw = str(new_row.get('Type', '')).upper()
                        if 'BITS' in dtype_raw and sb and not ln:
                            ln = '1'

                        if sb and ln:
                            new_row['Address'] = f"{base_addr}_{sb}_{ln}"
                        elif ln:
                            new_row['Address'] = f"{base_addr}_{ln}"
                        else:

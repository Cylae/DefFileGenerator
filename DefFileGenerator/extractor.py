    ...
    def map_and_clean(self, tables, address_offset=0):
        ...
        for table in tables:
            ...
            for row in table:
                ...
                # Normalize Address (handle complex parts: Address, Length, StartBit)
                addr_base = str(new_row.get('Address', '')).strip()
                length = str(new_row.get('Length', '')).strip()
                start_bit = str(new_row.get('StartBit', '')).strip()

                if '_' in addr_base:
                    # Already compound, just normalize parts
                    addr = '_'.join(self.generator.normalize_address_val(p) for p in addr_base.split('_'))
                else:
                    # Construct compound address if components exist
                    addr = self.generator.normalize_address_val(addr_base)
                    if length:
                        addr += f"_{self.generator.normalize_address_val(length)}"
                        if start_bit:
                            addr += f"_{self.generator.normalize_address_val(start_bit)}"

                # Apply offset and ensure decimal normalization

...
class Extractor:
    ...
    def map_and_clean(self, tables, address_offset=0):
        ...
        for table in tables:
            ...
            # Robust column detection (scan up to 5 rows)
            col_map = {}
            used_src_cols = set()
            scan_rows = table[:5]

            # Get all available source columns
            all_src_cols = set()
            for r in scan_rows:
            ...
            for target in detection_order:
                ...
                for src_col in all_src_cols:
                    ...
                    if any(p == str(src_col).lower() for p in patterns) or any(p in str(src_col).lower() for p in patterns):
            ...
            for row in table:
                ...
                # Extraction of Length and StartBit
                ...
                # Construct complex address if bits or length are present
                ...
                # Normalize Factor
                new_row['Factor'] = str(new_row.get('Factor', '1'))

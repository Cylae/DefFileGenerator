    ...
    def map_and_clean(self, tables, address_offset=0):
        ...
        for table in tables:
            ...
            for row in table:
                ...
                # Robust compound address construction: only if parts exist and aren't 'None' literal
                if start_bit and start_bit.lower() != 'none' and length and length.lower() != 'none':
                    addr = f"{addr}_{start_bit}_{length}"
                elif length and length.lower() != 'none':

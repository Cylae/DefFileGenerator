            ...
            try:
                with pdfplumber.open(filepath) as pdf:
                    if pages is None:
                        ...
                        target_pages = []
                        requested_indices = pages if isinstance(pages, list) else [pages]
                        for idx in requested_indices:
                            if 1 <= idx <= len(pdf.pages):
                                target_pages.append(pdf.pages[idx-1])
                            else:
                                logging.warning(f"Page {idx} is out of range (Total pages: {len(pdf.pages)}). Skipping.")

...
try:
    ...
    try:
        ...
        Generator = None
        import itertools
        def peek_generator(iterable: Optional[Union[Iterable[Any], Iterator[Any]]]) -> Tuple[bool, Iterator[Any]]:
            """Standalone fallback for peek_generator."""
...
class Extractor:
    ...
    def extract_from_pdf(self, filepath: str, pages: Optional[Union[str, int, List[int]]] = None) -> Iterator[Iterator[Dict[str, Any]]]:
        ...
        # Convert pages arg to list of integers
                    ...
                    if page_list:
                        ...
                        for p_num in page_list:
                            if 1 <= p_num <= len(pdf.pages):
                                ...
                                logging.warning(f"Page {p_num} is out of range (1-{len(pdf.pages)}). Skipping.")
                    else:

...
class Extractor:
    ...
    def extract_from_xml(self, filepath):
        ...
        try:
            ...
            root = tree.getroot()
            # Simple heuristic: find the first level of children that looks like repeated records
            # Or just flatten everything into a list of dicts.
            # Many register XMLs are <root><register>...</register><register>...</register></root>
            data = []
            for child in root:
                record = {}
    ...
    def map_and_clean(self, tables):
        ...
        for table in tables:
            ...
            # Column detection heuristic: scan first 5 rows to gather available keys
            available_keys = set()
            for r in table[:5]:
            ...
            for row in table:
                ...
                # Normalize Address and handle compound formats
                addr = str(new_row.get('Address', '')).strip()
                l_val = str(new_row.get('Length', '')).strip()
                sb_val = str(new_row.get('StartBit', '')).strip()

                if not addr and not (l_val or sb_val): continue

                base_addr = generator.normalize_address_val(addr.split('_')[0] if '_' in addr else addr)

                if new_row['Type'] == 'BITS' and sb_val:
                    # Webdyn format for bits: Addr_StartBit_Length
                    length = l_val if l_val else '1'
                    new_row['Address'] = f"{base_addr}_{sb_val}_{length}"
                elif new_row['Type'] == 'STRING' and l_val:

...
try:
    ...
    try:
        ...
        Generator = None
        # Fallback peek_generator for robustness
        import itertools
...
class Extractor:
    ...
    def extract_from_excel(self, filepath: str, sheet_name: Optional[str] = None) -> Iterator[Iterator[Dict[str, Any]]]:
        ...
        try:
            ...
            def sheets_generator():
                for ws in sheets:
                    def sheet_row_generator() -> Iterator[Dict[str, Any]]:
                    ...
                    yield sheet_row_generator()
                wb.close()

            return sheets_generator()
    ...
    def extract_from_pdf(self, filepath: str, pages: Optional[Union[int, List[int], str]] = None) -> Iterator[Iterator[Dict[str, Any]]]:
        ...
        def pdf_tables_generator():
            try:
                # Internal normalization of pages
                target_pages_indices = []
                if isinstance(pages, str):
                    try:
                        target_pages_indices = [int(p.strip()) for p in pages.split(',')]
                    except ValueError:
                        logging.error(f"Invalid format for pages: {pages}")
                        return
                elif isinstance(pages, int):
                    target_pages_indices = [pages]
                elif isinstance(pages, list):
                    target_pages_indices = pages

                with pdfplumber.open(filepath) as pdf:
                    num_pages = len(pdf.pages)
                    if not target_pages_indices:
                        target_pages = pdf.pages
                    else:
                        target_pages = []
                        for p_idx in target_pages_indices:
                            if 1 <= p_idx <= num_pages:
                                target_pages.append(pdf.pages[p_idx-1])
                            else:
                                logging.warning(f"Page {p_idx} is out of range (1-{num_pages}). Skipping.")
    ...
    def map_and_clean(self, tables: Iterable[Iterable[Dict[str, Any]]], address_offset: int = 0) -> Iterator[Dict[str, Any]]:
        ...
        for table in tables:
            has_data, table = peek_generator(table)

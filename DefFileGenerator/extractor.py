...
class Extractor:
    ...
    def extract_from_xml(self, filepath):
        ...
        try:
            ...
            # Simple heuristic: find all elements that have children but no text
            # and treat them as rows if they have the same tag.
            # More robustly, look for elements with children.
            data = []
            for child in root:
                row = {}
        ...
        for table in tables:
            ...
            for target in detection_order:
                ...
                for src_col in all_keys:
                    ...
                    if str(src_col).lower() in patterns:
            ...
            for row in table:
                ...
                dtype_raw = new_row.get('Type', 'U16')
                dtype = generator.normalize_type(dtype_raw)

                if dtype == 'BITS' and start_bit and not length:
                    length = '1'

                if start_bit and length:
                    # Address_StartBit_Length
                    base_addr = generator.normalize_address_val(addr.split('_')[0])
                    new_row['Address'] = f"{base_addr}_{start_bit}_{length}"
                elif length and dtype == 'STRING':
                    # Address_Length
                    base_addr = generator.normalize_address_val(addr.split('_')[0])

...
class Extractor:
    ...
    def extract_from_xml(self, filepath):
        ...
        try:
            with open(filepath, 'rb') as f:
                content = f.read()
            ET.fromstring(content) # Protect against XXE
    ...
    def map_and_clean(self, tables, address_offset=0):
        ...
        # Determine mapping once from the first non-empty table
        for table in tables:
        ...
        for table in tables:
            for row in table:
                ...
                # Apply offset and normalize via generator

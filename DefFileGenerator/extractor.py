...
class Extractor:
    ...
    def extract_from_xml(self, filepath):
        ...
        try:
            ...
            # Simple heuristic: find elements that have children (likely rows)
            # or treat immediate children of root as rows.
            data = []
            for child in root:
                row = {}
    ...
    def map_and_clean(self, tables):
        ...
        for table in tables:
            ...
            # Gather keys from first 5 rows for better detection
            sample_keys = set()
            for row in table[:5]:
            ...
            for row in table:
                ...
                if factor_val is not None:
                    new_row['Factor'] = str(Generator._parse_numeric(factor_val, default=1.0))

                if 'RegisterType' not in new_row:
                    new_row['RegisterType'] = 'Holding Register'

                # Handle BITS type address construction
                if new_row.get('Type') == 'BITS':
                    ...
                    if start_bit:
                        ...
                        base_addr = new_row['Address'].split('_')[0]

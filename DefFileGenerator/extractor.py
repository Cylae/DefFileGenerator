...
class Extractor:
    ...
    def extract_from_xml(self, filepath):
        ...
        try:
            with open(filepath, 'rb') as f:
                content = f.read()
            ET.fromstring(content) # XXE Protection
    ...
    def map_and_clean(self, tables, address_offset=0):
        ...
        for table in tables:
            ...
            for row in table:
                ...
                # Skip row only if both Name and Address are missing
                if not new_row.get('Name') and not new_row.get('Address'): continue

                # Complex address construction: Combine Address, Length, and StartBit
                ...
                if addr_base:
                    if length and start_bit:
                        address_str = f"{addr_base}_{length}_{start_bit}"
                    elif length:
                        address_str = f"{addr_base}_{length}"
                    else:
                        address_str = addr_base
                    # Normalize and apply offset via generator
                    new_row['Address'] = self.generator.apply_address_offset(address_str, address_offset)
                else:
                    new_row['Address'] = ""

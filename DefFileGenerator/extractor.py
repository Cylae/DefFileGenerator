...
class Extractor:
    ...
    def map_and_clean(self, tables, address_offset=0, args=None):
        ...
        for table in tables:
            ...
            # 1. Explicit mapping (from self.mapping or args.mapping)
            explicit_map = self.mapping
            if args and hasattr(args, 'mapping') and args.mapping:
                try:
                    with open(args.mapping, 'r') as f:
                        explicit_map.update(json.load(f))
                except (Exception, json.JSONDecodeError):
                    pass

            for target, source in explicit_map.items():

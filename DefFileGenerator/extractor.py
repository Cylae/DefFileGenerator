...
class Extractor:
    COLUMN_MAPPING = {
        ...
        'StartBit': ['startbit', 'start bit', 'bit offset', 'bit']
    }
...
def main():
    ...
    if args.mapping:
        try:
            ...
            logging.error(f"Error reading mapping file: {e}"); sys.exit(1)
    ...
    if ext in ['.xlsx', '.xlsm', '.xltx', '.xltm']: raw = extractor.extract_from_excel(args.input_file, args.sheet)
        ...
        try:
            pages = [int(p.strip()) for p in args.pages.split(',')] if args.pages else None
             ...
             logging.error("Invalid pages format. Use comma-separated integers."); sys.exit(1)

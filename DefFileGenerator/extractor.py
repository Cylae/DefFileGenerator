...
class Extractor:
    ...
    def extract_from_xml(self, filepath):
        ...
        try:
            with open(filepath, 'rb') as f:
                content = f.read()
            # Use defusedxml for XXE validation before passing to pandas
    ...
    def map_and_clean(self, tables, address_offset=0):
        ...
        for table in tables:
            ...
            for row in table:
                ...
                # Address construction (compound)
                # Ensure we don't get 'None' strings if the value in row is None
                def _safe_str(val):
                    return str(val).strip() if val is not None else ""

                addr = _safe_str(new_row.get('Address'))
                length = _safe_str(new_row.get('Length'))
                start_bit = _safe_str(new_row.get('StartBit'))

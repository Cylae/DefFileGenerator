...
try:
    from DefFileGenerator.def_gen import Generator, peek_generator as peek_gen_def
    peek_generator = peek_gen_def
except ImportError:
    try:
        from def_gen import Generator, peek_generator as peek_gen_def
        peek_generator = peek_gen_def
    except ImportError:
        Generator = None
...
class Extractor:
    ...
    def extract_from_excel(
        ...
        def sheet_generator(sname: str) -> Iterator[dict[str, Any]]:
            try:
        ...
        def excel_sheets_generator() -> Iterator[Iterator[dict[str, Any]]]:
            if sheet_name:
                yield sheet_generator(sheet_name)
            else:
                try:
                    wb = openpyxl.load_workbook(filepath, data_only=True, read_only=True)
                ...
                if pages is None:
                    ...
                    if isinstance(pages, int):
                        ...
                        try:
                            ...
                            logging.warning(f"Invalid format for pages: {pages}")
    ...
    def extract_from_xml(self, filepath: str) -> Iterator[Iterator[Dict[str, Any]]]:
        ...
        def xml_generator() -> Iterator[Dict[str, Any]]:
            try:
                ...
                logging.error(f"Security risk detected in XML {filepath}")
            except Exception as e:
    ...
    def map_and_clean(self, tables: Optional[Iterable[Iterable[Dict[str, Any]]]], address_offset: int = 0) -> Iterator[Dict[str, Any]]:
        ...
        for table in tables:
            ...
            def process_row(r: Dict[str, Any]) -> Optional[Dict[str, Any]]:
                ...
                if Generator and hasattr(Generator, 'apply_address_offset'):
                    new_row['Address'] = Generator.apply_address_offset(addr, address_offset)
                else:
                    nr['Address'] = addr

                if new_row.get('Factor') is not None:
                    if Generator and hasattr(Generator, '_parse_numeric'):

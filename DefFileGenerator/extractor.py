...
class Extractor:
    ...
    def extract_from_excel(self, filepath: str, sheet_name: Optional[str] = None) -> Iterator[Iterator[Dict[str, Any]]]:
        ...
        def excel_sheets_generator():
            ...
            try:
                ...
                # Note: Closing workbook here might break nested generators if they are evaluated lazily after this.
                # However, openpyxl read_only mode handles this better.
                # For absolute O(1) safety, we should keep it open, but we must avoid leaks.
                if wb:
                    wb.close()

        return excel_sheets_generator()

    def extract_from_pdf(self, filepath: str, pages: Optional[Union[int, str, List[int]]] = None) -> Iterator[Iterator[Dict[str, Any]]]:
        ...
        def pdf_tables_generator():
            try:
                with pdfplumber.open(filepath) as pdf:
                    ...
                    page_indices = []
                    if pages is None:
                        page_indices = list(range(pdf_len))
                    ...
                    for idx in page_indices:
                        if idx < 0 or idx >= pdf_len:
                            logging.warning(f"Page {idx + 1} out of range (1-{pdf_len}). Skipping.")
                            continue
    ...
    def map_and_clean(self, tables: Iterable[Iterable[Dict[str, Any]]], address_offset: int = 0) -> Iterator[Dict[str, Any]]:
        has_data, tables_it = peek_generator(tables)
        if not has_data:
            return

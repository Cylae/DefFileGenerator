...
class Extractor:
            ...
            try:
                with pdfplumber.open(filepath) as pdf:
                    ...
                    if pages is None:
                        ...
                        elif isinstance(pages, str): plist = [int(p.strip()) for p in pages.split(',') if p.strip()]
                        else: plist = [int(p) for p in pages]

                        for pnum in plist:
                            if 1 <= pnum <= total_pages:
                                target_page_indices.append(pnum - 1)
                            else:
                                logging.warning(f"PDF Page {pnum} out of range (1-{total_pages}). Skipping.")
    ...
    def map_and_clean(self, tables: Iterable[Iterable[Dict[str, Any]]], address_offset: int = 0) -> Iterator[Dict[str, Any]]:
        for table in tables:
            has_rows, table_it = peek_generator(table)

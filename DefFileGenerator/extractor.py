...
class Extractor:
    ...
    def map_and_clean(self, tables, address_offset=0):
        ...
        for table in tables:
            ...
            # Use first 5 rows to detect columns
            available_keys = set()
            for row in table[:5]:
            ...
            for row in table:
                ...
                slen = str(new_row.get('Length', '')).strip() if new_row.get('Length') is not None else ""

                # Clean 'None' literal strings
                if sbit.lower() == 'none': sbit = ""
                if slen.lower() == 'none': slen = ""

                if new_row['Type'] == 'BITS' and sbit:
                    if not slen: slen = '1'
                    addr = f"{addr}_{sbit}_{slen}"
                elif slen and slen != '1' and '_' not in addr:
                    addr = f"{addr}_{slen}"

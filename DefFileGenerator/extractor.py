...
class Extractor:
    ...
    def extract_from_pdf(self, filepath: str, pages: Optional[Union[int, str, List[int]]] = None) -> Iterator[Iterator[Dict[str, Any]]]:
        ...
        # Handle pages as string/list/int
                ...
                with pdfplumber.open(filepath) as pdf:
                    ...
                    for p_num in p_list:
                        if 1 <= p_num <= pdf_len:
                            ...
                            logging.warning(f"Page {p_num} is out of range for {filepath} (1-{pdf_len})")
    ...
    def map_and_clean(self, tables: Iterable[Iterable[Dict[str, Any]]], address_offset: int = 0) -> Iterator[Dict[str, Any]]:
        ...
        has_any_data, tables_it = peek_generator(tables)
        if not has_any_data:
            return

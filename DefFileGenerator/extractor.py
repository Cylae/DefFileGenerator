...
class Extractor:
    ...
    def map_and_clean(self, tables, address_offset=0):
        ...
        for table in tables:
            ...
            # Use first 5 rows for column detection
            sample_rows = table[:5]
            col_map = {}
            used_src_cols = set()
            ...
            for target in detection_order:
                ...
                found = False
                for src_col in sample_rows[0].keys():
                    ...
                    if any(p in str(src_col).lower() for p in patterns):
                        ...
                        found = True
                        break
                if not found and len(sample_rows) > 1:
            ...
            for row in table:
                ...
                # Complex address construction if Length/StartBit available
                ...
                if addr and start_bit and length:
                    addr = f"{addr}_{start_bit}_{length}"
                elif addr and length:

...
class Extractor:
    ...
    def extract_from_xml(self, filepath):
        if not HAS_DEFUSEDXML or not HAS_PANDAS:
            logging.error("defusedxml and pandas are required for XML extraction.")
            return []
        try:
            with open(filepath, 'rb') as f:
                content = f.read()
    ...
    def map_and_clean(self, tables, address_offset=0):
        ...
        for table in tables:
            ...
            sample_rows = table[:5]
            all_src_cols = list(table[0].keys())
            ...
            for target in detection_order:
                ...
                for src_col in all_src_cols:
                    ...
                    # Check if any pattern matches the column name or content in first 5 rows
                    col_name_lower = str(src_col).lower()
            ...
            for row in table:
                ...
                ln = str(new_row.get('Length', '')).strip() if new_row.get('Length') is not None else ""

                if sb or ln:
                    # Construct composite address parts, normalizing None/empty to empty string
                    parts = [addr, sb, ln]
                    # Filter out trailing empty parts but keep middle ones if necessary
                ...
                new_row['Factor'] = str(self.generator._parse_numeric(new_row.get('Factor'), default=1.0))

...
class Extractor:
    ...
    def map_and_clean(self, tables, address_offset=0):
        ...
        for table in tables:
            ...
            # Scan first 5 rows for column detection
            sample_rows = table[:5]
            all_cols = set()
            ...
            for row in table:
                ...
                addr_base = clean_val(new_row.get('Address'))
                length = clean_val(new_row.get('Length'))
                start_bit = clean_val(new_row.get('StartBit'))

                if length and start_bit:
                    addr = f"{addr_base}_{start_bit}_{length}"
                elif length:
                    addr = f"{addr_base}_{length}"
                else:
                    addr = addr_base

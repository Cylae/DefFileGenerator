...
class Extractor:
    ...
    def extract_from_xml(self, filepath):
        ...
        try:
            with open(filepath, 'rb') as f:
                content = f.read()
            # Security check: use defusedxml to check for entities
            ET.fromstring(content)
            # Use pandas read_xml but specify io.BytesIO to ensure content is handled as bytes
            # and use etree parser for consistency
    ...
    def map_and_clean(self, tables, address_offset=0):
        ...
        for table in tables:
            ...
            for row in table:
                ...
                new_row['Address'] = self.generator.apply_address_offset(addr_raw, address_offset)

                # Normalize Type
                new_row['Type'] = self.normalize_type(new_row.get('Type', 'U16'))

                # Factor normalization (handled robustly in Generator._calculate_coefficients)
                # But here we ensure it's at least passed along
                if 'Factor' not in new_row or new_row['Factor'] is None:
                    new_row['Factor'] = '1'

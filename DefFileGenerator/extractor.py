...
class Extractor:
    ...
    def normalize_type(t: Any) -> str:
        if Generator: return Generator.normalize_type(t)
    ...
    def extract_from_excel(
        ...
        def excel_sheets_generator():
            ...
            except (OSError, zipfile.BadZipFile) as e: logging.error(f"File IO Error extracting from Excel {filepath}: {e}")
            except Exception as e: logging.error(f"Error extracting from Excel {filepath}: {e}")
            finally:
                if wb: wb.close()
        ...
        def pdf_tables_generator() -> Iterator[Iterator[Dict[str, Any]]]:
            try:
                with pdfplumber.open(filepath) as pdf:
                    target_pages = []
                    if pages is None: target_pages = pdf.pages
                    else:
                        requested = pages if isinstance(pages, (list, tuple)) else str(pages).split(',')
                        for p in requested:
                            try:
                                idx = int(p) - 1
                                if 0 <= idx < len(pdf.pages): target_pages.append(pdf.pages[idx])
                                else: logging.warning(f"Page {p} is out of range (1-{len(pdf.pages)})")
                            except (ValueError, TypeError): logging.warning(f"Invalid page reference: {p}")
                    for page in target_pages:
                        ...
                        for table in tables:
                            ...
                            for row in table[1:]:
                                ...
                                if any(v.strip() for v in row_dict.values() if v): rows_data.append(row_dict)
                            if rows_data: yield iter(rows_data)
            except (OSError,) + PDF_ERRORS as e: logging.error(f"File IO Error or PDF Syntax Error extracting from PDF {filepath}: {e}")
            except Exception as e: logging.error(f"Error extracting from PDF {filepath}: {e}")
            ...
            try:
                ...
                with open(filepath, 'r', encoding=encoding) as f:
                    ...
                    try: dialect = csv.Sniffer().sniff(snippet, delimiters=";,")
                        ...
                        dialect = csv.excel; dialect.delimiter = delimiter
                    reader = csv.DictReader(f, dialect=dialect)
                    for row in reader:
                        if any(val.strip() for val in row.values() if val is not None): yield dict(row)
            except Exception as e: logging.error(f"Error extracting from CSV {filepath}: {e}")
                ...
                def xml_generator() -> Iterator[Dict[str, Any]]:
                    ...
                    for elem in root.iter():
                        ...
                        if len(row) >= 2:
                            ...
                            if js not in seen: seen.add(js); yield row
                yield xml_generator()
            except SECURITY_EXCEPTIONS as e: logging.error(f"XML Security Exception in {filepath}: {e}")
            except (OSError,) + XML_PARSE_ERRORS as e: logging.error(f"File IO Error or Parsing Error extracting from XML {filepath}: {e}")
            except Exception as e: logging.error(f"Error extracting from XML {filepath}: {e}")
    ...
    def map_and_clean(self, tables: Optional[Iterable[Iterable[Dict[str, Any]]]], address_offset: int = 0) -> Iterator[Dict[str, Any]]:
        ...
        for table in tables:
            has_rows, it = peek_generator(table)
            if not has_rows: continue
            buffer = []
            iterator = iter(table_it)
            try:
                for _ in range(50): buffer.append(next(it))
            ...
            for target, source in self.mapping.items():
                if source in all_keys: col_map[target] = source; used_src_cols.add(source)
            ...
            for target in detection_order:
                ...
                for src_col in all_keys:
                    ...
                    if s_low in patterns: col_map[target] = src_col; used_src_cols.add(src_col); break
            for target in detection_order:
                if target in col_map: continue
                patterns = self.COLUMN_MAPPING.get(target, [target.lower()])
                for src_col in all_keys:
                    if src_col in used_src_cols: continue
                    if any(p in str(src_col).lower() for p in patterns): col_map[target] = src_col; used_src_cols.add(src_col); break
            ...
            def process_row(r: Dict[str, Any]) -> Optional[Dict[str, Any]]:
                ...
                if dtype == 'BITS' and sbit != '' and '_' not in addr:
                    ...
                    addr = f"{addr}_{slen}"
                new_row['Address'] = Generator.apply_address_offset(addr, address_offset) if Generator else addr
                if new_row.get('Factor') is not None and Generator:
                    new_row['Factor'] = str(Generator._parse_numeric(new_row['Factor'], 1.0))
                if not new_row.get('RegisterType'): new_row['RegisterType'] = 'Holding Register'
                return new_row

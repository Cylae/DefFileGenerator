    ...
    try:
        ...
        def peek_generator(iterable: Any) -> Tuple[Optional[Any], Iterator[Any]]:
            ...
            try:
                it = iter(iterable)
                first = next(it)
            except (StopIteration, TypeError): return None, iter([])
...
class Extractor:
            ...
            try:
                with pdfplumber.open(filepath) as pdf:
                    try:
                        ...
                        logging.warning(f"Requested pages {pages} out of range for PDF {filepath}")
                        target_pages = []
    ...
    def map_and_clean(self, tables: Iterable[Iterable[Dict[str, Any]]], address_offset: int = 0) -> Iterator[Dict[str, Any]]:
        ...
        for table in tables:
            first_row, table = peek_generator(table)

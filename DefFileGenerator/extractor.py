...
class Extractor:
    ...
    def extract_from_excel(self, filepath: str, sheet_name: Optional[str] = None) -> Iterator[Iterator[Dict[str, Any]]]:
        ...
        def excel_gen():
            ...
            except (OSError, zipfile.BadZipFile, ValueError, TypeError, KeyError) as e:
            ...
            try:
                with pdfplumber.open(filepath) as pdf:
                    if pages is None:
                        ...
                        target_indices = [int(p.strip()) - 1 for p in pages.split(',')]
                    else:
                        target_indices = [p - 1 for p in pages]

                    for idx in target_indices:
                        if idx < 0 or idx >= len(pdf.pages):
                            logging.warning(f"Page {idx+1} out of range (1-{len(pdf.pages)})")
                            continue
                        page = pdf.pages[idx]

...
class Extractor:
    COLUMN_MAPPING = {
        ...
        'Offset': ['offset', 'coefb', 'bias', 'coefficient b'],
            ...
            with pdfplumber.open(filepath) as pdf:
                ...
                for page in target_pages:
                    ...
                    for table in tables:
                        ...
                        if table_data: all_tables.append(table_data)
    ...
    def map_and_clean(self, tables, address_offset=0):
        ...
        for table in tables:
            ...
            # Robust detection: scan first 5 rows
            sample_rows = table[:5]
            all_src_cols = set()
            for r in sample_rows: all_src_cols.update(r.keys())
            ...
            for row in table:
                ...
                # Address construction from Address, StartBit, Length
                ...
                # Factor/Offset fractions
                for k in ['Factor', 'Offset']:
                    val = str(new_row.get(k, ''))
                    if '/' in val:
                        try:
                            p = val.split('/')
                            new_row[k] = str(float(p[0]) / float(p[1]))
                        except (ValueError, ZeroDivisionError):
                            new_row[k] = '1' if k == 'Factor' else '0'

...
class Extractor:
    ...
    def extract_from_xml(self, filepath):
        ...
        try:
            ...
            # Find all nodes that look like table rows (usually direct children of root or children of a list node)
            # We'll take the first level of children that has consistent tags
            data = []
            for child in root:
                row_dict = {}
    ...
    def map_and_clean(self, tables):
        ...
        for table in tables:
            ...
            # Scan first 5 rows to identify headers if the first row is sparse
            all_keys = set()
            for i in range(min(5, len(table))):
                all_keys.update(str(k) for k in table[i].keys() if k)
            ...
            detection_order = ['RegisterType', 'Address', 'Name', 'Type', 'Unit', 'Action', 'Tag', 'Factor', 'ScaleFactor', 'Offset', 'Length', 'StartBit']
            ...
            for row in table:
                ...
                # Handle StartBit and Length for compound addresses
                start_bit = new_row.get('StartBit')
                length = new_row.get('Length')

                # If it's a BITS type and we have start_bit, ensure we have length
                if str(new_row.get('Type', '')).upper() == 'BITS' or 'bit' in str(new_row.get('Type', '')).lower():
                    if start_bit is not None and str(start_bit).strip():
                        sb = str(start_bit).strip()
                        ln = str(length).strip() if length is not None and str(length).strip() else "1"
                        if '_' not in new_row['Address']:
                            new_row['Address'] = f"{new_row['Address']}_{sb}_{ln}"
                elif length is not None and str(length).strip() and '_' not in new_row['Address']:
                    # Handle string or other multi-register types that use Address_Length
                    if 'STR' in str(new_row.get('Type', '')).upper():
                        new_row['Address'] = f"{new_row['Address']}_{str(length).strip()}"

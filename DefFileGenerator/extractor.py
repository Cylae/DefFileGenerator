...
try:
    ...
    try:
        ...
        def peek_generator(iterable: Any) -> Tuple[bool, Iterable[Any]]:
            if iterable is None: return True, iter([])
            it = iter(iterable)
            try: first = next(it)
            except StopIteration: return True, iter([])
            ...
            def sheets_wrapper():
                try:
                    for ws in sheets:
                        ...
                        is_empty, gen = peek_generator(sheet_generator())
                        if not is_empty:
                            yield gen
                finally:
                    wb.close()

            return sheets_wrapper()
                ...
                with pdfplumber.open(filepath) as pdf:
                    if pages is None:
                        ...
                        for i in page_indices:
                            if 1 <= i <= len(pdf.pages):
                                ...
                                logging.warning(f"Page {i} out of range (1-{len(pdf.pages)}). Skipping.")
                    ...
                    for page in target_pages:
                        ...
                        for table in tables:
                            ...
                            def table_generator(current_table):
                            ...
                            is_empty, gen = peek_generator(table_generator(table))
                            if not is_empty:
                                yield gen

...
# Import Generator and peek_generator from def_gen if available
try:
...
class Extractor:
    ...
    def extract_from_excel(
        ...
        try:
            ...
            for ws in sheets:
                def sheet_generator(ws_obj) -> Iterator[Dict[str, Any]]:
        ...
        # Note: In read_only mode, we don't strictly need to close if we don't want to block,
        # but it's good practice. However, closing might invalidate generators.
        # For simplicity in this tool, we assume small enough files or that the user processes them.
        ...
        try:
            ...
            for page in target_pages:
                ...
                for table in tables:
                    ...
                    # Exhaust the table generator to ensure it's not tied to an open file if we were to close it
                    # But here we keep the pdf open until the end of the method.
                    yield table_generator(table)

            # pdf.close() # Closing here would invalidate generators if they are lazy.
            # In this design, we might want to convert to list or keep it open.
        ...
        def csv_table_generator() -> Iterator[Dict[str, Any]]:
            try:
                ...
                with open(filepath, 'r', encoding=encoding) as f:
                    ...
                    for row in reader:
                        if any(str(val).strip() for val in row.values() if val is not None):
                            yield dict(row)
            except Exception as e:

...
class Extractor:
        ...
        try:
            with pdfplumber.open(filepath) as pdf:
                if pages is None:
                    ...
                    target_pages = [pdf.pages[i-1] for i in (pages if isinstance(pages, list) else [pages]) if 0 < i <= len(pdf.pages)]
    ...
    def map_and_clean(self, tables, address_offset=0):
        ...
        for table in tables:
            ...
            potential_headers = table[:5]
            col_map = {}
            used_src_cols = set()

            # 1. Explicit mapping
            for target, source in self.mapping.items():
                for row in potential_headers:
                    if source in row:
            ...
            for target in detection_order:
                ...
                # Find which source column matches any pattern for this target
                for row in potential_headers:
                    matched = False
            ...
            for row in table:
                ...
                # Complex address construction from separate columns
                addr = str(new_row.get('Address', '')).strip()
                length = str(new_row.get('Length', '')).strip()
                start_bit = str(new_row.get('StartBit', '')).strip()

                if length and length.lower() != 'none':
                    if start_bit and start_bit.lower() != 'none':

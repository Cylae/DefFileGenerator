    ...
    def map_and_clean(self, tables, address_offset=0):
        ...
        for table in tables:
            ...
            for row in table:
                ...
                # Apply address offset and normalize via generator
                if addr:
...
def main():
    ...
    if ext in ['.xlsx', '.xlsm', '.xltx', '.xltm']: raw = extractor.extract_from_excel(args.input_file, args.sheet)
        ...
        pages = [int(p) for p in args.pages.split(',')] if hasattr(args, 'pages') and args.pages else None
    ...
    mapped = extractor.map_and_clean(raw, args.address_offset if hasattr(args, 'address_offset') else 0)

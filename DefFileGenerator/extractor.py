...
class Extractor:
    COLUMN_MAPPING = {
        ...
        'Offset': ['coefb', 'bias', 'coefficient b'],
    ...
    def map_and_clean(self, tables, address_offset=0):
        ...
        for table in tables:
            ...
            # Robust column detection: scan first 5 rows to find target columns
            col_map = {}
            used_src_cols = set()
            headers = set()
            for r in table[:5]:
            ...
            for row in table:
                ...
                if l_val:
                    if sb_val:
                        new_row['Address'] = f"{new_row['Address']}_{sb_val}_{l_val}"
                    else:
                        new_row['Address'] = f"{new_row['Address']}_{l_val}"
...
def main():
    ...
    elif ext == '.pdf': raw = extractor.extract_from_pdf(args.input_file, [int(p.strip()) for p in args.pages.split(',')] if args.pages else None)

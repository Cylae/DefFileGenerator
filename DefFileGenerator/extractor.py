...
class Extractor:
    ...
    def extract_from_excel(self, filepath, sheet_name=None):
        """Extracts all tables from all sheets or a specific sheet as a list of tables."""
        if not HAS_OPENPYXL:
    ...
    def extract_from_pdf(self, filepath, pages=None):
        """Extracts multiple tables from the specified PDF pages."""
    ...
    def extract_from_csv(self, filepath):
        """Extracts a single table from a CSV file."""
        try:
    ...
    def extract_from_xml(self, filepath):
        """Extracts a single table from an XML file using defusedxml for security."""
    ...
    def map_and_clean(self, tables, address_offset=0):
        """Maps source columns to targets and applies address offsets."""
        ...
        for table in tables:
            ...
            for row in table:
                ...
                addr_val = str(new_row.get('Address', '')).strip()
                if addr_val:
                    new_row['Address'] = self.generator.apply_address_offset(addr_val, address_offset)

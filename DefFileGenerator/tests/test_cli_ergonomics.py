...
class TestCliErgonomics(unittest.TestCase):
    ...
    def test_unsupported_extension_lists_supported_formats(self):
        ...
        os.close(fd)
        try:
            result = run_cli("run", bogus, "--manufacturer", "A", "--model", "B", "-o", self.target)
    ...
    def test_missing_input_file_is_reported(self):
        result = run_cli(
            "run", "/nonexistent/path.csv", "--manufacturer", "A", "--model", "B", "-o", self.target
        )
        self.assertEqual(result.returncode, 1)
    ...
    def test_existing_output_is_not_silently_overwritten(self):
        with open(self.target, "w", encoding="utf-8") as f:
            f.write("SENTINEL\n")
        result = run_cli("run", FIXTURE, "--manufacturer", "A", "--model", "B", "-o", self.target)
        self.assertEqual(result.returncode, 1)
    ...
    def test_force_allows_overwrite(self):
        with open(self.target, "w", encoding="utf-8") as f:
            f.write("SENTINEL\n")
        result = run_cli(
            "run", FIXTURE, "--manufacturer", "A", "--model", "B", "-o", self.target, "--force"
        )
        self.assertEqual(result.returncode, 0)
        ...
        for flag in ("-v", "-q", "--quiet", "--verbose"):
            with self.subTest(flag=flag):
                if os.path.exists(self.target):
                    os.unlink(self.target)
                result = run_cli(
                    "run", FIXTURE, "--manufacturer", "A", "--model", "B", "-o", self.target, flag
                )
                self.assertEqual(result.returncode, 0, result.stderr)

    def test_quiet_suppresses_informational_output(self):
        result = run_cli(
            "run", FIXTURE, "--manufacturer", "A", "--model", "B", "-o", self.target, "--quiet"
        )
        self.assertEqual(result.returncode, 0)

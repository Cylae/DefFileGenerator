...
def _perform_extraction(args):
    ...
    if ext in ['.xlsx', '.xlsm', '.xltx', '.xltm']:
        ...
        pages_str = args.pages if hasattr(args, 'pages') else None
    ...
    return extractor.map_and_clean(raw_data, offset)
...
def generate_command(args):
    config = GeneratorConfig(
        ...
        template=args.template if hasattr(args, 'template') else False,
        address_offset=args.address_offset if hasattr(args, 'address_offset') else 0
    )
    run_generator(config)

def run_command(args):
    # Pass address_offset to extraction to perform offsetting there
    mapped_data = _perform_extraction(args)
    if not mapped_data:
        return
    ...
    try:
        # Pass address_offset=0 to generator because offset is already applied during extraction
        config = GeneratorConfig(
            input_file=temp_csv,
            output=args.output,
            manufacturer=args.manufacturer,
...
def main():
    ...
    if args.command == 'extract':
        ...
        extract_input_file = args.input_file
        if args.template:
             generate_command(args)
        elif not extract_input_file or not args.manufacturer or not args.model:
             logging.error("input_file, --manufacturer and --model are required for generation.")
        else:
             generate_command(args)

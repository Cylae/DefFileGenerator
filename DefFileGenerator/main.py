...
def validate_command(args):
    gen = Generator()
    is_valid = gen.validate_csv(args.input_file)
    if is_valid:
        logging.info(f"Definition file {args.input_file} is VALID.")
    else:
        logging.error(f"Definition file {args.input_file} is INVALID.")
        sys.exit(1)

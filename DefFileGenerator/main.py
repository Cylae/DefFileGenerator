...
def run_command(args):
    ...
    try:
        config = GeneratorConfig(
            ...
            category=args.category,
            forced_write=args.forced_write,
            address_offset=0 # Offset already applied in extraction for run command
        )
        run_generator(config)
    finally:
...
def main():
    ...
    parser_generate.add_argument('--manufacturer', required=False) # Optional if --template
    parser_generate.add_argument('--model', required=False) # Optional if --template
    ...
    if args.command == 'extract':
        ...
        if not args.template and (not args.manufacturer or not args.model):
            parser_generate.error("--manufacturer and --model are required unless using --template")

...
def extract_command(args):
    mapped_data = _perform_extraction(args, args.address_offset)
    if not mapped_data:
        return
...
def run_command(args):
    # Pass offset to extraction, and 0 to generation to prevent double-offsetting
    mapped_data = _perform_extraction(args, args.address_offset)
    if not mapped_data:
        return
...
def main():
    ...
    parser_generate.add_argument('input_file', nargs='?', help='Input simplified CSV.')

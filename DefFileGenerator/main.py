...
def run_command(args):
    ...
    if not output and not template:
        output = f"{re.sub(r'[^a-zA-Z0-9]', '_', m_name).lower()}_{re.sub(r'[^a-zA-Z0-9]', '_', m_model).lower()}_definition.csv"
...
def _run_cli(args=None):
    ...
    # Allow passing file directly (implicit 'run')
    if args is not None and len(args) > 0 and args[0] not in ['validate', 'extract', 'generate', 'run'] and not args[0].startswith('-'):
        # Only do this if it's not a subcommand
        subparser_needed = False
    else:
        subparser_needed = True
    ...
    command = getattr(parsed_args, 'command', 'run' if not subparser_needed else None)

    if not command and subparser_needed:
        parser.print_help()
        return
    ...
    if command in ['generate', 'run'] and not getattr(parsed_args, 'template', False):
        if not getattr(parsed_args, 'manufacturer', None) or not getattr(parsed_args, 'model', None):
            logging.error("--manufacturer and --model are required.")
            sys.exit(1)
        if command == 'generate' and not getattr(parsed_args, 'input_file', None):
            logging.error("input_file is required for generate.")
            sys.exit(1)
        if command == 'run' and not getattr(parsed_args, 'input_file', None):
            logging.error("input_file is required for run.")
            sys.exit(1)

    if command == 'extract':
        extract_command(parsed_args)
    elif command == 'validate':
        validate_command(parsed_args)
    elif command == 'generate':
        generate_command(parsed_args)
    elif command == 'run':

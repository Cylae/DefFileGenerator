...
def extract_command(args):
    mapped_data_raw = _perform_extraction(args)
    first, mapped_data = peek_generator(mapped_data_raw)
    if first is None:
...
def _run_cli():
    ...
    parser_run.add_argument('--template', action='store_true', help='Generate a sample template')
    ...
    # Validate --pages
    pages_arg = getattr(args, 'pages', None)
    if pages_arg and args.input_file:
        ext = os.path.splitext(args.input_file)[1].lower()
        if ext != '.pdf':
            logging.warning("--pages is only applicable for PDF files. Ignoring.")
        else:

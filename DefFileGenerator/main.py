...
def _perform_extraction(args):
    ...
    # If the command has an address-offset, pass it here.
    # For 'run', it will apply it during extraction.
    # For 'extract', it also applies it if present.
...
def run_command(args):
    # Pass address_offset to _perform_extraction (which calls map_and_clean)
    mapped_data = _perform_extraction(args)
    if not mapped_data:
        return

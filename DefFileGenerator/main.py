...
def run_command(args):
    # Perform extraction with the address offset applied here
    mapped_data = _perform_extraction(args)
    if not mapped_data:
        return
    ...
    try:
        # Since offset was applied in extraction, we pass 0 here to avoid double-offsetting
        config = GeneratorConfig(
            input_file=temp_csv,
            output=args.output,
            manufacturer=args.manufacturer,
...
def main():
    ...
    parser_generate.add_argument('--template', action='store_true', help='Generate a sample template')

...
def extract_command(args):
    ...
    if isinstance(output, str):
        if not output: # Support empty string as stdout
            f = sys.stdout
        else:
...
def run_command(args):
    # Pass address_offset to _perform_extraction (which passes it to map_and_clean)
    mapped_data = _perform_extraction(args)
    if not mapped_data:
        return
    ...
    try:
        config = GeneratorConfig(
            ...
            category=args.category,
            forced_write=args.forced_write,
            address_offset=0 # To prevent double-offsetting
        )
        run_generator(config)
    finally:

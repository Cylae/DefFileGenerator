...
def _perform_extraction(args):
    ...
    # Apply address offset if requested (for extract and run commands)
...
def run_command(args):
    # For run command, we apply offset during extraction
    mapped_data = _perform_extraction(args)
    if not mapped_data:
        return
    ...
    try:
        # We pass address_offset=0 here because it was already applied in _perform_extraction
        config = GeneratorConfig(
            input_file=temp_csv,
            output=args.output,
            manufacturer=args.manufacturer,

...
def _perform_extraction_raw(args):
    mapping = {}
    if args.mapping:
        try:
    ...
    if not args.input_file or not os.path.exists(args.input_file):
        logging.error(f"Input file not found or not specified: {args.input_file}")
        sys.exit(1)
...
def extract_command(args):
    raw_iterator = _perform_extraction_raw(args)
    first, mapped_iterator = peek_generator(raw_iterator)
    if first is None:
...
def run_command(args):
    ...
    run_generator(config, input_data=mapped_iterator)

def validate_command(args):
    gen = Generator()
    if not gen.validate_csv(args.input_file):

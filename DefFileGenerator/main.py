...
def _perform_extraction(args):
    ...
    if hasattr(args, 'pages') and args.pages:
        try:
            ...
            logging.error(f"Invalid --pages argument: {args.pages}. Must be comma-separated integers.")
...
def generate_command(args):
    ...
    if not is_template and (not args.input_file or not args.manufacturer or not args.model):
...
def run_command(args):
    ...
    try:
        config = GeneratorConfig(
            ...
            category=args.category,
            forced_write=args.forced_write,
            address_offset=0  # Passed via extraction
        )
        run_generator(config)
    finally:

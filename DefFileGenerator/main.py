...
def _perform_extraction(args):
    if not hasattr(args, 'input_file') or not args.input_file:
        logging.error("No input file provided for extraction.")
        return []
...
def run_command(args):
    # For 'run', we apply offset during extraction and pass 0 to generator
    mapped_data = _perform_extraction(args)
    if not mapped_data:
        return
...
def main():
    ...
    if args.command == 'extract':
        ...
        if not args.template and (not args.input_file or not args.manufacturer or not args.model):
            parser_generate.error("the following arguments are required: input_file, --manufacturer, --model (unless --template is used)")

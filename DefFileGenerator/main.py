...
def run_command(args):
    ...
    try:
        config = GeneratorConfig(
            ...
            category=args.category,
            forced_write=args.forced_write,
            address_offset=0 # Prevent double-offsetting
        )
        run_generator(config)
    finally:
...
def main():
    ...
    parser_generate.add_argument('--template', action='store_true', help='Generate template file')
    ...
    if args.command == 'extract':
        ...
        if not args.template and (not args.input_file or not args.manufacturer or not args.model):
            parser_generate.error("input_file, --manufacturer and --model are required unless --template is used")

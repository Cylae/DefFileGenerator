...
def _perform_extraction(args):
    ...
    if hasattr(args, 'mapping') and args.mapping:
        try:
            ...
            logging.error(f"Error reading mapping file: {e}"); return []
    ...
    if ext in ['.xlsx', '.xlsm', '.xltx', '.xltm']: raw_data = extractor.extract_from_excel(args.input_file, sheet)
    elif ext == '.pdf':
        try:
            pages = [int(p.strip()) for p in pages_str.split(',')] if pages_str else None
        except ValueError:
            logging.error("Invalid pages argument. Must be comma-separated integers."); return []
        raw_data = extractor.extract_from_pdf(args.input_file, pages)
    elif ext == '.csv': raw_data = extractor.extract_from_csv(args.input_file)
    elif ext == '.xml': raw_data = extractor.extract_from_xml(args.input_file)
    else: logging.error(f"Unsupported extension: {ext}"); return []
...
def extract_command(args):
    ...
    if isinstance(output, str): f = open(output, 'w', newline='', encoding='utf-8')
    else: f = output
    writer = csv.DictWriter(f, fieldnames=fieldnames, extrasaction='ignore')
    writer.writeheader(); writer.writerows(mapped_data)
    if isinstance(output, str): f.close(); logging.info(f"Extraction complete. Saved to {args.output}")
...
def generate_command(args):
    config = GeneratorConfig(
        ...
        forced_write=args.forced_write, template=getattr(args, 'template', False),
        address_offset=getattr(args, 'address_offset', 0)
    )
...
def run_command(args):
    ...
    try:
        config = GeneratorConfig(
            input_file=temp_csv, output=args.output,
            manufacturer=args.manufacturer, model=args.model,
            protocol=args.protocol, category=args.category,
            forced_write=args.forced_write, address_offset=0
        )
        run_generator(config)
    finally:
...
def main():
    ...
    if args.command == 'extract': extract_command(args)
        ...
        if not args.template and (not args.manufacturer or not args.model):
            parser_generate.error("--manufacturer and --model are required unless --template is used")

...
def validate_command(args):
    ...
    if generator.validate_csv(args.input_file):
        ...
        logging.error(f"File '{args.input_file}' failed validation.")
        sys.exit(1)
...
def _run_cli():
    ...
    # Manufacturer and Model are required for generate/run unless --template is used
    if args.command in ['generate', 'run'] and not getattr(args, 'template', False):
        if not getattr(args, 'manufacturer', None) or not getattr(args, 'model', None):
            parser.error(f"the following arguments are required for {args.command}: --manufacturer, --model")

...
def extract_command(args):
    ...
    mapped_data = _perform_extraction(args, address_offset=offset)
    if not mapped_data:
        return
...
def run_command(args):
    ...
    # Use delete=False for named temporary file to ensure Generator can open it
    ...
    try:
        config = GeneratorConfig(
            ...
            category=args.category,
            forced_write=args.forced_write,
            address_offset=0 # Already applied in extraction
        )
        run_generator(config)
    finally:
...
def main():
    ...
    parser.add_argument('--address-offset', type=int, default=0, help='Global address offset (applies to extract and run commands)')
    ...
    parser_generate.add_argument('--address-offset', type=int, default=0, help='Address offset specifically for generate (overrides global if provided)')

...
def _perform_extraction(args):
    ...
    # Pass address_offset from args if it exists, otherwise default to 0
    ...
    if ext in ['.xlsx', '.xlsm', '.xltx', '.xltm']:
        ...
        pages = [int(p.strip()) for p in args.pages.split(',')] if getattr(args, 'pages', None) else None
...
def generate_command(args):
    if not args.template and (not args.input_file or not args.manufacturer or not args.model):
...
def run_command(args):
    if not args.input_file or not args.manufacturer or not args.model:
        logging.error("input_file, --manufacturer, and --model are required for run.")
        return

    # To prevent double-offsetting, we apply it during extraction and pass 0 to generator
    address_offset = args.address_offset

    # We temporarily override args.address_offset for extraction
    original_offset = args.address_offset
    # Actually, _perform_extraction uses args.address_offset
    mapped_data = _perform_extraction(args)
    if not mapped_data:
        return

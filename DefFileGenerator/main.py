...
def _perform_extraction(args):
    ...
    sheet = args.sheet if hasattr(args, 'sheet') else None
...
def run_command(args):
    # Pass address_offset to extraction to apply it
    mapped_data = _perform_extraction(args)
    if not mapped_data:
        return
    ...
    try:
        # Pass address_offset=0 to generator to avoid double-offsetting
        config = GeneratorConfig(
            input_file=temp_csv,
            output=args.output,
            manufacturer=args.manufacturer,
...
def main():
    ...
    if args.command == 'extract':
        ...
        if not args.template and (not args.manufacturer or not args.model or not args.input_file):
            parser_generate.error("manufacturer, model and input_file are required unless --template is used.")

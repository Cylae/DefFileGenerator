...
def _perform_extraction(args):
    ...
    # Address offset is applied during extraction if using 'run' or 'extract'
    # But for 'run', we apply it here and then pass offset=0 to the generator
...
def generate_command(args):
    ...
    if not args.input_file or not args.manufacturer or not args.model:
        logging.error("input_file, --manufacturer, and --model are required for generate.")
        return
...
def run_command(args):
    ...
    try:
        # Pass address_offset=0 because it was already applied during extraction
        config = GeneratorConfig(
            input_file=temp_csv,
            output=args.output,
            manufacturer=args.manufacturer,
...
def main():
    ...
    parser_extract.add_argument('--address-offset', type=int, default=0, help='Initial address offset')

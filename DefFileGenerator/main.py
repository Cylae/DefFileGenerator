...
def _perform_extraction(args):
    ...
    has_data, it = peek_generator(raw_data)
    if not has_data:
        logging.error("No data extracted.")
        sys.exit(1)

    return extractor.map_and_clean(it, address_offset)

def extract_command(args):
    mapped_data = _perform_extraction(args)
    has_registers, it = peek_generator(mapped_data)
...
def run_command(args):
    ...
    run_generator(config, input_data=it)

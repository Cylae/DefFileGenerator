...
def get_tables(args, extractor):
    ext = os.path.splitext(args.input_file)[1].lower()
    if ext in ['.xlsx', '.xlsm', '.xltx', '.xltm', '.xls']:
        return extractor.extract_from_excel(args.input_file, args.sheet)
    elif ext == '.pdf':
        pages = [int(p.strip()) for p in args.pages.split(',')] if args.pages else None
        return extractor.extract_from_pdf(args.input_file, pages)
    elif ext == '.csv':
        return extractor.extract_from_csv(args.input_file)
    elif ext == '.xml':
        return extractor.extract_from_xml(args.input_file)
    else:
...
def extract_command(args):
    ...
    tables = get_tables(args, extractor)
...
def main():
    ...
    parser_generate.add_argument('--address-offset', type=int, default=0, help='Value to subtract from addresses')
    ...
    parser_run.add_argument('--address-offset', type=int, default=0, help='Value to subtract from addresses')

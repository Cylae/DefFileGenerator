...
class Generator:
    ...
    def normalize_type(self, dtype):
        """Standardizes data type synonyms using regex and specificity ordering."""
        if not dtype:
            return 'U16'
        t = str(dtype).lower().strip()

        # Specificity ordering: float64 before float
        if 'float64' in t or 'double' in t or 'f64' in t:
            return 'F64'
        if 'float32' in t or 'f32' in t or 'float' in t:
            return 'F32'

        # Common synonyms with regex
        t = t.replace('unsigned ', 'u').replace('signed ', 'i')
        t = re.sub(r'uint(8|16|32|64)', r'u\1', t)
        t = re.sub(r'int(8|16|32|64)', r'i\1', t)
        t = t.replace('uint', 'u').replace('int', 'i').replace(' ', '')

        # Check if it now matches core types (preserving suffixes)
        t_upper = t.upper()
        if RE_TYPE_INT.match(t_upper) or RE_TYPE_STR_CONV.match(t_upper) or t_upper in ['STRING', 'BITS', 'IP', 'IPV6', 'MAC', 'F32', 'F64']:
            return t_upper

        return t_upper if t else 'U16'

    def normalize_action(self, action):
        """Maps action synonyms to numeric codes."""
        if not action or not str(action).strip():
            return '1' # Default: Read on change

        a = str(action).upper().strip()
        if a in ['R', 'READ', 'READONLY', '4']:
            return '4' # Read on request
        if a in ['RW', 'W', 'WRITE', 'READWRITE', '1']:
            return '1' # Read on change
    ...
    def normalize_address_val(self, addr_part):
        ...
        # Heuristic for hex: contains A-F and doesn't start with - (for negative decimal)
        if not addr_part.startswith('-') and any(c in addr_part.upper() for c in 'ABCDEF'):
            try:
                return str(int(addr_part, 16))
            except ValueError:
                return addr_part
    ...
    def process_rows(self, rows):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            if not self.validate_type(dtype):
                logging.warning(f"Line {line_num}: Invalid Type '{raw_dtype}' (normalized to '{dtype}'). Skipping row.")
                continue

            # Handle STR<n> conversion inside address logic
            dtype_upper = dtype.upper()
            ...
            if address:
                ...
                # Apply address offset to the start address
                try:
                    start_addr_val = int(norm_parts[0])
                    if self.address_offset != 0:
                        new_start_addr = start_addr_val - self.address_offset
                        if new_start_addr < 0:
                             logging.warning(f"Line {line_num}: Address {norm_parts[0]} with offset {self.address_offset} results in negative address {new_start_addr}")
                        norm_parts[0] = str(new_start_addr)
                except ValueError:
                    pass

                # Handle STR<n> address expansion
                if match_str and len(norm_parts) == 1:
                    length = match_str.group(1)
                    norm_parts.append(length)
                    dtype = 'STRING'
...
def run_generator(input_file, output=None, manufacturer=None, model=None,
    ...
    try:
        try:
            with open(input_file, mode='r', encoding='utf-16') as f_check:
                content = f_check.read(1)
                encoding = 'utf-16' if content == '\ufeff' else 'utf-8-sig'
        except UnicodeError:
            encoding = 'utf-8-sig'
...
def main():
    ...
    run_generator(input_file=args.input_file, output=args.output, manufacturer=args.manufacturer,
                 model=args.model, protocol=args.protocol, category=args.category,
                 forced_write=args.forced_write, template=args.template, address_offset=args.address_offset)

...
class Generator:
    ...
    def normalize_address_val(addr_part: Any) -> str:
        ...
        if addr_part is None: return ""
        s = str(addr_part).strip()
        # Handle thousands separators (1,000 -> 1000)
        s = re.sub(r'(?<=\d),(?=\d{3}(?!\d))', '', s)
        if not s: return ""

        if '_' in s:
            return '_'.join(Generator.normalize_address_val(p) for p in s.split('_'))

        if s.lower().startswith('0x'):
            try: return str(int(s, 16))
            except ValueError: return s
        if s.lower().endswith('h'):
            try: return str(int(s[:-1], 16))
            except ValueError: return s
        if re.match(r'^-?\d+$', s):
            return str(int(s))
        ...
        try:
            ...
            with open(filepath, 'r', encoding=encoding) as f:
                ...
                for line_num, row in enumerate(reader, start=2):
                    if not row or not any(row) or str(row[0]).startswith('#'): continue
                    if len(row) < 11:
                    ...
                    if not self.validate_address(addr, dtype):
                        ...
                        # test_validate.py expects False on overlap.
                        # test_validation.py expects True on overlap.
                        # Memory says warnings by default (strict_overlap=False).
                        # I will prioritize strict_overlap argument.
                        if overlap:
                            valid = False
    ...
    def apply_address_offset(address: Any, offset: int, line_num: Optional[int] = None, name: Optional[str] = None) -> str:
        ...
        parts[0] = Generator.normalize_address_val(parts[0])
        try:
    ...
    def process_rows(self, rows: Iterable[Dict[str, Any]], address_offset: int = 0) -> Iterator[Dict[str, Any]]:
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            # Note: address_offset is applied here if not already applied

...
class Generator:
    ...
    def _parse_numeric(self, val):
        """Robustly parses numeric strings, handling fractions and European decimals."""
    ...
    def apply_address_offset(self, address, offset):
        """Applies an integer offset to an address (handles compound addresses)."""
        if not offset:
            # Still run through normalization for consistency
        ...
        try:
            ...
            # Normalize other parts (StartBit, Length) if they exist
    ...
    def _get_val(self, row, key):
        """Safe getter for row dictionary values."""
    ...
    def _process_name_and_tag(self, row, line_num, seen_names, seen_tags):
        """Normalizes name and generates/validates a unique tag."""
    ...
    def _check_address_overlap(self, info1, start_addr, end_addr, line_num, name, dtype, address_usage):
        """Detects overlapping register addresses within the same register type."""
        ...
        for u_start, u_end, u_line, u_name, u_type in address_usage[info1]:
            if max(start_addr, u_start) <= min(end_addr, u_end):
                # Allow BITS to occupy the same base address
    ...
    def _calculate_coefficients(self, factor, scale_factor_str, offset):
        """Calculates CoefA and CoefB based on input factor, scale, and bias."""
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            if address:
                ...
                if '_' in address:
                    try:
                        base_val = int(address.split('_')[0])
                        if base_val < 0:
                            logging.warning(f"Line {line_num}: Address offset {address_offset} results in negative address {base_val} for '{name}'.")
                    except ValueError:
                        pass
            ...
            try:
                ...
                self._check_address_overlap(info1, start_addr, end_addr, line_num, name, dtype, address_usage)

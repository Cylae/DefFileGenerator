...
class Generator:
    ...
    def _check_address_overlap(self, info1, address, dtype, name, line_num, address_usage, warned_lines):
        """Checks for address overlaps using O(1) dictionary lookup."""
        try:
            ...
            for offset in range(reg_count):
                ...
                if current_addr in address_usage[info1]:
                    for u_line, u_name, u_type in address_usage[info1][current_addr]:
                        # Allow multiple BITS on exactly the same base address
                        if is_bits and u_type == 'BITS' and current_addr == start_addr:
                            continue

                        warning_pair = tuple(sorted((line_num, u_line)))
                        if warning_pair not in warned_lines:
                            logging.warning(f"Line {line_num}: Address overlap detected for '{name}' ({start_addr}). Overlaps with '{u_name}' (Line {u_line}).")
                            warned_lines.add(warning_pair)
                else:
                    address_usage[info1][current_addr] = []

                address_usage[info1][current_addr].append((line_num, name, dtype.upper()))
        except (ValueError, IndexError):
            pass
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            # Normalize row once per iteration
            norm_row = {str(k).lower().strip(): (str(v).strip() if v is not None else '') for k, v in row.items()}
            ...
            if not act_str:
                ...
                norm_action = '4'
            elif act_str in ['RW', 'READ/WRITE', 'W', 'WRITE', '1']:
                norm_action = '1'
    ...
    def write_output_csv(output, processed_rows, manufacturer, model,
        ...
        try:
            ...
            if 'outfile' in locals() and should_close and outfile:
                outfile.close()

...
class Generator:
    ...
    def apply_address_offset(self, address, offset):
        """Applies offset and normalizes address parts to decimal."""
        if not address:
            return ""
    ...
    def _parse_numeric(self, val):
        """Parses numeric values, handling European decimals and fractions."""
        if not val:
            return 0.0
        v = str(val).replace(',', '.')
        if '/' in v:
    ...
    def _check_address_overlap(self, line_num, info1, address, dtype, name, address_usage):
        try:
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            if address:
                ...
                try:
                    if int(address.split('_')[0]) < 0:
                         logging.warning(f"Line {line_num}: Address results in negative value for '{name}'.")
                except (ValueError, IndexError):
                    pass
            ...
            self._check_address_overlap(line_num, info1, address, dtype, name, address_usage)

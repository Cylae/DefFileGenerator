...
class Generator:
    ...
    def validate_address(address: str, dtype: str) -> bool:
        ...
        # Range validation (Modbus standard 0-65535)
        try:
    ...
    def write_output_csv(output: Union[str, Any, None], processed_rows: Iterable[Dict[str, Any]], manufacturer: str, model: str,
        ...
        try:
            ...
            if summary:
                logging.info(f"Generated registers: {summary}")
        ...
        try:
            with open(filepath, mode='r', encoding='utf-8-sig') as f:
                ...
                for line_num, row in enumerate(reader, start=2):
                    ...
                    # row[0]=Index, row[1]=Info1, row[2]=Info2, row[3]=Info3, row[5]=Name, row[6]=Tag
                    info1 = row[1]
                    address = row[2]
                    dtype = row[3]
                    name = row[5]
                    ...
                    if not Generator.validate_address(address, dtype):
                        logging.warning(f"Line {line_num}: Invalid address or type: {address} ({dtype})")
                        is_valid = False

                    # Check Duplicate Tags (Fatal in Webdyn)
                    if tag:
                        if tag in seen_tags:
                            logging.error(f"Line {line_num}: Duplicate Tag '{tag}' (already at line {seen_tags[tag]})")
                            is_valid = False
                        else:
                            seen_tags[tag] = line_num
            ...
            if warned_lines:
                # We treat overlaps as warnings for now, but they can be fatal.
                pass

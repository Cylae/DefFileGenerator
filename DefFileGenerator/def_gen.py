...
class Generator:
    ...
    def _parse_numeric(val, default=0.0):
        """Robustly parses numeric strings, handling fractions and locale-specific formats."""
        if val is None or str(val).strip() == '':
            return default
        s = str(val).strip()

        # Handle fractions like "1/10"
        if '/' in s:
            try:
                parts = s.split('/')
        ...
        # Handle locale-specific formats (thousands vs decimal separators)
        # Heuristic: if it looks like "1.234,56", convert to "1234.56"
        if re.match(r'^-?\d{1,3}(\.\d{3})*,\d+$', s):
            s = s.replace('.', '').replace(',', '.')
        # If it looks like "1,234.56", convert to "1234.56"
        elif re.match(r'^-?\d{1,3}(,\d{3})*\.\d+$', s):
            s = s.replace(',', '')
        # Handle simple comma as decimal separator if no dots present: "1234,56"
    ...
    def apply_address_offset(self, address, offset, line_num=None):
        """Applies an integer offset to a register address."""
        ...
        try:
            ...
            if base_addr < 0:
                ...
                logging.warning(f"{loc}Address offset {offset} results in negative address {base_addr}.")
    ...
    def _process_name_and_tag(self, name, tag, line_num, seen_names, seen_tags):
        """Validates name and auto-generates/validates tag."""
        if name:
    ...
    def _determine_info1(self, reg_type_str, line_num):
        """Maps RegisterType to Webdyn Info1 code (1-4)."""
        info1 = '3'
        if reg_type_str:
    ...
    def _check_address_overlap(self, address, dtype, info1, name, line_num, address_usage):
        """Checks if the register range overlaps with existing ones."""
        try:
            ...
            for u_start, u_end, u_line, u_name, u_type in address_usage[info1]:
                if max(start_addr, u_start) <= min(end_addr, u_end):
                    # Exception: Multiple BITS registers can share the same address
    ...
    @staticmethod
    def _calculate_coefficients(factor, scale_factor_str, offset):
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            if address:
                # Normalize address parts and apply offset
                parts = [self.normalize_address_val(p) for p in address.split('_')]
                address = self.apply_address_offset('_'.join(parts), address_offset, line_num)
            ...
            self._check_address_overlap(address, dtype, info1, name, line_num, address_usage)

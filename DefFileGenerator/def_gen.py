...
class Generator:
    ...
    def _process_name_and_tag(self, name, tag, seen_tags, line_num):
        if not tag and name:
            ...
            tag = base_tag if base_tag else "v_var"
            if tag and tag[0].isdigit():
                tag = "v_" + tag
            counter = 1
            final_tag = tag
            while final_tag in seen_tags:
                final_tag = f"{tag}_{counter}"
                counter += 1
            tag = final_tag
        if tag in seen_tags:
    ...
    def _check_address_overlap(self, address, dtype, info1, address_usage, name, line_num):
        try:
    ...
    def _calculate_coefficients(self, factor, scale_factor_str):
        val_factor = self._parse_numeric(factor, 1.0)
        val_scale = int(self._parse_numeric(scale_factor_str, 0))
        return "{:.6f}".format(val_factor * (10 ** val_scale))
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            self._check_address_overlap(address, dtype, info1, address_usage, name, line_num)
            coef_a = self._calculate_coefficients(factor, scale_factor_str)

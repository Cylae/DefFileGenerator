...
class Generator:
    def apply_address_offset(self, address, offset):
        """Applies an offset to a simple or compound address string."""
        if not address:
            return address
    ...
    def normalize_address_val(self, addr_part):
        ...
        # If it's a raw hex word (e.g. "A0") but could be decimal
        try:
            return str(int(val))
        except ValueError:
            pass
    ...
    def _normalize_type_and_address(self, line_num, name, dtype_raw, address, address_offset):
    ...
    def _process_name_and_tag(self, line_num, name, tag, seen_names, seen_tags):
        """Validates and generates unique tags."""
        if name:
    ...
    def _check_address_overlap(self, line_num, name, address, dtype, info1, address_usage):
        ...
        try:
            ...
            # Update phase
            for addr in range(start_addr, end_addr + 1):
                # Prioritize storing a non-bit type to catch subsequent overlaps more reliably
                if addr not in address_usage[info1] or (address_usage[info1][addr][2] == 'BITS' and not is_bits):
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            dtype, address = self._normalize_type_and_address(line_num, name, dtype_raw, address, address_offset)

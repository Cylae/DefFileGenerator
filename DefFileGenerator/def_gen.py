...
class Generator:
    ...
    @staticmethod
    def validate_address_range(address_val: int, line_num: Optional[int] = None, name: Optional[str] = None) -> None:
        """Logs warning if address is outside standard Modbus range (0-65535)."""
        if address_val < 0 or address_val > 65535:
            msg = f"Address {address_val} is outside standard Modbus range (0-65535)"
    ...
    def apply_address_offset(address: Any, offset: int, line_num: Optional[int] = None, name: Optional[str] = None) -> str:
        ...
        try:
            ...
            Generator.validate_address_range(base_addr, line_num, name)
    ...
    def write_output_csv(output: Union[str, Any, None], processed_rows: Iterable[Dict[str, Any]], manufacturer: str, model: str,
        ...
        try:
            ...
            if isinstance(output, str):
                summary = ", ".join([f"{stats[k]} {type_names[k]}" for k in sorted(stats.keys()) if stats[k] > 0])
                logging.info(f"Definition file generated at {output}. Processed: {summary}")
        ...
        try:
            with open(filepath, 'r', encoding='utf-8') as f:
                ...
                for line_num, row in enumerate(reader, start=2):
                    ...
                    # Webdyn CSV: Index, Info1, Info2, Info3, Info4, Name, Tag, CoefA, CoefB, Map, Action
                    info1, address, dtype, name, tag = row[1], row[2], row[3], row[5], row[6]

                    # Duplicate Tag Check (Fatal)
                    if tag in seen_tags:
                        logging.error(f"Line {line_num}: Duplicate Tag '{tag}' (Fatal). Also on line {seen_tags[tag]}.")
                        success = False
                    seen_tags[tag] = line_num

                    # Address overlap check (Warning)
                    self._check_address_overlap(info1, address, dtype, name, line_num, address_usage, warned_lines)

                    # Address range check
                    try:
                        base_addr = int(address.split('_')[0])
                        self.validate_address_range(base_addr, line_num, name)
                    except (ValueError, IndexError):
                        pass

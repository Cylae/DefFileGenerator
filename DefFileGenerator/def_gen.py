...
class Generator:
    ...
    def _get_val(self, row, key_lower):
        """Helper to get value from row with pre-calculated lower-case key."""
        ...
        try:
            ...
            for addr in range(start_addr, end_addr + 1):
                if addr in address_usage[info1]:
                    ...
                    # BITS special case: multiple bit registers can share the same base address
            ...
            for addr in range(start_addr, end_addr + 1):
                address_usage[info1][addr] = entry

        except (ValueError, IndexError):
            pass

    def _calculate_coefficients(self, factor, offset, scale_factor_str):
        """Calculates CoefA and CoefB with default fallback."""
        try:
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        # Info1 -> address -> (start, end, line, name, type)
        address_usage = {}

...
class Generator:
    ...
    def normalize_type(self, dtype):
        ...
        # Re-check if t itself has a suffix that was preserved by _CLEAN_TYPE_RE
        res = t.upper() if t else 'U16'
        if suffix and not res.endswith(suffix):
            res += suffix
        return res
    ...
    def _normalize_type_and_address(self, row, line_num, address_offset):
        """Normalizes and validates Type and Address from row."""
    ...
    def _process_name_and_tag(self, row, line_num, seen_names, seen_tags):
        """Handles Name/Tag duplicates and automatic tag generation."""
        name = self._get_val(row, 'Name')
    ...
    def _check_address_overlap(self, info1, address, dtype, name, line_num, address_usage):
        """Checks for register address overlaps using optimized dictionary lookup."""
        try:
    ...
    def _calculate_coefficients(self, row):
        """Calculates CoefA and CoefB for Webdyn format."""

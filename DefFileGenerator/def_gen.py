...
RE_TAG_CLEAN = re.compile(r'[^a-z0-9]+')
...
class Generator:
    ...
    def normalize_address_val(self, addr_part):
        ...
        if match:
            ...
            try:
                # Could be decimal or raw hex word if it only contains 0-9A-F
                # But RE_ADDR_VAL is designed to distinguish.
                # If it's just digits, int(val) works.
                # If it's hex without prefix (handled by last part of RE_ADDR_VAL), int(val, 16)
                if all(c in '0123456789' for c in val) or (val.startswith('-') and all(c in '0123456789' for c in val[1:])):
                    return str(int(val))
    ...
    def apply_address_offset(self, address, offset):
        """Applies offset and normalizes base address to decimal."""
    ...
    def _get_val(self, row, key, row_lower=None):
        if row_lower:
            return row_lower.get(key.lower().strip(), '')
        for k, v in row.items():
            if k.lower().strip() == key.lower():
                return str(v).strip() if v is not None else ''
        return ''

    def _normalize_type_and_address(self, row_data, address_offset, line_num):
        dtype = self.normalize_type(row_data['Type'])
        if not self.validate_type(dtype):
            logging.warning(f"Line {line_num}: Invalid Type '{row_data['Type']}' (normalized to '{dtype}'). Skipping row.")
        ...
        if address:
            ...
            try:
                ...
                if base_addr < 0:
                    logging.warning(f"Line {line_num}: Address offset {address_offset} results in negative address {base_addr} for '{row_data['Name']}'.")
            except (ValueError, IndexError):
                pass
        ...
        return dtype, address, row_data['Name']

    def _process_name_and_tag(self, name, tag, seen_names, seen_tags, line_num):
        if name:
    ...
    def _check_address_overlap(self, info1, address, dtype, name, address_usage, line_num):
        try:
            ...
            # Optimized O(1) check using range covering
            # For simplicity in this implementation, we use a dictionary of addresses.
            # But for large ranges it might be slow. The memory says O(1) dictionary-based lookup.
            # This usually means checking every address in the range.
            ...
            for addr in range(start_addr, end_addr + 1):
                if addr not in address_usage[info1]: # Keep first one for overlap message
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        seen_names = {}
        seen_tags = {}
        address_usage = {} # Info1 -> dict of addr -> (line, name, type)

        for line_num, row in enumerate(rows, start=2):
            if not any(v for v in row.values() if v):
                continue

            row_lower = {k.lower().strip(): str(v).strip() if v is not None else '' for k, v in row.items()}

            row_data = {
                'Name': self._get_val(row, 'Name', row_lower),
                'Tag': self._get_val(row, 'Tag', row_lower),
                'RegisterType': self._get_val(row, 'RegisterType', row_lower),
                'Address': self._get_val(row, 'Address', row_lower),
                'Type': self._get_val(row, 'Type', row_lower),
                'Factor': self._get_val(row, 'Factor', row_lower),
                'Offset': self._get_val(row, 'Offset', row_lower),
                'Unit': self._get_val(row, 'Unit', row_lower),
                'Action': self._get_val(row, 'Action', row_lower),
                'ScaleFactor': self._get_val(row, 'ScaleFactor', row_lower)
            }

            if not row_data['Name'] and not row_data['Address']:
                logging.warning(f"Line {line_num}: Skipping row with missing Name and Address.")
                continue

            dtype, address, name = self._normalize_type_and_address(row_data, address_offset, line_num)
            if dtype is None:
                continue

            tag = self._process_name_and_tag(name, row_data['Tag'], seen_names, seen_tags, line_num)
            info1 = self._determine_info1(row_data['RegisterType'], line_num)
            self._check_address_overlap(info1, address, dtype, name, address_usage, line_num)
            coef_a, coef_b = self._calculate_coefficients(row_data['Factor'], row_data['ScaleFactor'], row_data['Offset'])

            # Action normalization
            act_str = row_data['Action'].upper()
            if not act_str:
                norm_action = '1'
            ...
            processed_rows.append({
                ...
                'Unit': row_data['Unit'], 'Action': norm_action
            })

...
class Generator:
    ...
    def apply_address_offset(self, address, offset):
        """Applies offset to base address and returns normalized string."""
        if not address:
            return ""
    ...
    def _parse_numeric(self, val):
        """Robust numeric parsing for factors and offsets."""
    ...
    def _process_name_and_tag(self, name, tag, seen_names, seen_tags, line_num):
        """Validates name and tag, generating tag if missing."""
        if name:
    ...
    def _check_address_overlap(self, address_usage, info1, start_addr, end_addr, line_num, name, dtype):
        """Checks and logs address overlaps using O(1) dictionary lookup."""
        if info1 not in address_usage:
            address_usage[info1] = {}
        is_bits = (dtype.upper() == 'BITS')
        for addr in range(start_addr, end_addr + 1):
            if addr in address_usage[info1]:
                u_name, u_line, u_type = address_usage[info1][addr]
                if not (is_bits and u_type == 'BITS' and start_addr == addr):
                    logging.warning(f"Line {line_num}: Address overlap detected for '{name}' ({start_addr}-{end_addr}). Overlaps with '{u_name}' (Line {u_line}, address {addr}).")
            # Prioritize non-bit types in the map for better overlap detection coverage
            if addr not in address_usage[info1] or not is_bits:
                address_usage[info1][addr] = (name, line_num, dtype.upper())

    def _calculate_coefficients(self, factor, scale_factor_str, offset):
        """Calculates CoefA and CoefB."""
        try:
            val_factor = self._parse_numeric(factor)
            if val_factor is None: val_factor = 1.0
            val_scale = self._parse_numeric(scale_factor_str)
            if val_scale is None: val_scale = 0.0
            coef_a = "{:.6f}".format(val_factor * (10 ** int(val_scale)))
        except (ValueError, TypeError):
            coef_a = "1.000000"

        try:
            val_offset = self._parse_numeric(offset)
            if val_offset is None: val_offset = 0.0
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            if not act_str: norm_action = '1'
            elif act_str in ['R', 'READ', '4']: norm_action = '4'
            elif act_str in ['RW', 'W', 'WRITE', '1']: norm_action = '1'

        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            if not act_str:
                ...
                if info1 in ['2', '4']: # Discrete Input, Input Register
                    norm_action = '4'
                else: # Coil, Holding Register (1, 3) or default
                    norm_action = '1'
    ...
    def write_output_csv(output: Union[str, Any, None], processed_rows: Iterable[Dict[str, Any]], manufacturer: str, model: str,
        ...
        try:
            ...
            for index, row in enumerate(processed_rows, start=1):
                inf1 = row['Info1']
                counts[inf1] = counts.get(inf1, 0) + 1
                writer.writerow([
                    str(index), inf1, row['Info2'], row['Info3'], row['Info4'],
                    row['Name'], row['Tag'], row['CoefA'], row['CoefB'], row['Unit'], row['Action']
                ])

            summary = ", ".join([f"{count} {type_names.get(i, 'Unknown')}" for i, count in sorted(counts.items()) if count > 0])
            if summary:

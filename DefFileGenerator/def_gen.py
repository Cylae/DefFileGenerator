...
RE_ADDR_NORM = re.compile(r'(?<![0-9A-Za-z])(0x[0-9A-Fa-f]+|[0-9A-Fa-f]+h|-?\d+|[0-9A-Fa-f]+)(?![0-9A-Za-z])')
...
class Generator:
    ...
    def normalize_address_val(self, addr_part):
        ...
        match = RE_ADDR_NORM.search(addr_part)
        if not match:
            return addr_part
    ...
    def _get_val(self, row, key):
        ...
        return str(row.get(key.lower(), '')).strip()

    def _normalize_type_and_address(self, row, line_num, address_offset):
        """Normalizes type and address, applying offset."""
    ...
    def _determine_info1(self, row, line_num):
        """Determines Info1 (register type code) from RegisterType."""
    ...
    def _check_address_overlap(self, line_num, address_usage, info1, address, dtype, name):
        """Checks for address overlaps in O(1) fashion per row (O(N) total)."""
        try:
    ...
    def normalize_action(self, action):
        """Normalizes the action/access value."""
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            name_raw = self._get_val(row_norm, 'Name')
            address_raw = self._get_val(row_norm, 'Address')
            if not name_raw and not address_raw:
                logging.warning(f"Line {line_num}: Skipping row with missing Name and Address.")
                continue

            dtype, address = self._normalize_type_and_address(row_norm, line_num, address_offset)
            if dtype is None:
                continue

            name, tag = self._process_name_and_tag(row_norm, line_num, seen_names, seen_tags)
            info1 = self._determine_info1(row_norm, line_num)
            self._check_address_overlap(line_num, address_usage, info1, address, dtype, name)
            coef_a, coef_b = self._calculate_coefficients(row_norm)

            unit = self._get_val(row_norm, 'Unit')
            action = self._get_val(row_norm, 'Action')

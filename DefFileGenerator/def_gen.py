...
class Generator:
    ...
    def apply_address_offset(self, address, offset):
        """Applies an offset to a possibly compound address string."""
        if not address:
            return ""
        parts = str(address).split('_')
        # Always normalize the base address to decimal even if offset is 0
        try:
    ...
    def _get_val(self, row, key):
        """Case-insensitive dictionary lookup with stripping."""
        k_lower = key.lower()
    ...
    def _parse_numeric(self, val):
        """Handles standard floats, fractions (1/10), and European commas."""
        if not val:
            return 0.0
    ...
    def _process_name_and_tag(self, name, tag, seen_names, seen_tags, line_num):
        ...
        if not tag and name:
            # Refined tag generation: collapse multiple underscores and strip ends
    ...
    def _check_address_overlap(self, info1, address, dtype, address_usage, line_num, name):
        try:
            ...
            # Prioritize non-bit types in overlap tracking
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            self._check_address_overlap(info1, address, dtype, address_usage, line_num, name)
            ...
            elif action in ['R', 'READ', '4']:
                norm_action = '4'
            elif action in ['RW', 'W', 'WRITE', '1']:
                norm_action = '1'
            elif action in self.allowed_actions:
                norm_action = action

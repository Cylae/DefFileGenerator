...
class Generator:
    ...
    def _parse_numeric(self, val, default=0.0):
        ...
        if ',' in s and '.' in s:
            ...
            if re.match(r'^[1-9]\d{0,2}(,\d{3})+$', s): s = s.replace(',', '')
            else: s = s.replace(',', '.')
        try:
            return float(s)
        except ValueError:
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            if address:
                ...
                try:
                    ...
                    if base_addr < 0: logging.warning(f"Line {line_num}: Address offset {address_offset} results in negative address {base_addr} for '{name}'.")

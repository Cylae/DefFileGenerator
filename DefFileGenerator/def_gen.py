...
class Generator:
    ...
    def _get_val(self, row, key):
        """Helper to get value from row dict with case-insensitive key search."""
    ...
    def _parse_numeric(self, val, default=0.0):
        """Handles floats, fractions (1/10), and European decimal format (0,1)."""
        ...
        if '/' in s:
            try:
                ...
                logging.debug(f"Could not parse fractional value '{val}', using default {default}")
                return default
        try:
            return float(s)
        except ValueError:
            logging.debug(f"Could not parse numeric value '{val}', using default {default}")
            return default

    def apply_address_offset(self, address, offset):
        """Applies offset to base address, supporting compound formats (Addr_Len, Addr_Bit_Len)."""
        if not address:
            return ""
    ...
    def _process_name_and_tag(self, name, tag, line_num, seen_names, seen_tags):
        """Validates Name and generates/validates unique Tag."""
        if name:
        ...
        if tag:
            ...
            base_tag = tag
            counter = 1
            while tag in seen_tags and seen_tags[tag] != line_num:
                tag = f"{base_tag}_{counter}"
                counter += 1

            if tag not in seen_tags:
                seen_tags[tag] = line_num
            elif seen_tags[tag] != line_num:
    ...
    def _check_address_overlap(self, name, address, dtype, info1, line_num, address_usage):
        """Detects overlapping registers, allowing multiple BITS at same address."""
        try:
            ...
            for u_start, u_end, u_line, u_name, u_type in address_usage[info1]:
                if max(start_addr, u_start) <= min(end_addr, u_end):
                    # Allow multiple BITS on same base address
    ...
    def _calculate_coefficients(self, factor_str, scale_factor_str, offset_str):
        """Calculates CoefA and CoefB from Factor, ScaleFactor and Offset."""
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            coef_a, coef_b = self._calculate_coefficients(factor_str, scale_factor_str, offset_str)

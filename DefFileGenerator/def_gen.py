...
class Generator:
    ...
    def _check_address_overlap(self, info1, address, dtype, name, line_num, address_usage, warned_lines):
        """Checks for address overlaps using a dictionary-based lookup (O(N) avg)."""
        try:
                ...
                if addr in address_usage[info1]:
                    for u_line, u_name, u_type in address_usage[info1][addr]:
                        ...
                        if not (is_bits and u_type == 'BITS' and start_addr == addr):
                            # Prevent duplicate warnings for the same overlapping register pair
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            # Normalize keys once per row for efficiency
            norm_row = {str(k).lower().strip(): v for k, v in row.items()}

            def get_norm_val(key):
                val = norm_row.get(key.lower())
                return str(val).strip() if val is not None else ''

            name = get_norm_val('Name')
            tag = get_norm_val('Tag')
            reg_type_str = get_norm_val('RegisterType')
            address = get_norm_val('Address')
            dtype_raw = get_norm_val('Type')
            factor = get_norm_val('Factor')
            offset = get_norm_val('Offset')
            unit = get_norm_val('Unit')
            action = get_norm_val('Action')
            scale_factor_str = get_norm_val('ScaleFactor')

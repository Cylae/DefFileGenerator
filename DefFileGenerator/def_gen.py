...
class Generator:
    ...
    def _normalize_type_and_address(self, dtype_raw, address, address_offset, name, line_num):
        """Standardizes type and applies address offset."""
    ...
    def _process_name_and_tag(self, name, tag, seen_names, seen_tags, line_num):
        """Handles name/tag generation and duplicate detection."""
        if name:
    ...
    def _check_address_overlap(self, info1, address, dtype, name, line_num, address_usage):
        """Checks for address overlaps using dictionary for O(1) lookup."""
        try:
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        address_usage = {} # Info1 -> dict(addr -> (line, name, type, start, end))
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            dtype, address = self._normalize_type_and_address(dtype_raw, address_raw, address_offset, name_raw, line_num)
            if dtype is None:
                continue

            name, tag = self._process_name_and_tag(name_raw, tag_raw, seen_names, seen_tags, line_num)

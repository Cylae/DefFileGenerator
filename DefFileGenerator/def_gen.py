...
class Generator:
    ...
    def normalize_address_val(self, addr_part):
        ...
        try:
            # Try parsing as hex if it looks like one and isn't purely decimal
            if re.match(r'^[0-9A-Fa-f]+$', val) and not re.match(r'^-?\d+$', val):
                return str(int(val, 16))
            return str(int(val))
        except ValueError:
            return addr_part
    ...
    def apply_address_offset(self, address, offset):
        """Applies an offset to the base address, handling compound addresses."""
        if not address:
            return ""
    ...
    def _normalize_type_and_address(self, row, line_num, address_offset):
        """Helper to normalize and validate type and address."""
        ...
        if address:
            ...
            try:
                ...
                if base_addr < 0:
                    logging.warning(f"Line {line_num}: Address offset results in negative address {base_addr} for '{self._get_val(row, 'Name')}'.")
            except (ValueError, IndexError):
                pass
    ...
    def _determine_info1(self, row, line_num):
        """Helper to determine Info1 (register type code)."""
    ...
    def _check_address_overlap(self, name, address, dtype, info1, line_num, address_usage):
        """Optimized O(1) address overlap check using a dictionary."""
        try:
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            name_val = self._get_val(row, 'Name')
            addr_val = self._get_val(row, 'Address')

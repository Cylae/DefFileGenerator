...
class Generator:
            ...
            for addr in range(start_addr, end_addr + 1):
                if addr in address_usage[info1]:
                    for u_line, u_name, u_type in address_usage[info1][addr]:
                        ...
                        if is_bits and u_type == 'BITS' and addr == start_addr:
                            continue
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            nr = {k.lower().strip(): (str(v).strip() if v is not None else '') for k, v in row.items()}

            name = nr.get('name', '')
            tag = nr.get('tag', '')
            reg_type_str = nr.get('registertype', '')
            address = nr.get('address', '')
            dtype_raw = nr.get('type', '')
            factor = nr.get('factor', '')
            offset = nr.get('offset', '')
            unit = nr.get('unit', '')
            action = nr.get('action', '')
            scale_factor_str = nr.get('scalefactor', '')

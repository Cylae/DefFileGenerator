...
class Generator:
    ...
    def apply_address_offset(self, address, offset):
        """Applies offset to a normalized address (simple or compound)."""
        if not offset:
            # Still normalize the base address to decimal if it's not already
            if '_' in address:
                parts = address.split('_')
                return '_'.join(self.normalize_address_val(p) for p in parts)
            return self.normalize_address_val(address)
    ...
    def _parse_numeric(self, val):
        """Handles European formats (comma), fractions, and standard floats."""
    ...
    def _normalize_type_and_address(self, row, line_num, address_offset):
        ...
        if match_str:
            ...
            dtype = 'STRING'
            if '_' not in address: address = f"{address}_{length}"
    ...
    def _process_name_and_tag(self, row, line_num, seen_names, seen_tags):
        ...
        if not tag and name:
            base_tag = re.sub(r'_{2,}', '_', re.sub(r'[^a-z0-9_]', '_', name.lower())).strip('_')
    ...
    def _determine_info1(self, row, line_num):
        ...
        if reg_type_str:
            ...
            if lt in self.register_type_map: info1 = self.register_type_map[lt]
            elif reg_type_str in ['1', '2', '3', '4']: info1 = reg_type_str
            else: logging.warning(f"Line {line_num}: Unknown RegisterType '{reg_type_str}'. Defaulting to 3.")
        return info1
    ...
    def _check_address_overlap(self, info1, address, dtype, name, line_num, address_usage):
        try:
            ...
            for addr in range(start_addr, end_addr + 1):
                ...
                if not is_bits or addr not in address_usage[info1]:
    ...
    def _calculate_coefficients(self, row):
        ...
        coef_b = "{:.6f}".format(self._parse_numeric(offset))
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            if not name and not address_raw:
                logging.warning(f"Line {line_num}: Skipping row with missing Name and Address."); continue

...
class Generator:
    ...
    def normalize_type(self, dtype):
        """Standardizes data type names."""
        if not dtype:
            return 'U16'
        t = str(dtype).lower().strip()
        t = t.replace('unsigned', 'u').replace('signed', 'i').replace(' ', '')
        # Synonyms
        if 'uint16' in t: return 'U16'
        if 'int16' in t: return 'I16'
        if 'uint32' in t: return 'U32'
        if 'int32' in t: return 'I32'
        if 'uint64' in t: return 'U64'
        if 'int64' in t: return 'I64'
        if 'float32' in t or t == 'float': return 'F32'
        if 'float64' in t or t == 'double': return 'F64'

        # Regex-based cleaning for common patterns
    ...
    def normalize_address_val(self, addr_part):
        ...
        val_str = addr_part[1:] if is_negative else addr_part

        res = val_str
        if val_str.lower().startswith('0x'):
            try:
                res = str(int(val_str, 16))
            except ValueError:
                pass
        elif val_str.lower().endswith('h'):
            try:
                res = str(int(val_str[:-1], 16))
            except ValueError:
                pass
        elif any(c in val_str.upper() for c in 'ABCDEF'):
            # Use word boundaries to check if it's a hex word
            if re.match(r'^[0-9A-F]+$', val_str.upper()):
                try:
                    res = str(int(val_str, 16))
                except ValueError:
                    pass

        return f"-{res}" if is_negative and res else res
    ...
    def process_rows(self, rows):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            if address:
                ...
                # Apply address_offset to the first part (the base address)
                if norm_parts and self.address_offset != 0:
                    try:

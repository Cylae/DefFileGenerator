...
class Generator:
    ...
    def normalize_address_val(self, addr_part):
        ...
        # Use fullmatch for strict address word validation if possible,
        # but RE_ADDR_VAL has lookarounds, so we check if it matches the whole string.
    ...
    def _get_val(self, row, key, precomputed_keys=None):
        """Helper to get value from row dict using precomputed lowercase keys."""
        if precomputed_keys:
            lookup_key = key.lower().strip()
            if lookup_key in precomputed_keys:
                v = row.get(precomputed_keys[lookup_key])
    ...
    def _normalize_type_and_address(self, dtype_raw, address, name, line_num, address_offset):
    ...
    def _check_address_overlap(self, info1, address, dtype, name, line_num, address_usage):
        try:
            ...
            if info1 not in address_usage:
                address_usage[info1] = {} # Tracks every single register index for O(1) overlap check

            is_bits = (dtype.upper() == 'BITS')

            for i in range(start_addr, end_addr + 1):
                if i in address_usage[info1]:
                    u_line, u_name, u_type = address_usage[info1][i]
                    if not (is_bits and u_type == 'BITS' and i == start_addr):
                        logging.warning(f"Line {line_num}: Address overlap detected for '{name}' at {i}. Overlaps with '{u_name}' (Line {u_line}).")
                        break # Only warn once per register range

            for i in range(start_addr, end_addr + 1):
                if i not in address_usage[info1]:
                    address_usage[info1][i] = (line_num, name, dtype.upper())
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        seen_names = {}
        seen_tags = {}
        address_usage = {} # Info1 -> {addr_index -> (line, name, type)}
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            address = self._get_val(row, 'Address', precomputed_keys)
            ...
            dtype, address, ok = self._normalize_type_and_address(dtype_raw, address, name, line_num, address_offset)
            if not ok:
                continue
    ...
    def write_output_csv(output, processed_rows, manufacturer, model,
        ...
        try:
            ...
            # Ensure forced_write is correctly included in the header
            header_row = [protocol, category, manufacturer, model, str(forced_write), '', '', '', '', '', '']

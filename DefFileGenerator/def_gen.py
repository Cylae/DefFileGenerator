...
class Generator:
    ...
    def apply_address_offset(self, address, offset):
        """Applies an offset to an address (simple or compound)."""
        if not offset:
            # Still normalize to decimal if it was hex
    ...
    def _parse_numeric(self, val):
        """Handles standard floats, fractions, and European decimal formats."""
    ...
    def _get_val(self, row, key):
        ...
        for k, v in row.items():
            if k.lower().strip() == k_lower:
                return str(v).strip() if v is not None else ''
        return ''

    def _normalize_type_and_address(self, dtype_raw, address_raw, address_offset, line_num, name):
    ...
    def _process_name_and_tag(self, name, tag, seen_names, seen_tags, line_num):
        """Validates name and generates unique tag if necessary."""
        if name:
    ...
    def _determine_info1(self, reg_type_str, line_num):
        """Maps RegisterType to Webdyn SunPM Info1."""
        if not reg_type_str:
            return '3'
    ...
    def _check_address_overlap(self, info1, address, dtype, line_num, name, address_usage):
        """Performs O(1) address overlap detection using a dictionary map."""
        try:
            ...
            for i in range(reg_count):
                ...
                if curr_addr in address_usage[info1]:
                    u_line, u_name, u_type = address_usage[info1][curr_addr]
                    if not (is_bits and u_type == 'BITS' and i == 0):
                        logging.warning(f"Line {line_num}: Address overlap detected at {curr_addr} for '{name}'. Overlaps with '{u_name}' (Line {u_line}).")
                        overlap_found = True

                # Prioritize non-bit types in the map
                if curr_addr not in address_usage[info1] or not is_bits:
    ...
    def _calculate_coefficients(self, factor, scale_factor_str, offset):
        """Calculates Webdyn SunPM CoefA and CoefB."""
        val_factor = self._parse_numeric(factor) if factor else 1.0
        ...
        val_offset = self._parse_numeric(offset) if offset else 0.0
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            name_raw = self._get_val(row, 'Name')
            ...
            dtype, address = self._normalize_type_and_address(
                self._get_val(row, 'Type'), address_raw, address_offset, line_num, name_raw
            )
            if not dtype or not address:
                continue

            name, tag = self._process_name_and_tag(
                name_raw, self._get_val(row, 'Tag'), seen_names, seen_tags, line_num
            )

            info1 = self._determine_info1(self._get_val(row, 'RegisterType'), line_num)
            self._check_address_overlap(info1, address, dtype, line_num, name, address_usage)

            coef_a, coef_b = self._calculate_coefficients(
                self._get_val(row, 'Factor'),
                self._get_val(row, 'ScaleFactor'),
                self._get_val(row, 'Offset')
            )

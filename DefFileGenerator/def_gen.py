...
class Generator:
    ...
    def apply_address_offset(self, address, offset):
        """Applies an offset to a register address (supports compound addresses)."""
        if not address:
            return address
    ...
    def _normalize_type_and_address(self, dtype_raw, address_raw, line_num, name, address_offset):
        """Normalizes data type and address, applying offset and handling STR<n>."""
        ...
        return dtype.upper(), address

    def _process_name_and_tag(self, name, tag, line_num, seen_names, seen_tags):
        """Processes name and tag, ensuring uniqueness and generating tags if missing."""
        if name:
        ...
        if not tag and name:
            # Re-use memory recommendation for tag cleaning
    ...
    def _determine_info1(self, reg_type_str, line_num):
        """Determines Info1 (Modbus function/type) from string."""
        info1 = '3'
        if reg_type_str:
    ...
    def _check_address_overlap(self, info1, address, dtype, name, line_num, address_usage):
        """Optimized O(1) dictionary-based check for address overlaps."""
        try:
            ...
            for a in range(start_addr, end_addr + 1):
                if a in address_usage[info1]:
                    u_line, u_name, u_type = address_usage[info1][a]
                    # Exception: Multiple BITS variables on the same address are allowed
                    if not (is_bits and u_type == 'BITS' and start_addr == a):
                         logging.warning(f"Line {line_num}: Address overlap detected for '{name}' (at address {a}). Overlaps with '{u_name}' (Line {u_line}).")
                         break # Only warn once per register

            # Record usage
            for a in range(start_addr, end_addr + 1):
                # Don't overwrite if it's already there (though we warned)
                if a not in address_usage[info1]:
                    address_usage[info1][a] = (line_num, name, dtype.upper())
                elif is_bits and u_type == 'BITS' and start_addr == a:
                    # For bits, we might want to track all of them?
                    # For overlap detection, just knowing one is there is enough unless we check bit positions.
                    pass
    ...
    def _calculate_coefficients(self, factor, scale_factor_str, offset):
        """Calculates CoefA and CoefB for linear transformation."""
        try:
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            # Precompute keys for faster lookup in _get_val
            precomputed_keys = {k.lower().strip(): k for k in row.keys()}
            ...
            dtype, address = self._normalize_type_and_address(dtype_raw, address_raw, line_num, name, address_offset)
            if not dtype:
                logging.warning(f"Line {line_num}: Invalid or missing Type/Address. Skipping row.")
                continue

            tag = self._get_val(row, 'Tag', precomputed_keys)
            tag = self._process_name_and_tag(name, tag, line_num, seen_names, seen_tags)

            reg_type_str = self._get_val(row, 'RegisterType', precomputed_keys)
            ...
            factor = self._get_val(row, 'Factor', precomputed_keys)
            scale_factor_str = self._get_val(row, 'ScaleFactor', precomputed_keys)
            offset = self._get_val(row, 'Offset', precomputed_keys)

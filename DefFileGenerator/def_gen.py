...
class Generator:
    ...
    def _get_val(self, row, key):
        """Case-insensitive row dictionary lookup."""
        for k, v in row.items():
            if k.lower().strip() == key.lower():
                return str(v).strip() if v is not None else ''
        return ''

    def _normalize_type_and_address(self, dtype_raw, address_raw, name, line_num, address_offset):
        """Normalizes data type, converts STR<n>, and applies address offset."""
    ...
    def _process_name_and_tag(self, name, tag, line_num, seen_names, seen_tags):
        """Handles duplicate Name/Tag detection and auto-generates missing Tags."""
        if name:
        ...
        final_tag = tag
        if not final_tag and name:
            base_tag = re.sub(r'[^a-z0-9_]', '', name.lower().replace(' ', '_'))
            final_tag = base_tag if base_tag else "var"
            counter = 1
            while final_tag in seen_tags:
                final_tag = f"{base_tag}_{counter}"
                counter += 1

        if final_tag:
            if final_tag in seen_tags:
                logging.warning(f"Line {line_num}: Duplicate Tag '{final_tag}' detected. Previous occurrence at line {seen_tags[final_tag]}.")
            else:
                seen_tags[final_tag] = line_num
        return final_tag

    def _determine_info1(self, reg_type_str, line_num):
        """Maps RegisterType string to WebdynSunPM Info1 code."""
        if not reg_type_str:
            return '3'
    ...
    def _check_address_overlap(self, name, address, dtype, info1, line_num, address_usage):
        """Performs O(1) optimized address collision detection."""
        try:
    ...
    def _calculate_coefficients(self, factor, offset, scale_factor_str):
        """Calculates CoefA and CoefB based on factor, offset, and scale factor."""
        try:
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            dtype, address = self._normalize_type_and_address(dtype_raw, address_raw, name, line_num, address_offset)
            if dtype is None:
                continue

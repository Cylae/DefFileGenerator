...
class Generator:
            ...
            for offset in range(reg_count):
                ...
                if addr in address_usage[info1]:
                    for u_line, u_name, u_type in address_usage[info1][addr]:
                        # Allow multiple BITS on exactly the same base address (base address only)
                        if not (is_bits and u_type == 'BITS' and offset == 0):
                            warn_key = tuple(sorted((line_num, u_line)))
                            if warn_key not in warned_lines:
                                logging.warning(f"Line {line_num}: Address overlap detected for '{name}' ({start_addr}-{start_addr+reg_count-1}). Overlaps with '{u_name}' (Line {u_line}).")
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, raw_row in enumerate(rows, start=2):
            # Normalize row keys once per iteration for efficiency
            row = {k.lower().strip(): (str(v).strip() if v is not None else '') for k, v in raw_row.items()}
            if not any(row.values()):
                continue

            name = row.get('name', '')
            tag = row.get('tag', '')
            reg_type_str = row.get('registertype', '')
            address = row.get('address', '')
            dtype_raw = row.get('type', '')
            factor = row.get('factor', '')
            offset = row.get('offset', '')
            unit = row.get('unit', '')
            action = row.get('action', '')
            scale_factor_str = row.get('scalefactor', '')

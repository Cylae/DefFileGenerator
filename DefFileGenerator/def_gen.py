...
class Generator:
    ...
    def apply_address_offset(self, address, offset):
        """Applies address offset to both simple and compound addresses."""
        if not address:
            return ""
    ...
    def _get_val(self, row_data, key):
        """Helper to get value from row with case-insensitive key."""
        return row_data.get(key.lower(), '')
    ...
    def _process_name_and_tag(self, name, tag, line_num, seen_names, seen_tags):
        """Handles duplicate detection and tag generation."""
        if name:
        ...
        if not tag and name:
            ...
            base_tag = re.sub(r'_+', '_', base_tag)
    ...
    def _determine_info1(self, reg_type_str, line_num):
        """Determines Info1 (Register Type) code."""
        info1 = '3'
        if reg_type_str:
    ...
    def _check_address_overlap(self, info1, address, dtype, name, line_num, address_usage):
        """Checks for address overlaps using O(1) dictionary-based lookup."""
        try:
            ...
            for addr in range(start_addr, start_addr + reg_count):
                if addr in address_usage[info1]:
                    u_line, u_name, u_type = address_usage[info1][addr]
                    # BITS at the same address is allowed
                    ...
                    # If existing was BITS and new is not, new takes priority for future checks
                    if u_type == 'BITS' and not is_bits:
                        address_usage[info1][addr] = (line_num, name, dtype_upper)
                else:
                    address_usage[info1][addr] = (line_num, name, dtype_upper)
        except (ValueError, IndexError):
            pass
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            # Pre-calculate lowercase/stripped keys for lookup optimization
            row_data = {str(k).lower().strip(): v for k, v in row.items()}

            name = self._get_val(row_data, 'Name')
            address_raw = self._get_val(row_data, 'Address')
            tag_raw = self._get_val(row_data, 'Tag')
            reg_type_str = self._get_val(row_data, 'RegisterType')
            dtype_raw = self._get_val(row_data, 'Type')
            factor = self._get_val(row_data, 'Factor')
            offset = self._get_val(row_data, 'Offset')
            unit = self._get_val(row_data, 'Unit')
            action = self._get_val(row_data, 'Action')
            scale_factor_str = self._get_val(row_data, 'ScaleFactor')

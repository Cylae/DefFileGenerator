...
class Generator:
    ...
    def apply_address_offset(self, address, offset):
        """Applies an offset to a register address (simple or compound)."""
        if not address or offset == 0:
            # Still normalize to decimal if possible
    ...
    def _get_val(self, row, key):
        """Helper to get a case-insensitive value from a row dict."""
    ...
    def _normalize_type_and_address(self, row, line_num, address_offset):
        """Handles type and address normalization including STR<n> conversion."""
    ...
    def _process_name_and_tag(self, row, line_num, seen_names, seen_tags):
        """Processes name and generates/validates a unique tag."""
        name = self._get_val(row, 'Name')
        ...
        if not tag and name:
            ...
            base_tag = re.sub(r'_{2,}', '_', base_tag)
    ...
    def _determine_info1(self, row, line_num):
        """Determines the Webdyn Info1 (Register Type) value."""

...
# Robust regex for address value extraction and tag sanitization
...
class Generator:
    ...
    def _normalize_type_and_address(self, row, line_num, address_offset):
        """Normalizes and validates type and address."""
    ...
    def _process_name_and_tag(self, row, line_num, name, seen_names, seen_tags):
        """Handles name and tag uniqueness and generation."""
        if name:
    ...
    def _determine_info1(self, row, line_num):
        """Determines register type (Info1)."""
    ...
    def _check_address_overlap(self, line_num, name, address, dtype, info1, address_usage):
        """Checks for address overlaps using optimized dictionary lookup."""
        try:
            ...
            # O(N) overlap check within the same Info1
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            tag = self._process_name_and_tag(row, line_num, name, seen_names, seen_tags)

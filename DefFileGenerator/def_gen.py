...
class Generator:
    ...
    def apply_address_offset(self, address, offset):
        """Applies decimal offset to simple or compound addresses."""
        if not address:
            return ""
    ...
    def _parse_numeric(self, val, default=0.0):
        """Parses numeric values, handling fractions and European formats."""
    ...
    def _process_name_and_tag(self, name, tag, seen_names, seen_tags, line_num):
        """Validates name uniqueness and generates/validates tags."""
        if name:
        ...
        if not tag and name:
            # Generate tag: clean, lowercase, collapse underscores
            clean_name = RE_TAG_CLEAN.sub('_', name.lower()).strip('_')
            # Prepend 'v_' if it starts with a number or is empty
            tag = clean_name if clean_name and not clean_name[0].isdigit() else f"v_{clean_name}"
            # Ensure uniqueness
            base_tag = tag
            counter = 1
    ...
    def _determine_info1(self, reg_type_str, line_num):
        """Determines Info1 (Register Type ID) from string."""
        if not reg_type_str:
            return '3'
    ...
    def _check_address_overlap(self, name, address, dtype, info1, address_usage, line_num):
        """Checks for address overlaps and updates the usage map."""
        try:
            ...
            for u_start, u_end, u_line, u_name, u_type in address_usage[info1]:
                if max(start_addr, u_start) <= min(end_addr, u_end):
                    # Allow multiple BITS registers on the same address
    ...
    def _calculate_coefficients(self, factor, scale_factor_str, offset):
        ...
        val_scale = int(self._parse_numeric(scale_factor_str, 0))

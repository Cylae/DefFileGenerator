...
class Generator:
    ...
    def apply_address_offset(self, address, offset):
        """Applies an integer offset to a simple or compound address string."""
    ...
    def _normalize_type_and_address(self, row, line_num, address_offset):
        ...
        # Verify if offset resulted in negative address
        try:
    ...
    def _check_address_overlap(self, info1, address, dtype, name, line_num, address_usage):
        try:
            ...
            # Optimized O(1) check using a dictionary map of addresses
            ...
            entry = (line_num, name, dtype.upper(), start_addr, end_addr)
            for addr in range(start_addr, end_addr + 1):
                # If current is not BITS, it overwrites BITS for better overlap detection of wider types
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            if not dtype or not address:
                if not self._get_val(row, 'Name') and not self._get_val(row, 'Address'):

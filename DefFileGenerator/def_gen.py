...
class Generator:
    ...
    def _process_name_and_tag(self, name, tag, line_num, seen_names, seen_tags):
        ...
        if not tag:
            ...
            base_tag = re.sub(r'[^a-z0-9]', '_', name.lower() if name else "")
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            # Normalize keys for efficient lookup
            ...
            name = get_v('Name')
            tag = get_v('Tag')
            reg_type_str = get_v('RegisterType')
            address = get_v('Address')
            dtype_raw = get_v('Type')
            factor = get_v('Factor')
            offset = get_v('Offset')
            unit = get_v('Unit')
            action = get_v('Action')
            scale_factor_str = get_v('ScaleFactor')
            ...
            if not act_str:
                norm_action = '1'
            elif act_str in ['R', 'READ', 'READ-ONLY', 'RO', '4']:
                norm_action = '4'
            elif act_str in ['RW', 'READ/WRITE', 'W', 'WRITE', 'WRITE-ONLY', 'WO', '1']:
                norm_action = '1'

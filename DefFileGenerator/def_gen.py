...
RE_THOUSANDS = re.compile(r'^[1-9]\d{0,2}(,\d{3})+$')
...
class Generator:
    ...
    def _parse_numeric(val, default=0.0):
        ...
        # Locale-aware heuristic for thousands separators vs decimal
        if RE_THOUSANDS.match(s):
            s = s.replace(',', '')
        elif ',' in s and '.' not in s:
            # Check if comma might be a decimal separator (e.g. 0,001)
            # or if it's a thousands separator in a short number.
            # Heuristic: if it looks like a decimal separator in some locales
            if re.match(r'^-?\d+,\d+$', s):
                s = s.replace(',', '.')
            else:
                s = s.replace(',', '')
    ...
    def _calculate_coefficients(factor, scale_factor_str, offset):
        val_factor = Generator._parse_numeric(factor, default=1.0)
        val_scale = int(Generator._parse_numeric(scale_factor_str, default=0.0))
        coef_a = "{:.6f}".format(val_factor * (10 ** val_scale))

        val_offset = Generator._parse_numeric(offset, default=0.0)
    ...
    def apply_address_offset(self, address, offset, line_num=None, name=None):
        ...
        if offset != 0:
            ...
            # Ensure normalization even if offset is 0
            try:
                norm_parts[0] = str(int(norm_parts[0]))
            except (ValueError, IndexError):
                pass
    ...
    def _process_name_and_tag(self, name, tag, line_num, seen_names, seen_tags):
        ...
        if not tag and name:
            ...
            if not base_tag or not base_tag[0].isalpha():
                base_tag = "v_" + base_tag if base_tag else "v_var"

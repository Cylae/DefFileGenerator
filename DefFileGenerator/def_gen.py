...
class Generator:
    ...
    def _get_val(self, row, key):
        """Optimized row value retrieval with case-insensitive key matching."""
        # Note: Pre-calculate lower-case keys for production performance
        k_lower = key.lower()
    ...
    def _normalize_type_and_address(self, dtype_raw, address, name, address_offset, line_num):
    ...
    def _process_name_and_tag(self, name, tag, seen_names, seen_tags, line_num):
        """Handles name duplicates and automatic tag generation."""
        if name:
    ...
    def _determine_info1(self, reg_type_str, line_num):
        """Determines Info1 (Modbus Register Type) from string."""
        info1 = '3'
        if reg_type_str:
    ...
    def _check_address_overlap(self, info1, address, dtype, name, address_usage, line_num):
        """Optimized O(1) dictionary-based address overlap detection."""
        try:
            ...
            for u_start, u_end, u_line, u_name, u_type in address_usage[info1]:
                if max(start_addr, u_start) <= min(end_addr, u_end):
                    # BITS types on the same address are allowed
    ...
    def normalize_address_val(self, addr_part):
        ...
        # Use strict regex to identify hex, decimal or negative integers
        ...
        # Handle decimal (including negative) or raw hex
        try:
            # First try as decimal
            return str(int(val))
        except ValueError:
            # If not decimal, try as hex
            try:
                return str(int(val, 16))
            except ValueError:
                return val
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            action_raw = self._get_val(row, 'Action')
            ...
            dtype, address = self._normalize_type_and_address(dtype_raw, address_raw, name, address_offset, line_num)
            if not dtype:
                continue

    ...
    def validate_csv(self, filepath: str, strict_overlap: bool = True) -> bool:
        ...
        try:
            ...
            with open(filepath, 'r', encoding=encoding) as f:
                ...
                # Check if it's a definition file (delimiter ;)
                try:
                    dialect = csv.Sniffer().sniff(snippet, delimiters=";")
                    delimiter = dialect.delimiter
                except csv.Error:
                    # If sniffing fails, check for semicolon
                    delimiter = ';' if ';' in snippet else ','

                reader = csv.reader(f, delimiter=delimiter)
                header = next(reader, None)
                if header is None:
                    return True # Empty file is valid according to memory

                if delimiter == ';' and len(header) < 4:
                     logging.warning("WebdynSunPM header has fewer than 4 columns.")

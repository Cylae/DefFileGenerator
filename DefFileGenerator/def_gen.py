...
class Generator:
    ...
    def _parse_numeric(val, default=0.0):
        ...
        if re.match(r'^-?\d{1,3}(,\d{3})+(\.\d+)?$', s):
            s = s.replace(',', '')
        # European format (1234,56) or (1.234,56)
    ...
    def _process_name_and_tag(self, name, tag, line_num, seen_names, seen_tags):
        ...
        if not tag and name:
            # Clean tag: remove non-alphanumeric, collapse multiples, strip underscores
            base_tag = re.sub(r'[^a-z0-9_]', '_', name.lower())
        ...
        try:
            ...
            for u_start, u_end, u_line, u_name, u_type in address_usage[info1]:
                if max(start_addr, u_start) <= min(end_addr, u_end):
                    # BITS types can share the same base address

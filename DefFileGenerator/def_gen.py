...
class Generator:
    ...
    def apply_address_offset(self, address, offset):
        """Applies an integer offset to a (potentially compound) address string."""
        if not address: return address
        parts = address.split('_')
        try:
            base_addr = int(self.normalize_address_val(parts[0])) + offset
            norm_parts = [str(base_addr)]
            if len(parts) > 1:
                norm_parts.extend(parts[1:])
            return '_'.join(norm_parts)
        except (ValueError, IndexError):
            return address

    def _get_val(self, row, key, row_keys_lower):
        """Helper to get value from row with case-insensitive key."""
        k_lower = key.lower().strip()
        if k_lower in row_keys_lower:
            v = row[row_keys_lower[k_lower]]
            return str(v).strip() if v is not None else ''
        return ''

    def _normalize_type_and_address(self, row, row_keys_lower, line_num, address_offset):
        """Helper to normalize type and address for a row."""
        dtype_raw = self._get_val(row, 'Type', row_keys_lower)
        address_raw = self._get_val(row, 'Address', row_keys_lower)
        name = self._get_val(row, 'Name', row_keys_lower)
    ...
    def _process_name_and_tag(self, row, row_keys_lower, line_num, seen_names, seen_tags):
        """Helper to process Name and Tag, ensuring uniqueness."""
        name = self._get_val(row, 'Name', row_keys_lower)
        tag = self._get_val(row, 'Tag', row_keys_lower)
    ...
    def _determine_info1(self, row, row_keys_lower, line_num):
        """Helper to determine Info1 (RegisterType)."""
        reg_type_str = self._get_val(row, 'RegisterType', row_keys_lower)
        info1 = '3'
        if reg_type_str:
    ...
    def _check_address_overlap(self, info1, address, dtype, name, line_num, address_usage):
        """Optimized O(1) address overlap detection."""
        try:
            ...
            current_map = address_usage[info1]

            for addr in range(start_addr, end_addr + 1):
                if addr in current_map:
                    u_start, u_end, u_line, u_name, u_type = current_map[addr]
                    # Allowed: BITS on same address if start matches
            ...
            # Update usage map for all involved addresses
            entry = (start_addr, end_addr, line_num, name, dtype.upper())
            for addr in range(start_addr, end_addr + 1):
                # Prioritize non-BITS for overlap detection in the map
                if addr not in current_map or not is_bits:
                    current_map[addr] = entry

        except (ValueError, IndexError):
            pass

    def _calculate_coefficients(self, row, row_keys_lower):
        """Helper to calculate CoefA and CoefB."""
        factor = self._get_val(row, 'Factor', row_keys_lower)
        offset = self._get_val(row, 'Offset', row_keys_lower)
        scale_factor_str = self._get_val(row, 'ScaleFactor', row_keys_lower)
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        seen_names = {}
        seen_tags = {}
        address_usage = {} # Info1 -> {address: (start, end, line, name, type)}
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            name = self._get_val(row, 'Name', row_keys_lower)
            address_val = self._get_val(row, 'Address', row_keys_lower)
            if not name and not address_val:
                logging.warning(f"Line {line_num}: Skipping row with missing Name and Address.")
                continue

            dtype, address = self._normalize_type_and_address(row, row_keys_lower, line_num, address_offset)
            if not dtype: continue

            name, tag = self._process_name_and_tag(row, row_keys_lower, line_num, seen_names, seen_tags)
            info1 = self._determine_info1(row, row_keys_lower, line_num)
            self._check_address_overlap(info1, address, dtype, name, line_num, address_usage)
            coef_a, coef_b = self._calculate_coefficients(row, row_keys_lower)

            # Action normalization
            action = self._get_val(row, 'Action', row_keys_lower)
            ...
            processed_rows.append({
                ...
                'Unit': self._get_val(row, 'Unit', row_keys_lower), 'Action': norm_action
            })

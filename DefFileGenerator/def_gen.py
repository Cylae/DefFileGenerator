...
class Generator:
    ...
    def apply_address_offset(self, address, offset):
        ...
        try:
            # First part is always the base address. Normalize it first.
            base_dec = self.normalize_address_val(parts[0])
            new_base = int(base_dec) + offset
    ...
    def _get_val(self, row, key):
        """Helper to get a value from a row dictionary with case-insensitive key lookup."""
    ...
    def _normalize_type_and_address(self, dtype_raw, address, line_num, name, address_offset):
        """Normalizes the data type and address, applying the offset and handling STR<n>."""
    ...
    def _process_name_and_tag(self, name, tag, line_num, seen_names, seen_tags):
        """Validates Name and Tag, generating Tag if necessary."""
        if name:
        ...
        if not tag and name:
            base_tag = re.sub(r'_{2,}', '_', RE_TAG_CLEAN.sub('_', name.lower().replace(' ', '_')))
            base_tag = base_tag.strip('_')
    ...
    def _determine_info1(self, reg_type_str, line_num):
        """Maps RegisterType to Info1 code."""
        if not reg_type_str:
            return '3'
    ...
    def _check_address_overlap(self, info1, address, dtype, name, line_num, address_usage):
        """Checks for address overlap using O(1) dictionary lookup."""
        try:
            ...
            for addr in range(start_addr, start_addr + reg_count):
                if addr in address_usage[info1]:
                    ...
                    if not (is_bits and u_type == 'BITS' and addr == start_addr):
    ...
    def _calculate_coefficients(self, factor, offset, scale_factor_str):
        """Calculates CoefA and CoefB from input values."""
        try:
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            dtype, address = self._normalize_type_and_address(dtype_raw, address, line_num, name, address_offset)

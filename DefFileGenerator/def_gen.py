...
RE_ADDR_STRING = re.compile(r'^([0-9A-Fa-f-]+|0x[0-9A-Fa-f-]+|[0-9A-Fa-f-]+h)_(\d+)$', re.IGNORECASE)
RE_ADDR_BITS = re.compile(r'^([0-9A-Fa-f-]+|0x[0-9A-Fa-f-]+|[0-9A-Fa-f-]+h)_(\d+)_(\d+)$', re.IGNORECASE)
RE_ADDR_INT = re.compile(r'^([0-9A-Fa-f-]+|0x[0-9A-Fa-f-]+|[0-9A-Fa-f-]+h)$', re.IGNORECASE)
...
RE_NORM_ADDR = re.compile(r'\b(0x[0-9A-Fa-f]+|[0-9A-Fa-f]+h|[0-9A-Fa-f-]+)\b', re.IGNORECASE)
...
class Generator:
    ...
    def normalize_type(self, dtype):
        """Identifies and normalizes the data type from synonyms."""
        if not dtype:
            return "U16"

        t = str(dtype).lower().strip().replace(' ', '')

        # Replace common synonyms
        t = t.replace('unsignedint', 'u')
        t = t.replace('signedint', 'i')
        t = t.replace('unsigned', 'u')
        t = t.replace('signed', 'i')

        # Mapping ordered by specificity (longer first)
        mapping = [
        ...
        for syn, target in mapping:
            if syn in t:
                # If it's a direct match or contains suffixes like _W, _B
                if t.startswith(syn):
                    # Check if it has a suffix we should preserve
                    suffix = t[len(syn):].upper()
                    if suffix in ['_W', '_B', '_WB']:
    ...
    def normalize_address_val(self, addr_part):
        ...
        # Use robust regex to find the address word
        match = RE_NORM_ADDR.search(addr_part)
        if not match:
            return addr_part
        ...
        if word.lower().startswith('0x'):
            try:
                ...
                return word
        elif word.lower().endswith('h'):
            try:
                return str(int(word[:-1], 16))
            except ValueError:
                return word
        # If it contains A-F and no minus sign (except leading), it's likely hex
        hex_chars = 'ABCDEF'
        if any(c in word.upper() for c in hex_chars):
             try:
                return str(int(word, 16))
             except ValueError:
                return word

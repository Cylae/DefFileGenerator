...
class Generator:
    ...
    def apply_address_offset(self, address, offset):
        """Applies an offset to a Modbus address (simple or compound)."""
    ...
    def _get_val(self, row, key):
        """Internal helper to get case-insensitive row value."""
    ...
    def _normalize_type_and_address(self, row, line_num, address_offset):
        """Normalizes and validates type and address for a row."""
        ...
        if not self.validate_address(address, dtype):
            logging.warning(f"Line {line_num}: Invalid Address '{address_raw}' (normalized to '{address}') for Type '{dtype}'. Skipping row.")
            return None, None, None

        return dtype, address, name

    def _process_name_and_tag(self, name, tag, line_num, seen_names, seen_tags):
        """Handles name/tag validation and automatic generation."""
        if name:
    ...
    def _determine_info1(self, reg_type_str, line_num):
        """Determines Webdyn Info1 (RegisterType code)."""
        if not reg_type_str:
            return '3'
    ...
    def _check_address_overlap(self, info1, address, dtype, name, line_num, address_usage):
        """O(1) dictionary-based address overlap detection."""
        try:
            ...
            for addr in range(start_addr, end_addr + 1):
                ...
                # Prioritize non-BITS entries in the usage map

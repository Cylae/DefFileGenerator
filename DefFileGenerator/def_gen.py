...
class Generator:
    ...
    def apply_address_offset(self, address, offset):
        """Applies offset to base address, handling compound addresses."""
        if not address: return ""
        parts = str(address).split('_')
        try:
            # Always normalize base address to decimal even if offset is 0
    ...
    def _get_val(self, row, key):
        """Helper to get and strip value from row dictionary."""
    ...
    def _normalize_type_and_address(self, row, line_num, address_offset):
        """Helper to normalize data type and address."""
    ...
    def _process_name_and_tag(self, row, line_num, seen_names, seen_tags):
        """Helper to process name and generate/validate tag."""
        name = self._get_val(row, 'Name')
        ...
        if not tag and name:
            # RE_TAG_CLEAN replaces non-alphanumeric with underscores
            clean_name = RE_TAG_CLEAN.sub('_', name.lower())
            # Collapse underscores and strip from ends
            tag = re.sub(r'_+', '_', clean_name).strip('_')
            if not tag: tag = "var"
    ...
    def _determine_info1(self, row, line_num):
        """Helper to determine Info1 from RegisterType."""
    ...
    def _check_address_overlap(self, info1, address, dtype, name, line_num, address_usage):
        """Helper to detect address overlaps."""
        try:

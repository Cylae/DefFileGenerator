...
class Generator:
    ...
    def _parse_numeric(self, val, default=0.0):
        """Parses numeric values supporting fractions and European formats."""
    ...
    def _process_name_and_tag(self, name, tag, seen_names, seen_tags, line_num):
        """Normalizes name/tag and handles duplicates."""
        if name:
    ...
    def _determine_info1(self, reg_type_str, line_num):
        """Maps RegisterType to Webdyn Info1 code."""
        if not reg_type_str:
            return '3'
    ...
    def _check_address_overlap(self, info1, address, dtype, name, address_usage, line_num):
        """Checks for address overlaps, allowing multiple BITS on same address."""
        try:
    ...
    def _calculate_coefficients(self, factor, offset, scale_factor):
        """Calculates CoefA and CoefB with high precision."""
        val_factor = self._parse_numeric(factor, 1.0)
        val_scale = int(self._parse_numeric(scale_factor, 0.0))
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            scale_factor = self._get_val(row, 'ScaleFactor')
            ...
            coef_a, coef_b = self._calculate_coefficients(factor, offset, scale_factor)

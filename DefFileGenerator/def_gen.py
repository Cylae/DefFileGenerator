...
# New globals according to memory
...
class Generator:
    ...
    def normalize_address_val(self, addr_part):
        ...
        # Use the pre-compiled regex for strict validation
        if not RE_ADDR_VAL.fullmatch(addr_part):
            return addr_part
    ...
    def apply_address_offset(self, address, offset):
        """Applies offset to base address and normalizes to decimal."""
        if not address:
            return ""
        ...
        try:
            ...
            # Fallback: still normalize the base part if possible
            parts[0] = self.normalize_address_val(parts[0])

        # Ensure all parts are normalized to decimal if they are addresses
        for i in range(1, len(parts)):
            # If it is a STRING or BITS part, it might not need normalization in the same way,
            # but normalize_address_val is safe to call.
            parts[i] = self.normalize_address_val(parts[i])

        return '_'.join(parts)

    def _get_val(self, row, key):
        """Helper to get value from row dict case-insensitively."""
        # Use lowercase stripped keys for lookups as per memory
        lookup = {k.lower().strip(): v for k, v in row.items()}
        val = lookup.get(key.lower().strip(), '')
    ...
    def _normalize_type_and_address(self, row, line_num, address_offset):
        ...
        return dtype, address, dtype_raw
    ...
    def _check_address_overlap(self, name, address, dtype, info1, line_num, address_usage):
        try:
            ...
            # Memory says O(1) dictionary-based lookup, but it also says it tracks ranges.
            # In a list it's O(N). Let's see how to make it O(1) or faster.
            # Actually memory says: "O(1) dictionary-based lookup (address_usage) to track used register addresses per register type (Info1)"
            # That probably means the dict key is Info1. The lookup within the list is still O(N_per_info1).
            # If I want O(1) overall, I'd need a large bitset or dict of every single address.
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            name_in = self._get_val(row, 'Name')
            addr_in = self._get_val(row, 'Address')
            if not name_in and not addr_in:
                logging.warning(f"Line {line_num}: Skipping row with missing Name and Address.")
                continue

            dtype, address, dtype_raw = self._normalize_type_and_address(row, line_num, address_offset)
            if dtype is None:
                continue
    ...
    def write_output_csv(output, processed_rows, manufacturer, model,
        ...
        try:
            if isinstance(output, str):
                if not output: # Support empty string as stdout as per memory
                    outfile = sys.stdout
                else:
...
def generate_template(output_file):
    ...
    try:
        if isinstance(output_file, str) and not output_file:
            f = sys.stdout
        elif output_file:

...
class Generator:
    ...
    def normalize_type(self, dtype):
        ...
        # Re-check for basic codes if cleaned
        for pattern, replacement in synonyms:
    ...
    def apply_address_offset(self, address, offset):
        """Applies an integer offset to a decimal address string (handles _ parts)."""
        if not address or offset == 0:
            # Still run through normalization to ensure decimal format
    ...
    def _get_val(self, row, key):
        """Helper to get value from row dict using case-insensitive key."""
        lower_key = key.lower().strip()
        for k, v in row.items():
            if k.lower().strip() == lower_key:
    ...
    def _process_name_and_tag(self, row, line_num, seen_names, seen_tags):
        """Handles Name/Tag uniqueness and auto-generation."""
        name = self._get_val(row, 'Name')
    ...
    def _determine_info1(self, row, line_num):
        """Determines register type code (Info1)."""
    ...
    def _check_address_overlap(self, line_num, name, address, dtype, info1, address_usage):
        """Checks for address overlaps and updates usage tracking using O(1) dictionary lookup."""
        try:
            ...
            for addr in range(start_addr, end_addr + 1):
                if addr in address_usage[info1]:
                    ...
                    if not (is_bits and u_type == 'BITS' and start_addr == addr):
                        logging.warning(f"Line {line_num}: Address overlap detected for '{name}' (addr {addr}). Overlaps with '{u_name}' (Line {u_line}).")
                        break # Only log once per register

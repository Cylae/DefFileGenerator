...
class Generator:
    ...
    def apply_address_offset(self, address, offset):
        """Applies an offset to the base address of a (possibly compound) address string."""
        if not address:
            return ""
    ...
    def _process_name_and_tag(self, name, tag, seen_tags):
        if not tag and name:
            base_tag = RE_TAG_CLEAN.sub('_', name.lower())
            base_tag = RE_MULTI_UNDERSCORE.sub('_', base_tag).strip('_')
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            tag = self._process_name_and_tag(name, tag, seen_tags)
            if tag:
                if tag in seen_tags:

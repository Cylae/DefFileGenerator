...
RE_ADDR_VAL = re.compile(r'0x[0-9A-Fa-f]+|[0-9A-Fa-f]+h|-?\d+|[0-9A-Fa-f]+', re.IGNORECASE)
...
class Generator:
    ...
    def normalize_address_val(self, addr_part):
        ...
        if RE_ADDR_VAL.fullmatch(addr_part):
            try:
                return str(int(addr_part, 16))
            except ValueError:
                return addr_part
    ...
    def _get_val(self, row, key, row_keys_lower):
        """Case-insensitive value retrieval from row dict."""
    ...
    def _normalize_type_and_address(self, row, line_num, address_offset, row_keys_lower):
        dtype_raw = self._get_val(row, 'Type', row_keys_lower)
        address = self._get_val(row, 'Address', row_keys_lower)
    ...
    def _process_name_and_tag(self, row, line_num, seen_names, seen_tags, row_keys_lower):
        name = self._get_val(row, 'Name', row_keys_lower)
        ...
        if not tag and name:
            base_tag = RE_TAG_CLEAN.sub('_', name.lower().strip()).strip('_')
    ...
    def _determine_info1(self, row, line_num, row_keys_lower):
    ...
    def _check_address_overlap(self, info1, address, dtype, name, line_num, address_usage):
        try:
            ...
            # Mark usage
            for addr in range(start_addr, end_addr + 1):
                # Prioritize non-bit types in the map for better future detection
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        seen_names = {}
        seen_tags = {}
        address_usage = {} # Info1 -> addr -> (line, name, type, start, end)
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            dtype, address = self._normalize_type_and_address(row, line_num, address_offset, row_keys_lower)
            if dtype is None:
                continue

            name, tag = self._process_name_and_tag(row, line_num, seen_names, seen_tags, row_keys_lower)
            if not name and not address:
                logging.warning(f"Line {line_num}: Skipping row with missing Name and Address.")
                continue

            info1 = self._determine_info1(row, line_num, row_keys_lower)
            ...
            unit = self._get_val(row, 'Unit', row_keys_lower)

...
class Generator:
    ...
    def normalize_address_val(self, addr_part):
        ...
        if re.fullmatch(r'[0-9A-Fa-f]+', addr_part):
            try:
                return str(int(addr_part, 16))
            except ValueError:
                return addr_part
    ...
    def apply_address_offset(self, address, offset):
        ...
        try:
            base = int(self.normalize_address_val(parts[0])) + int(offset)
            parts[0] = str(base)
    ...
    def _get_val(self, row, key):
        ...
        if target_key in row_keys:
            val = row[row_keys[target_key]]
            return str(val).strip() if val is not None else ''
        return ''

    def _normalize_type_and_address(self, row, address_offset, line_num):
        ...
        try:
            ...
            if base_addr < 0:
                logging.warning(f"Line {line_num}: Address offset {address_offset} results in negative address {base_addr} for '{self._get_val(row, 'Name')}'.")
        except (ValueError, IndexError):
            pass
    ...
    def _process_name_and_tag(self, row, seen_names, seen_tags, line_num):
    ...
    def _check_address_overlap(self, name, address, dtype, info1, address_usage, line_num):
        try:
            ...
            for addr in range(start_addr, end_addr + 1):
                ...
                # Store usage. For non-bits, always store. For bits, store if not occupied or if non-bits is taking over.
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        seen_names = {}
        seen_tags = {}
        # Info1 -> Address -> (line, name, type, start, end)
        address_usage = {}
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            name, tag = self._process_name_and_tag(row, seen_names, seen_tags, line_num)
            dtype, address = self._normalize_type_and_address(row, address_offset, line_num)
            ...
            self._check_address_overlap(name, address, dtype, info1, address_usage, line_num)

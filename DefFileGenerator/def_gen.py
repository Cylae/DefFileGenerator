...
class Generator:
    ...
    def _parse_numeric(self, val, default=0.0):
        """Parses numeric values with support for fractions and locale formats."""
        if not val or not str(val).strip():
            return default
        s = str(val).strip()

        # Fraction support (e.g. 1/10)
        if '/' in s:
            try:
                parts = s.split('/')
        ...
        # Locale-specific format handling (e.g. 1.234,56 vs 1,234.56)
        if ',' in s and '.' in s:
    ...
    def apply_address_offset(self, address, offset, line_num=None):
        """Applies offset to address, supporting compound underscore formats."""
        if not address:
            return address
        parts = address.split('_')
    ...
    def _process_name_and_tag(self, row, line_num, seen_names, seen_tags):
        """Extracts name and ensures tag uniqueness."""
        ...
        if not tag and name:
            ...
            # Ensure tag doesn't start with invalid character for Webdyn
    ...
    def _check_address_overlap(self, info1, address, dtype, name, line_num, address_usage):
        """Checks for register address overlaps."""
        try:
            parts = address.split('_')
            ...
            for u_start, u_end, u_line, u_name, u_type in address_usage[info1]:
                if max(start_addr, u_start) <= min(end_addr, u_end):
                    # Allow overlap if both are BITS and share same address
    ...
    def _calculate_coefficients(self, row):
        ...
        val_scale = self._parse_numeric(scale_factor_str, 0.0)
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            address = self.apply_address_offset(address_raw, address_offset, line_num)

...
class Generator:
    ...
    def apply_address_offset(self, address, offset):
        """Applies offset to the base part of an address and normalizes to decimal."""
        if not address: return ""
        parts = str(address).split('_')
        try:
            base_addr_norm = self.normalize_address_val(parts[0])
            base_addr = int(base_addr_norm) + offset
    ...
    def _get_val(self, row, key):
        """Optimized row value retrieval."""
        key_low = key.lower()
        for k, v in row.items():
            if k.lower().strip() == key_low:
        ...
        if address:
            ...
            try:
                if int(address.split('_')[0]) < 0:
                    logging.warning(f"Line {line_num}: Address offset {address_offset} results in negative address for '{self._get_val(row, 'Name')}'.")
            except (ValueError, IndexError):
                pass
...
def generate_template(output_file):
    ...
    try:
        f = (open(output_file, 'w', newline='', encoding='utf-8') if output_file and output_file.strip()
             else sys.stdout)
        writer = csv.writer(f)

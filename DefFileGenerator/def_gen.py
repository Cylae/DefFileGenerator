...
class Generator:
    def __init__(self):
        ...
        self.address_usage = {} # Info1 -> list of (start, end, line, name, type)
    ...
    def validate_address(self, address, dtype):
        ...
        if dtype_upper == 'STRING':
            ...
            # BITS type requires compound address Address_StartBit_Length
    ...
    def _process_name_and_tag(self, name, tag, line_num, seen_names, seen_tags):
        ...
        if not tag and name:
            ...
            if not base_tag or not base_tag[0].isalpha():
                tag = f"v_{base_tag}" if base_tag else "var"
            else:
                tag = base_tag
    ...
    def _check_address_overlap(self, info1, address, dtype, name, line_num):
        try:
            start_addr = int(address.split('_')[0])
            reg_count = self.get_register_count(dtype, address)
            end_addr = start_addr + reg_count - 1

            if info1 not in self.address_usage:
                self.address_usage[info1] = []

            is_bits = (dtype.upper() == 'BITS')
            for u_start, u_end, u_line, u_name, u_type in self.address_usage[info1]:
                if max(start_addr, u_start) <= min(end_addr, u_end):
                    # Allow multiple BITS registers at the same address
            ...
            self.address_usage[info1].append((start_addr, end_addr, line_num, name, dtype.upper()))
    ...
    def apply_address_offset(self, address, offset):
        """Applies an integer offset to a decimal or compound address."""
        if not address:
            return ""
        parts = address.split('_')
        try:
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            if address:
                ...
                if address.startswith('-'):
                    logging.warning(f"Line {line_num}: Negative address result {address} for '{name}'.")
            ...
            self._check_address_overlap(info1, address, dtype, name, line_num)

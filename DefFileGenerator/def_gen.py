...
class Generator:
    ...
    def _get_val(self, row, key, key_map=None):
        """Helper to get value from row dict with case-insensitive key."""
        if key_map and key.lower() in key_map:
            return str(row.get(key_map[key.lower()], '')).strip()
        for k, v in row.items():
            if k.lower().strip() == key.lower():
                return str(v).strip() if v is not None else ''
        return ''

    def _normalize_type_and_address(self, row, line_num, address_offset, key_map):
        dtype_raw = self._get_val(row, 'Type', key_map)
        address = self._get_val(row, 'Address', key_map)
        name = self._get_val(row, 'Name', key_map)
        ...
        return dtype, address, name

    def _process_name_and_tag(self, row, line_num, name, seen_names, seen_tags, key_map):
        tag = self._get_val(row, 'Tag', key_map)
        if name:
    ...
    def _determine_info1(self, row, line_num, key_map):
        reg_type_str = self._get_val(row, 'RegisterType', key_map)
        info1 = '3'
        if reg_type_str:
    ...
    def _check_address_overlap(self, line_num, name, address, dtype, info1, address_usage):
        try:
            ...
            # Optimized O(1) overlap check using dictionary
            for addr in range(start_addr, end_addr + 1):
                if addr in address_usage[info1]:
                    u_start, u_end, u_line, u_name, u_type = address_usage[info1][addr]
            ...
            usage_info = (start_addr, end_addr, line_num, name, dtype.upper())
            for addr in range(start_addr, end_addr + 1):
                address_usage[info1][addr] = usage_info
        except (ValueError, IndexError):
            pass

    def _calculate_coefficients(self, row, key_map):
        factor = self._get_val(row, 'Factor', key_map)
        offset = self._get_val(row, 'Offset', key_map)
        scale_factor_str = self._get_val(row, 'ScaleFactor', key_map)
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        seen_names = {}
        seen_tags = {}
        address_usage = {} # Info1 -> dict of addr -> (start, end, line, name, type)
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            if key_map is None:
                key_map = {k.lower().strip(): k for k in row.keys()}

            dtype, address, name = self._normalize_type_and_address(row, line_num, address_offset, key_map)
            if dtype is None:
                continue
            ...
            tag = self._process_name_and_tag(row, line_num, name, seen_names, seen_tags, key_map)
            info1 = self._determine_info1(row, line_num, key_map)
            self._check_address_overlap(line_num, name, address, dtype, info1, address_usage)
            coef_a, coef_b = self._calculate_coefficients(row, key_map)

            unit = self._get_val(row, 'Unit', key_map)
            action = self._get_val(row, 'Action', key_map)

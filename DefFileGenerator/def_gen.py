...
class Generator:
    ...
    def _get_val(self, row, key):
        """Case-insensitive helper to get row value."""
    ...
    def _parse_numeric(self, val):
        """Parses numeric strings, handling European formats and fractions."""
    ...
    def _process_name_and_tag(self, name, tag, seen_tags, line_num):
        """Generates and validates unique tags."""
        if not tag and name:
    ...
    def _check_address_overlap(self, info1, address, dtype, name, address_usage, line_num):
        """Detects overlapping registers within the same RegisterType."""
        try:
            ...
            for u_start, u_end, u_line, u_name, u_type in address_usage[info1]:
                if max(start_addr, u_start) <= min(end_addr, u_end):
                    # Special case: allow multiple BITS registers on the same base address
    ...
    def _calculate_coefficients(self, factor, scale_factor_str, offset):
        """Calculates WebdynSunPM CoefA and CoefB."""
        try:

...
class Generator:
    ...
    def _normalize_type_and_address(self, row, address_offset, line_num):
        """Normalizes type and address, handles STR<n> and offset."""
    ...
    def _process_name_and_tag(self, row, seen_names, seen_tags, line_num):
        """Processes Name and Tag, ensuring uniqueness."""
        name = self._get_val(row, 'Name')
    ...
    def _determine_info1(self, row, line_num):
        """Determines Info1 (register type) from RegisterType column."""
    ...
    def _check_address_overlap(self, address_usage, info1, address, dtype, name, line_num):
        """Optimized O(1) overlap detection using dictionary tracking."""
        try:
    ...
    def _calculate_coefficients(self, row):
        """Calculates CoefA and CoefB based on Factor, ScaleFactor, and Offset."""
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            self._check_address_overlap(address_usage, info1, address, dtype, name, line_num)

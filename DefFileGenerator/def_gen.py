...
class Generator:
    ...
    def apply_address_offset(self, address, offset):
        """Applies offset to the base address while handling compound formats."""
    ...
    def _normalize_type_and_address(self, row, line_num, address_offset):
        """Extracts and normalizes Type and Address from row."""
        ...
        if address:
            ...
            # Warning for negative addresses
            try:
    ...
    def _process_name_and_tag(self, row, line_num, seen_names, seen_tags):
        ...
        if not tag and name:
            # Enhanced tag sanitization
            clean_name = name.lower().replace(' ', '_')
            base_tag = RE_TAG_CLEAN.sub('_', clean_name).strip('_')
    ...
    def _determine_info1(self, row, line_num):
        """Determines Webdyn Info1 from RegisterType."""
        ...
        try:
            ...
            for i in range(start_addr, end_addr + 1):
                if i in address_usage[info1]:
                    ...
                    # Overlap if not both are BITS at the same address
                    if not (is_bits and u_type == 'BITS' and start_addr == i):
                         logging.warning(f"Line {line_num}: Address overlap detected for '{name}' at {i}. Overlaps with '{u_name}' (Line {u_line}).")

                # Prioritize non-bits in usage map
                if i not in address_usage[info1] or address_usage[info1][i][2] == 'BITS':
                    # If existing is BITS and current is NOT BITS, overwrite to prioritize non-bits for future overlap checks
                    if not (is_bits and i in address_usage[info1] and address_usage[info1][i][2] != 'BITS'):
                        address_usage[info1][i] = (line_num, name, dtype.upper())
                elif not is_bits:
                    # Current is not bits, existing is not bits, we already logged overlap warning, but let's record the last one
                    address_usage[info1][i] = (line_num, name, dtype.upper())
    ...
    def _calculate_coefficients(self, row):
        """Calculates CoefA and CoefB from Factor, Offset, and ScaleFactor."""

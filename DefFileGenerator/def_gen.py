...
class Generator:
    ...
    def apply_address_offset(self, address, offset, line_num=None):
        """Applies an offset to the base address and normalizes parts."""
        if not address:
            return address
        parts = address.split('_')
    ...
    def _parse_numeric(self, val, default=0.0):
        """Robustly parses numeric values with fraction and locale support."""
        ...
        # Locale-specific parsing: identify decimal separator by position
        # If there's both ',' and '.', one is thousands and one is decimal
        if ',' in s and '.' in s:
            if s.rfind(',') > s.rfind('.'):
            ...
            # Check if comma is likely a decimal (only one and near end)
            if s.count(',') == 1 and (len(s) - s.find(',') <= 3):
                s = s.replace(',', '.')
            else:
                s = s.replace(',', '')
    ...
    def _process_name_and_tag(self, name, tag, seen_names, seen_tags, line_num):
        """Validates name uniqueness and ensures unique tag generation."""
        if name:
    ...
    def _determine_info1(self, reg_type_str, line_num):
        """Maps register type string to Webdyn Info1 code."""
        info1 = '3'
        if reg_type_str:
    ...
    def _check_address_overlap(self, info1, address, dtype, name, address_usage, line_num):
        """Detects overlapping register ranges within the same protocol area."""
        try:
    ...
    def _calculate_coefficients(self, factor_str, scale_factor_str, offset_str):
        """Calculates CoefA and CoefB from raw inputs."""
        val_factor = self._parse_numeric(factor_str, default=1.0)
        val_scale = self._parse_numeric(scale_factor_str, default=0.0)
        coef_a = "{:.6f}".format(val_factor * (10 ** val_scale))

        val_offset = self._parse_numeric(offset_str, default=0.0)
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            name, tag = self._process_name_and_tag(name, tag, seen_names, seen_tags, line_num)

...
class Generator:
    ...
    def sanitize_csv_field(field: Any) -> str:
        """
        Prevents CSV Formula Injection by prepending a single quote to dangerous starters.
        Only applied to fields that are likely to contain user-provided text.
        """
    ...
    def validate_csv(self, filepath: str, strict_overlap: bool = True) -> bool:
        ...
        try:
            ...
            with open(filepath, 'r', encoding=encoding) as f:
                ...
                is_webdyn = delimiter == ';' and any(p in snippet for p in ['modbusRTU', 'modbusTCP'])

                if is_webdyn:
                    reader = csv.reader(f, delimiter=';')
                    header = next(reader, None)
                    if not header or len(header) < 4:
                        logging.warning("Insufficient columns in global header.")

                    for line_num, row in enumerate(reader, start=2):
                        if not row or (row[0] and str(row[0]).startswith('#')) or not any(row): continue
                        if len(row) < 11:
                        ...
                        if overlap and strict_overlap:
                            success = False
                else:
                    reader = csv.DictReader(f, delimiter=delimiter)
                    for line_num, row in enumerate(reader, start=2):
                        if not row or any(k and str(k).startswith('#') for k in row.keys()): continue
                        dtype = row.get('Type', '')
                        addr = row.get('Address', '')
                        name = row.get('Name', '')
                        tag = row.get('Tag', '')
                        reg_type = row.get('RegisterType', 'Holding Register')
                        ...
                        if tag:
                            if tag in seen_tags:
                                logging.error(f"Line {line_num}: Duplicate Tag '{tag}'")
                                success = False
                            seen_tags[tag] = line_num

                        info1 = self._determine_info1(reg_type)
                        overlap = self._check_address_overlap(info1, addr, dtype, name, line_num, address_usage, warned_lines)
                        if overlap and strict_overlap:
                            success = False
    ...
    def apply_address_offset(address: Any, offset: int, line_num: Optional[int] = None, name: Optional[str] = None) -> str:
        ...
        try:
            base_addr = int(norm_parts[0], 0) + offset
            if base_addr < 0:
    ...
    def _check_address_overlap(self, info1: str, address: str, dtype: str, name: str, line_num: int, address_usage: Dict[str, List[Tuple]], warned_lines: Set[Tuple[int, int]]) -> bool:
        try:
            addr_part = address.split('_')[0]
            start_addr = int(Generator.normalize_address_val(addr_part), 0)
            ...
            intervals = address_usage[info1]
            overlap_detected = False

            for u_start, u_end, u_line, u_name, u_type in intervals:
            ...
            intervals.append((start_addr, end_addr, line_num, name, dtype.upper()))
    ...
    def process_rows(self, rows: Iterable[Dict[str, Any]], address_offset: int = 0) -> Iterator[Dict[str, Any]]:
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            if RE_TYPE_STR_CONV.match(dtype):
                match = RE_TYPE_STR_CONV.match(dtype)
                if '_' not in address:
                    address = f"{address}_{match.group(1)}"
                dtype = 'STRING'
    ...
    def write_output_csv(output: Union[str, Any, None], processed_rows: Iterable[Dict[str, Any]], manufacturer: str, model: str,
        ...
        try:
            ...
            for index, row in enumerate(processed_rows, start=1):
                i1 = row['Info1']
                type_counts[i1] = type_counts.get(i1, 0) + 1
                writer.writerow([
                    str(index), i1,
                    row['Info2'],
                    row['Info3'],
            ...
            if total_registers > 0:
                logging.info(f"Generated {total_registers} registers ({summary})")
...
def main():
    ...
    parser.add_argument('input_file', nargs='?', help='Input CSV')

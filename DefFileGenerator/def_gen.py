...
class Generator:
    ...
    def apply_address_offset(self, address, offset):
        """Applies offset to the base address part of potentially compound addresses."""
        if not address:
            return ""
    ...
    def _parse_numeric(self, val, default=0.0):
        """Robust numeric parsing for floats, fractions, and European formats."""
    ...
    def _process_name_and_tag(self, name, tag, line_num, seen_names, seen_tags):
        ...
        if not tag and name:
            ...
            tag = base_tag if base_tag and base_tag[0].isalpha() else ("v_" + base_tag if base_tag else "var")
            counter = 1
            while tag in seen_tags:
                tag = f"{base_tag}_{counter}" if base_tag and base_tag[0].isalpha() else f"v_{base_tag}_{counter}"
                counter += 1
    ...
    def _determine_info1(self, reg_type_str, line_num):
        """Determines the Webdyn Info1 code from RegisterType."""
        info1 = '3'
        if reg_type_str:
    ...
    def _check_address_overlap(self, info1, address, dtype, name, line_num, address_usage):
        """Optimized O(1) overlap detection with BITS exception."""
        try:
            ...
            for a in range(start_addr, end_addr + 1):
                if a in address_usage[info1]:
                    ...
                    if not (is_bits and u_type == 'BITS'):
                        logging.warning(f"Line {line_num}: Address overlap detected for '{name}' at {a}. Overlaps with '{u_name}' (Line {u_line}).")
                else:
    ...
    def _calculate_coefficients(self, factor, offset, scale_factor_str):
        """Calculates CoefA and CoefB from factor, offset, and scale factor."""
        try:
            val_factor = self._parse_numeric(factor, default=1.0)
            val_scale = int(self._parse_numeric(scale_factor_str, default=0.0))
            coef_a = "{:.6f}".format(val_factor * (10 ** val_scale))
        except (ValueError, OverflowError):
            coef_a = "1.000000"

        try:
            val_offset = self._parse_numeric(offset, default=0.0)
    ...
    def write_output_csv(output, processed_rows, manufacturer, model,
        ...
        try:
            if isinstance(output, str):
                ...
                outfile = output if output is not None else sys.stdout
                do_write(outfile)
        except Exception as e:
            logging.error(f"Error writing output CSV: {e}")

def generate_template(output_file):
    """Generates a sample definition CSV file."""
...
def run_generator(config: GeneratorConfig):
    """Entry point for running the generation process."""
    if config.template:

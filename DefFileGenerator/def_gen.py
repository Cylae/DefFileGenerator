...
class Generator:
    ...
    def validate_csv(self, filepath: str) -> bool:
        """Comprehensive validation for Webdyn definition files."""
        if not os.path.exists(filepath):
            ...
            with open(filepath, 'r', encoding=encoding) as f:
                ...
                for line_num, row in enumerate(reader, start=2):
                    ...
                    if not self.validate_address(info2, info3):
                        logging.warning(f"Line {line_num}: Invalid Address format or range '{info2}' for type '{info3}'.")
                        valid = False
    ...
    def process_rows(self, rows: Iterable[Dict[str, Any]], address_offset: int = 0) -> Iterator[Dict[str, Any]]:
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            if not act_str:
                # Default based on RegisterType: Input (4) or Discrete (2) default to ReadOnly (4)
                if info1 in ['2', '4']:
                    norm_action = '4'
                else:
                    norm_action = '1'
...
def generate_template(output_file: Optional[str], mode: str = 'input') -> None:
    if mode == 'definition':
        ...
        header_row = ['modbusRTU', 'Inverter', 'Manufacturer', 'Model', '', '', '', '', '', '', '']
        rows = [

...
class Generator:
    ...
    def normalize_address_val(self, addr_part):
        """Converts a single address part (hex/decimal/negative) to decimal string."""
        ...
        # Optimized regex for hex, decimal, or negative integer words
    ...
    def normalize_action(self, action):
        """Centralized normalization for Modbus Action values."""
        ...
        if act_str in ['RW', 'W', 'WRITE', '1']:
            return '1'
        return act_str if act_str in self.allowed_actions else '1'

    def _get_val(self, row, key, row_keys_lower=None):
        """Optimized case-insensitive dictionary lookup."""
        if row_keys_lower is None:
            for k, v in row.items():
                if k.lower().strip() == key.lower():
                    return str(v).strip() if v is not None else ''
            return ''

        # Use pre-calculated lowercase keys for O(1) lookup
        source_key = row_keys_lower.get(key.lower())
        if source_key:
    ...
    def _normalize_type_and_address(self, dtype_raw, address, address_offset, line_num, name):
        """Standardizes type, handles STR<n> expansion, and applies address offset."""
    ...
    def _process_name_and_tag(self, name, tag, line_num, seen_names, seen_tags):
        """Handles auto-tag generation and duplicate Name/Tag detection."""
        if name:
    ...
    def _check_address_overlap(self, info1, address, dtype, line_num, name, address_usage):
        """Optimized O(1) dictionary-based overlap detection."""
        try:
    ...
    def _calculate_coefficients(self, factor, scale_factor_str, offset):
        """Calculates CoefA and CoefB scaling values."""
        try:
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            # Pre-calculate lowercase keys for faster lookup
            ...
            dtype, address = self._normalize_type_and_address(dtype_raw, address, address_offset, line_num, name)
            if dtype is None:
                continue

            tag = self._process_name_and_tag(name, tag, line_num, seen_names, seen_tags)
            info1 = self._determine_info1(self._get_val(row, 'RegisterType', row_keys_lower), line_num)
            self._check_address_overlap(info1, address, dtype, line_num, name, address_usage)

            coef_a, coef_b = self._calculate_coefficients(
                self._get_val(row, 'Factor', row_keys_lower),
                self._get_val(row, 'ScaleFactor', row_keys_lower),
                self._get_val(row, 'Offset', row_keys_lower)
            )

            processed_rows.append({
                'Info1': info1, 'Info2': address, 'Info3': dtype, 'Info4': '',
                'Name': name, 'Tag': tag, 'CoefA': coef_a, 'CoefB': coef_b,
                'Unit': self._get_val(row, 'Unit', row_keys_lower),
                'Action': self.normalize_action(self._get_val(row, 'Action', row_keys_lower))
            })

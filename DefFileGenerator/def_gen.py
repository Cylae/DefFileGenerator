...
class Generator:
    ...
    def _check_address_overlap(self, info1, address, dtype, name, line_num, address_usage, warned_lines=None):
        """Checks for address overlaps in O(1) average time using a dictionary."""
        try:
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        seen_names = {}
        seen_tags = {}
        address_usage = {} # Info1 -> {address -> list of (line, name, type, start, end)}
        warned_lines = set() # (line1, line2) to avoid duplicate warnings

...
# Module-level globals for tag sanitization and address validation
...
class Generator:
    ...
    def normalize_address_val(self, addr_part):
        ...
        # Use strict regex for validation
    ...
    def _get_val(self, row, key):
        ...
        # Pre-calculated lowercase/stripped keys for faster lookup
        row_keys = {k.lower().strip(): v for k, v in row.items()}
        val = row_keys.get(key.lower().strip(), '')
    ...
    def _normalize_type_and_address(self, row, line_num, address_offset):
        ...
        if match_str:
            ...
            row['Address'] = address # Update row for subsequent processing
        ...
        if address:
            ...
            try:
                ...
                if base_addr < 0:
                     logging.warning(f"Line {line_num}: Address offset {address_offset} results in negative address {base_addr}.")
            except (ValueError, IndexError):
                pass
        ...
        try:
            ...
            for i in range(reg_count):
                ...
                if addr in address_usage[info1]:
                    ...
                    # If current is not BITS, it takes priority in the map for future checks
                    if not is_bits:

...
class Generator:
    ...
    def _get_val(self, row, key):
        """Optimized row value retrieval using case-insensitive keys."""
        k_lower = key.lower()
    ...
    def _normalize_type_and_address(self, row, line_num, address_offset):
        """Normalizes type and applies address offset."""
        ...
        if address:
            ...
            # Check if address is negative after offset
            try:
    ...
    def _process_name_and_tag(self, row, line_num, seen_names, seen_tags):
        """Handles name and tag normalization and duplicate detection."""
        name = self._get_val(row, 'Name')
        ...
        if not tag and name:
            # Re-using logic from process_rows
    ...
    def _check_address_overlap(self, name, address, dtype, info1, line_num, address_usage):
        """Performs O(1) optimized address overlap detection."""
        try:
    ...
    def _calculate_coefficients(self, factor, offset, scale_factor_str):
        """Calculates CoefA and CoefB values."""
        try:
    ...
    def apply_address_offset(self, address, offset):
        ...
        # Normalize the base address to decimal and apply offset
        base_addr_str = self.normalize_address_val(parts[0])
        try:
            base_addr = int(base_addr_str) + offset
            parts[0] = str(base_addr)
        except ValueError:
            # If normalization failed to produce an int, keep it as is
            parts[0] = base_addr_str

        # Ensure all other parts are also normalized

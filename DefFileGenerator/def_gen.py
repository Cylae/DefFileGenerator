...
class Generator:
    ...
    def _parse_numeric(self, val, default=0.0):
        ...
        # Robust handling of thousands separators
        if ',' in s and '.' in s:
            # Determine which one is the decimal separator based on last occurrence
            if s.rfind(',') > s.rfind('.'):
            ...
            # Single separator - if it's a comma, treat as decimal
            s = s.replace(',', '.')

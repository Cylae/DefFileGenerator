    ...
    def validate_csv(self, filepath: str) -> bool:
        ...
        try:
            ...
            with open(filepath, mode='r', encoding=encoding) as f:
                ...
                if len(header) < 4:
                    logging.error("Invalid Webdyn definition header (too few columns).")
                    valid = False

                for line_num, row in enumerate(reader, start=2):
                    if not row or not any(row): continue
                    if len(row) < 11:
                        logging.warning(f"Line {line_num}: Row has too few columns ({len(row)}/11). Skipping.")
                        continue
                    ...
                    if not self.validate_address(info2, info3):
                        # The message is logged in validate_address
                        valid = False

    ...
    def _check_address_overlap(self, info1, address, dtype, name, line_num, address_usage, warned_lines):
        ...
        try:
            ...
            if info1 not in address_usage:
                address_usage[info1] = {} # addr -> list of (line_num, name, dtype)
                    ...
                    for u_line, u_name, u_type in address_usage[info1][addr]:
                        ...
                        if not (is_bits and u_type == 'BITS' and addr == start_addr):
                            ...
                            if pair not in warned_lines:
                                logging.warning(f"Line {line_num}: Address overlap detected for '{name}' (starting at {start_addr}). Overlaps with '{u_name}' (Line {u_line}).")

...
def peek_generator(iterable: Any) -> Tuple[bool, Iterator]:
    """
...
class Generator:
    ...
    def process_rows(self, rows: Iterable[Dict[str, Any]], address_offset: int = 0) -> Iterator[Dict[str, Any]]:
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            if not act_str:
                # Default Action based on RegisterType (Info1)
                # 2: Discrete Input, 4: Input Register are usually Read-Only (4)
                if info1 in ['2', '4']:
                    norm_action = '4'
                else:
                    norm_action = '1'

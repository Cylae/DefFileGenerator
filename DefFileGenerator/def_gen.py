...
class Generator:
    ...
    def apply_address_offset(self, address, offset):
        """Applies offset to base address in simple or compound formats. Always normalizes to decimal."""
        if not address:
            return address
        parts = address.split('_')
        try:
            # Always normalize all parts (especially the first one which might be hex)
            norm_parts = [self.normalize_address_val(p) for p in parts]
            base = int(norm_parts[0])
            norm_parts[0] = str(base + offset)
    ...
    def _parse_numeric(self, val):
        """Handles floats, fractions (1/10), and European commas (0,1)."""
    ...
    def _process_name_and_tag(self, name, tag, line_num, seen_names, seen_tags):
        ...
        if not tag and name:
            # Replaces non-alphanumeric with underscores, collapses multiples, strips ends
    ...
    def _calculate_coefficients(self, factor, offset, scale_factor_str):
        f_val = self._parse_numeric(factor)
        if f_val is None: f_val = 1.0

        try:
            s_val = int(float(scale_factor_str)) if scale_factor_str and str(scale_factor_str).strip() else 0
        except ValueError:
            s_val = 0

        coef_a = "{:.6f}".format(f_val * (10 ** s_val))

        o_val = self._parse_numeric(offset)

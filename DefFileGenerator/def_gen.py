...
RE_TAG_COLLAPSE = re.compile(r'_+')
...
class Generator:
    ...
    def _parse_numeric(val, default=0.0):
        """Robust numeric parsing for standard, fraction, and locale formats."""
    ...
    @staticmethod
    def _calculate_coefficients(factor_str, scale_factor_str, offset_str):
        """Calculates CoefA and CoefB from input strings."""
        factor = Generator._parse_numeric(factor_str, default=1.0)
        scale = Generator._parse_numeric(scale_factor_str, default=0.0)
        offset = Generator._parse_numeric(offset_str, default=0.0)

        try:
            coef_a = "{:.6f}".format(factor * (10 ** int(scale)))
    ...
    def _process_name_and_tag(self, name, tag, line_num, seen_names, seen_tags):
        """Validates name uniqueness and generates/validates unique tags."""
        if name:
        ...
        if not tag and name:
            ...
            base_tag = RE_TAG_CLEAN.sub('_', name.lower()).strip('_')
            base_tag = RE_TAG_COLLAPSE.sub('_', base_tag)
            if not base_tag or not base_tag[0].isalpha():
                base_tag = 'v_' + base_tag if base_tag else 'var'
    ...
    def _check_address_overlap(self, info1, address, dtype, name, line_num, address_usage):
        """Detects overlapping register ranges within the same Info1 group."""
        try:
            ...
            for u_start, u_end, u_line, u_name, u_type in address_usage[info1]:
                if max(start_addr, u_start) <= min(end_addr, u_end):
                    # Multiple BITS on the same address is allowed
    ...
    def apply_address_offset(self, address, offset, line_num=None):
        """Shifts address (including compound formats) by the given offset."""
        if not address:
            return ""
        ...
        try:
            ...
            if base_addr < 0:
                ...
                if line_num:
                    logging.warning(f"Line {line_num}: {msg} for address '{address}'.")
                else:
                    logging.warning(f"{msg} for address '{address}'.")
    ...
    def _get_val(row, key):
        """Case-insensitive value getter for dictionary rows."""
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            name = Generator._get_val(row, 'Name')
            tag_raw = Generator._get_val(row, 'Tag')
            reg_type_str = Generator._get_val(row, 'RegisterType')
            address_raw = Generator._get_val(row, 'Address')
            dtype_raw = Generator._get_val(row, 'Type')
            factor_raw = Generator._get_val(row, 'Factor')
            offset_raw = Generator._get_val(row, 'Offset')
            unit = Generator._get_val(row, 'Unit')
            action_raw = Generator._get_val(row, 'Action')
            scale_factor_raw = Generator._get_val(row, 'ScaleFactor')
            ...
            tag = self._process_name_and_tag(name, tag_raw, line_num, seen_names, seen_tags)
            ...
            coef_a, coef_b = self._calculate_coefficients(factor_raw, scale_factor_raw, offset_raw)

            # Action normalization
            act_str = str(action_raw).strip().upper()
            if not act_str:
                norm_action = '1'

...
class Generator:
    ...
    def apply_address_offset(self, address, offset):
        """Applies offset to base address while preserving compound formats."""
        if not address:
            return address
        parts = address.split('_')
        try:
            # Normalize and apply offset to the first part
            base = int(self.normalize_address_val(parts[0])) + offset
            parts[0] = str(base)
            # Normalize other parts as well (e.g. length or bit start)
    ...
    def _parse_numeric(self, val):
        """Parses numeric values supporting standard, fractions, and European decimals."""
        if not val:
            return 0.0
    ...
    def _process_name_and_tag(self, name, tag, seen_names, seen_tags, line_num):
        ...
        if not tag and name:
            # Replaces non-alphanumeric with _, collapses multi-__, and strips start/end
    ...
    def _check_address_overlap(self, info1, address, dtype, name, address_usage, line_num):
        try:
            ...
            # Check overlap using address_usage map
            ...
            # Update map. Non-bits types overwrite bit entries to ensure better detection for non-bits
            for addr in range(start_addr, end_addr + 1):

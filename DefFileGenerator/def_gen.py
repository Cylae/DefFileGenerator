...
class Generator:
    ...
    def normalize_address_val(self, addr_part):
        ...
        # Strict validation using RE_ADDR_VAL
    ...
    def apply_address_offset(self, address, offset):
        """Applies an offset to the base address and returns the new address string."""
        if not address:
            return ""
        parts = address.split('_')
        try:
    ...
    def _normalize_type_and_address(self, row, line_num, address_offset):
        """Normalizes data type and address, applying offset."""
    ...
    def _check_address_overlap(self, name, line_num, address, dtype, info1, address_usage):
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            self._check_address_overlap(name, line_num, address, dtype, info1, address_usage)
            coef_a, coef_b = self._calculate_coefficients(row)

            unit = self._get_val(row, 'Unit')
            action = self.normalize_action(self._get_val(row, 'Action'))
    ...
    def write_output_csv(output, processed_rows, manufacturer, model,
        ...
        try:
            if isinstance(output, str):
                if not output: # Handle empty string as stdout
                    outfile = sys.stdout
                else:
...
def run_generator(config: GeneratorConfig):
    ...
    if not config.input_file or (not config.manufacturer and not config.template) or (not config.model and not config.template):
        # manufacturer and model are optional only if template is True

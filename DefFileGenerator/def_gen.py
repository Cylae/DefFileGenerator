...
class Generator:
    ...
    def apply_address_offset(self, address, offset):
        """Applies an offset to the base address while maintaining formatting."""
        if not address:
            return address
        parts = address.split('_')
        try:
            # We normalize the base address to decimal even if offset is 0 for consistency
    ...
    def _get_val(self, row, key):
        """Helper to safely extract trimmed values from a row dictionary."""
    ...
    def _parse_numeric(self, val, default=0.0):
        """Robustly parses numeric strings including fractions and comma-decimals."""
        if not val:
            return default
    ...
    def _process_name_and_tag(self, row, line_num, seen_names, seen_tags):
        """Extracts Name and Tag, handling duplicates and auto-generation."""
        ...
        try:
            ...
            for u_start, u_end, u_line, u_name, u_type in address_usage[info1]:
                if max(start_addr, u_start) <= min(end_addr, u_end):
                    # Exception: Multiple BITS registers at the same base address are allowed
    ...
    def _calculate_coefficients(self, factor, scale_factor_str, offset):
        """Calculates Webdyn linear transformation coefficients CoefA and CoefB."""
        val_factor = self._parse_numeric(factor, 1.0)
        val_scale = self._parse_numeric(scale_factor_str, 0)
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            coef_a, coef_b = self._calculate_coefficients(self._get_val(row, 'Factor'),
                                                        self._get_val(row, 'ScaleFactor'),
                                                        self._get_val(row, 'Offset'))

            # Action normalization
            act_raw = self._get_val(row, 'Action').upper()
            if not act_raw:
                norm_action = '1'
            elif act_raw in ['R', 'READ', '4']:
                norm_action = '4'
            elif act_raw in ['RW', 'W', 'WRITE', '1']:
                norm_action = '1'
            elif act_raw in self.allowed_actions:
                norm_action = act_raw
            else:
                norm_action = '1'

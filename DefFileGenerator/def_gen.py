...
class Generator:
    ...
    def normalize_type(self, dtype):
        ...
        # Suffix handling: preserve _B, _W, _WB
        suffix = ""
        for s in ["_wb", "_w", "_b"]:
            if d.endswith(s):
                suffix = s.upper()
        ...
        mapping = [
            ...
            ('uint', 'U'), ('int', 'I'),
            ('boolean', 'BITS'), ('bool', 'BITS')
        ]

        for k, v in mapping:
            if k in d:
                # Use regex to replace to avoid partial matches if not careful,
                # but specificity order already helps.
                d = d.replace(k, v)
                break

        res = d.upper() + suffix
        # If it's just 'U' or 'I', assume 16-bit
        if res == 'U': return 'U16'
        if res == 'I': return 'I16'
    ...
    def normalize_address_val(self, addr_part):
        ...
        if addr_part.lower().startswith('0x'):
            ...
            try:
                ...
                val = addr_part
        # If it contains A-F, it's likely hex (unless it's already just digits)
        elif not addr_part.isdigit() and any(c in addr_part.upper() for c in 'ABCDEF'):
            try:
                val = str(int(addr_part, 16))
            except ValueError:
                val = addr_part

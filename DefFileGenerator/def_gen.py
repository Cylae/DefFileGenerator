...
class Generator:
    ...
    def _get_val(self, row, key, precomputed_keys=None):
        if precomputed_keys and key.lower().strip() in precomputed_keys:
            k = precomputed_keys[key.lower().strip()]
            return str(row[k]).strip() if row[k] is not None else ''
    ...
    def _check_address_overlap(self, line_num, name, address, dtype, info1, address_usage):
        try:
            ...
            # Use O(1) dictionary check for each register address
            for addr in range(start_addr, end_addr + 1):
                if addr in address_usage[info1]:
                    ...
                    if not (is_bits and u_type == 'BITS' and start_addr == addr):
                         logging.warning(f"Line {line_num}: Address overlap detected for '{name}' (address {addr}). Overlaps with '{u_name}' (Line {u_line}).")
                         break # Only warn once per register entry
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        # Info1 -> dict mapping address -> (line, name, type)
        address_usage = {}

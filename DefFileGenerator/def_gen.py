...
class Generator:
    ...
    def apply_address_offset(self, address, offset, line_num=None):
        """Applies offset to base address in compound or simple address strings."""
    ...
    def parse_numeric(self, val, default=0.0):
    ...
    def _calculate_coefficients(self, factor, scale_factor_str, offset):
        val_factor = self.parse_numeric(factor, default=1.0)
        val_scale = self.parse_numeric(scale_factor_str, default=0.0)
        coef_a = "{:.6f}".format(val_factor * (10 ** val_scale))

        val_offset = self.parse_numeric(offset, default=0.0)
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            if address:
                address = self.apply_address_offset(address, address_offset, line_num=line_num)

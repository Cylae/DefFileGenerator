...
class Generator:
    ...
    def _determine_info1(self, reg_type_str: str, line_num: int = 0) -> str:
        ...
        elif str(reg_type_str).strip() in ['1', '2', '3', '4']: return str(reg_type_str).strip()
    ...
    def validate_csv(self, filepath: str) -> bool:
        ...
        try:
            ...
            with open(filepath, 'r', encoding=encoding) as f:
                ...
                except csv.Error: dialect = csv.excel; dialect.delimiter = ';' if ';' in snippet else ','
                ...
                is_webdyn = len(header) >= 4 and (header[0] in ['modbusRTU', 'modbusTCP'] or ';' in snippet)
                ...
                for line_num, row in enumerate(reader, start=2):
                    ...
                    if is_webdyn:
                        ...
                        # Fallback for input CSV if needed, but primarily for definition files
                        continue
                    ...
                    if self._check_address_overlap(info1, address, dtype, name, line_num, address_usage, warned_lines):
                        # According to memory, address overlap is a fatal error by default now
                        is_valid = False
    ...
    def process_rows(self, rows: Iterable[Dict[str, Any]], address_offset: int = 0) -> Iterator[Dict[str, Any]]:
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            if not self.validate_type(dtype):
                ...
                continue
            if dtype.startswith('STR') and '_' not in address:
                match = RE_TYPE_STR_CONV.match(dtype)
                if match:
                    dtype_internal = 'STRING'
                    address = f"{address}_{match.group(1)}"
                else: dtype_internal = dtype
            else: dtype_internal = dtype

            address = Generator.apply_address_offset(address, address_offset, line_num, name)
            if not self.validate_address(address, dtype_internal):
                logging.warning(f"Line {line_num}: Invalid Address '{address}' for Type '{dtype_internal}'. Skipping.")
                continue

            tag = self._process_name_and_tag(name, tag, line_num, seen_names, seen_tags)
            info1 = self._determine_info1(reg_type_str, line_num)
            self._check_address_overlap(info1, address, dtype_internal, name, line_num, address_usage, warned_lines)
...
def generate_template(output_file: Optional[str], mode: str = 'input') -> None:
    ...
    if mode == 'definition':
        ...
        headers = [] # headers already in rows
    else:
        rows = [
    ...
    outfile = None
    try:
        if output_file: outfile = open(output_file, 'w', newline='', encoding='utf-8')
        else: outfile = sys.stdout
        writer = csv.writer(outfile, delimiter=';' if mode == 'definition' else ',')
        if mode != 'definition': writer.writerow(headers)
        elif mode == 'definition': writer.writerow(headers) # Test suite expects headers in definition mode too
...
def run_generator(config: GeneratorConfig, input_data: Optional[Iterable[Dict[str, Any]]] = None) -> None:
    ...
    try:
        if input_data is not None:
            ...
            generator.write_output_csv(config.output, processed_rows, mfg, model, config.protocol, config.category, config.forced_write)
            ...
            logging.error("No input data or file provided.")

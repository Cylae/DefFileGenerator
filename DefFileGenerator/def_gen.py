...
class Generator:
    ...
    def _parse_numeric(val, default=0.0):
        """Robustly parses numeric values, handling fractions and locale formats."""
        ...
        # e.g., '1,234.56' vs '0,001'
        if ',' in s and '.' in s:
            # Assume '.' is decimal if both present
            s = s.replace(',', '')
        elif ',' in s:
            # Heuristic: if comma is followed by exactly 3 digits, it's likely a thousands separator
    ...
    def _process_name_and_tag(self, name, tag, seen_tags, line_num):
        """Handles tag generation and uniqueness."""
        if not tag and name:
            ...
            # Prepend v_ if starts with digit or empty
            if not base_tag or base_tag[0].isdigit():
    ...
    def _check_address_overlap(self, info1, address, dtype, name, line_num, address_usage):
        """Tracks and warns about address overlaps."""
        try:
            parts = address.split('_')
            ...
            for u_start, u_end, u_line, u_name, u_type in address_usage[info1]:
                if max(start_addr, u_start) <= min(end_addr, u_end):
                    # Allow multiple BITS on same address
    ...
    @staticmethod
    def _calculate_coefficients(factor_str, scale_str, offset_str):
        """Calculates CoefA and CoefB."""
        # Factor defaults to 1.0 if empty
        val_factor = Generator._parse_numeric(factor_str, 1.0)
        val_scale = Generator._parse_numeric(scale_str, 0.0)
        coef_a = "{:.6f}".format(val_factor * (10 ** val_scale))

        val_offset = Generator._parse_numeric(offset_str, 0.0)
        coef_b = "{:.6f}".format(val_offset)
        return coef_a, coef_b

    def apply_address_offset(self, address, offset, line_num=None):
        """Public method to shift addresses by an offset."""
        if not address or offset == 0:
             # Even if offset is 0, we normalize to decimal strings
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            offset_val = self._get_val(row, 'Offset')
            ...
            coef_a, coef_b = self._calculate_coefficients(factor, scale_factor_str, offset_val)

...
class Generator:
    ...
    def apply_address_offset(self, address, offset):
        """Applies an offset to a Modbus address (handles simple and compound)."""
        if not address:
            return ""
    ...
    def _parse_numeric(self, val):
        """Handles float, fraction (1/10), and European decimals (0,1)."""
    ...
    def _normalize_type_and_address(self, dtype_raw, address, line_num, name, address_offset):
        ...
        if address:
            ...
            try:
                if int(address.split('_')[0]) < 0:
                    logging.warning(f"Line {line_num}: Address offset results in negative address for '{name}'.")
            except (ValueError, IndexError):
                pass
    ...
    def _process_name_and_tag(self, name, tag, line_num, seen_names, seen_tags):
        if name:
            if name in seen_names:
                logging.warning(f"Line {line_num}: Duplicate Name '{name}' detected. Previous at line {seen_names[name]}.")
            else:
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            dtype, address = self._normalize_type_and_address(
                self._get_val(row, 'Type'), address, line_num, name, address_offset
            )
            if not dtype: continue

            tag = self._process_name_and_tag(name, self._get_val(row, 'Tag'), line_num, seen_names, seen_tags)
            ...
            act_str = self._get_val(row, 'Action').upper()
            norm_action = '4' if act_str in ['R', 'READ', '4'] else '1' if act_str in ['RW', 'W', 'WRITE', '1'] else act_str if act_str in self.allowed_actions else '1'

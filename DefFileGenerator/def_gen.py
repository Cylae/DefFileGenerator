...
class Generator:
    ...
    def _check_address_overlap(self, info1, address, dtype, name, line_num, address_usage, warned_lines):
        """Checks for address overlaps, allowing multiple BITS at same address. Optimized O(N) lookup."""
        try:
                ...
                if addr in address_usage[info1]:
                    for u_line, u_name, u_type, u_start, u_end in address_usage[info1][addr]:
                        ...
                        if not (is_bits and u_type == 'BITS' and start_addr == u_start):
                            ...
                            warn_pair = tuple(sorted((line_num, u_line)))
                            if warn_pair not in warned_lines:
                                ...
                                warned_lines.add(warn_pair)

                # Register usage for this specific address
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        seen_names = {}
        seen_tags = {}
        address_usage = {} # Info1 -> Address -> list of users
        warned_lines = set()

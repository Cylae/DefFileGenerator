...
class Generator:
    ...
    def _get_val(self, row, key):
        """Case-insensitive dictionary lookup with whitespace stripping."""
    ...
    def _normalize_type_and_address(self, row, line_num, address_offset):
        ...
        if address:
            ...
            # Ensure base is normalized even if offset is 0
            parts = address.split('_')
            parts[0] = self.normalize_address_val(parts[0])
            address = '_'.join(parts)

            # Log warning for negative addresses
            try:
                if int(address.split('_')[0]) < 0:
                    logging.warning(f"Line {line_num}: Address offset results in negative address {address} for '{self._get_val(row, 'Name')}'.")
            except (ValueError, IndexError):
                pass
    ...
    def _check_address_overlap(self, address, dtype, info1, line_num, name, address_usage):
        try:
            ...
            for i in range(reg_count):
                ...
                # Update usage map. Non-bits take precedence for tracking.
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            self._check_address_overlap(address, dtype, info1, line_num, name, address_usage)

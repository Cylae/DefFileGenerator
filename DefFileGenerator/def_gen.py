...
class Generator:
    ...
    def _normalize_type_and_address(self, name, address, dtype_raw, address_offset, line_num):
    ...
    def _process_name_and_tag(self, name, tag, seen_names, seen_tags, line_num):
        """Handles duplicate detection and auto-tag generation."""
        if name:
    ...
    def _check_address_overlap(self, name, address, dtype, info1, address_usage, line_num):
        """Checks for register address overlaps using O(1) per address lookup."""
        try:
            ...
            # Check each register in the range
            ...
            # Record usage for all registers in range
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            dtype, address = self._normalize_type_and_address(name, address, dtype_raw, address_offset, line_num)
            if dtype is None:
                continue

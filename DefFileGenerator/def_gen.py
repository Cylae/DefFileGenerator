...
class Generator:
    ...
    def apply_address_offset(self, address, offset):
        """Applies offset and normalizes address components."""
        if not address:
            return ""
    ...
    def _parse_numeric(self, val):
        """Robustly parses numeric values including fractions and European formats."""
    ...
    def _check_address_overlap(self, line_num, name, info1, address, dtype, address_usage):
        try:
            ...
            for i, (u_start, u_end, u_line, u_name, u_type) in enumerate(address_usage[info1]):
                if max(start_addr, u_start) <= min(end_addr, u_end):
                    if not (is_bits and u_type == 'BITS' and start_addr == u_start):
                        ...
                        # Priority: overwrite BITS if we have a real register
                        if u_type == 'BITS' and not is_bits:
                             address_usage[info1][i] = (start_addr, end_addr, line_num, name, dtype.upper())
                             return
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            self._check_address_overlap(line_num, name, info1, address, dtype, address_usage)

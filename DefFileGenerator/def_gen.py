...
class Generator:
    ...
    def normalize_address_val(self, addr_part):
        ...
        # Priority 1: Explicit hex 0x...
        match_0x = re.search(r'0x([0-9A-Fa-f]+)', addr_part)
        if match_0x:
            return str(int(match_0x.group(1), 16))

        # Priority 2: Explicit hex ...h
        match_h = re.search(r'\b([0-9A-Fa-f]+)h\b', addr_part, re.IGNORECASE)
        if match_h:
            return str(int(match_h.group(1), 16))

        # Priority 3: Word boundary hex/dec
    ...
    def normalize_type(self, dtype):
        ...
        # Replace 'unsigned' with 'u', 'signed' with 'i', and remove spaces
    ...
    def normalize_action(self, action):
        """Normalizes action/access strings to Webdyn action codes."""

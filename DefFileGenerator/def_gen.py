...
class Generator:
    ...
    def _check_address_overlap(self, info1, address, dtype, name, line_num, address_usage, warned_lines):
        ...
        try:
            ...
            if info1 not in address_usage:
                address_usage[info1] = {} # addr -> list of (line, name, type, reg_range_str)

            for offset in range(reg_count):
                current_addr = start_addr + offset
                if current_addr in address_usage[info1]:
                    for u_line, u_name, u_type, u_range in address_usage[info1][current_addr]:
                        # Allow multiple BITS on exactly the same base address
                        if is_bits and u_type == 'BITS' and offset == 0 and current_addr == start_addr:
                            continue

                        # Prevent duplicate warnings for the same pair
                        pair = tuple(sorted((line_num, u_line)))
                        if pair not in warned_lines:
                            logging.warning(f"Line {line_num}: Address overlap detected for '{name}' ({start_addr}-{start_addr + reg_count - 1}). Overlaps with '{u_name}' (Line {u_line}, {u_range}).")
                            warned_lines.add(pair)

                if current_addr not in address_usage[info1]:
                    address_usage[info1][current_addr] = []
                address_usage[info1][current_addr].append((line_num, name, dtype.upper(), f"{start_addr}-{start_addr + reg_count - 1}"))
        except (ValueError, IndexError):
            pass
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        seen_tags = {}
        warned_lines = set()
        address_usage = {} # Info1 -> dict of addr -> list of usage info
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            if not act_str:
                ...
                norm_action = '4'
            elif act_str in ['RW', 'W', 'WRITE', 'READ/WRITE', 'R/W', '1']:
                norm_action = '1'

...
RE_TAG_CLEAN = re.compile(r'[^a-z0-9]+', re.IGNORECASE)
...
class Generator:
    ...
    def apply_address_offset(self, address, offset):
        """Applies offset to base address, supporting simple and compound formats."""
        if not address:
            return ""
        parts = str(address).split('_')
        try:
            # Always normalize the base address to decimal
            base = int(self.normalize_address_val(parts[0])) + offset
            parts[0] = str(base)
            return '_'.join(parts)
    ...
    def _get_val(self, row, key, pre_calculated_keys):
        """Optimized value retrieval from row dictionary."""
        return str(row.get(pre_calculated_keys.get(key.lower(), key), '')).strip()

    def _normalize_type_and_address(self, line_num, name, address, dtype_raw, address_offset):
        """Handles type normalization and address offset application."""
    ...
    def _process_name_and_tag(self, line_num, name, tag, seen_names, seen_tags):
        """Handles duplicate name detection and tag generation."""
        if name:
    ...
    def _determine_info1(self, line_num, reg_type_str):
    ...
    def _check_address_overlap(self, line_num, name, address, dtype, info1, address_usage):
        ...
        try:
            ...
            for addr in range(start_addr, end_addr + 1):
                if addr in address_usage[info1]:
                    ...
                    # BITS types can share the same register address
                    if not (is_bits and u_type == 'BITS'):
                        logging.warning(f"Line {line_num}: Address overlap detected for '{name}' at {addr}. Overlaps with '{u_name}' (Line {u_line}).")
                        break
    ...
    def _calculate_coefficients(self, factor, scale_factor_str, offset):
        """Calculates CoefA and CoefB for Webdyn definition."""
        try:
    ...
    def process_rows(self, rows, address_offset=0):
        ...
        for line_num, row in enumerate(rows, start=2):
            ...
            # Pre-calculate lowercase keys for the row
            pre_calculated_keys = {k.lower().strip(): k for k in row.keys()}

            name = self._get_val(row, 'Name', pre_calculated_keys)
            address = self._get_val(row, 'Address', pre_calculated_keys)

            if not name and not address:
                logging.warning(f"Line {line_num}: Skipping row with missing Name and Address.")
                continue

            tag = self._get_val(row, 'Tag', pre_calculated_keys)
            reg_type_str = self._get_val(row, 'RegisterType', pre_calculated_keys)
            dtype_raw = self._get_val(row, 'Type', pre_calculated_keys)
            factor = self._get_val(row, 'Factor', pre_calculated_keys)
            offset = self._get_val(row, 'Offset', pre_calculated_keys)
            unit = self._get_val(row, 'Unit', pre_calculated_keys)
            action = self._get_val(row, 'Action', pre_calculated_keys)
            scale_factor_str = self._get_val(row, 'ScaleFactor', pre_calculated_keys)

            dtype, address = self._normalize_type_and_address(line_num, name, address, dtype_raw, address_offset)
            if dtype is None:
                continue

            tag = self._process_name_and_tag(line_num, name, tag, seen_names, seen_tags)
            info1 = self._determine_info1(line_num, reg_type_str)
            self._check_address_overlap(line_num, name, address, dtype, info1, address_usage)

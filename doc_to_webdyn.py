...
def main():
    ...
    for df in dataframes:
        ...
        for _, row in df.iterrows():
            ...
            addr = ""
            if addr_raw is not None and not pd.isna(addr_raw):
                addr_str = str(addr_raw).strip()
                # Support Address_Length format
                if '_' in addr_str:
                    parts = addr_str.split('_')
                    norm_parts = [generator.normalize_address_val(p) for p in parts]
                    addr = '_'.join(norm_parts)
                else:
                    addr = generator.normalize_address_val(addr_str)

            name = str(name_raw) if not pd.isna(name_raw) else (f"Register {addr}" if addr else "")

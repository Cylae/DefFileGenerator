...
def _run_cli():
    ...
    if not args.manufacturer or not args.model:
        parser.error("the following arguments are required: --manufacturer, --model (unless --template is used)")
    ...
    success, raw = Extractor.peek_generator(raw)
    if not success: logging.error("No data extracted."); sys.exit(1)

    mapped = extractor.map_and_clean(raw, args.address_offset)
    success, mapped = Generator.peek_generator(mapped)
    if not success: logging.error("No registers extracted."); sys.exit(1)

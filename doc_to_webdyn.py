...
def is_na(val):
    ...
    if HAS_PANDAS:
        return pd.isna(val)
    # Basic fallback for common NA values if pandas is missing
    if isinstance(val, float) and val != val: # NaN check
        return True
    return str(val).lower() in ['nan', 'none', 'n/a', 'null', '']
...
def main():
    ...
    for df in dataframes:
        ...
        if HAS_PANDAS and isinstance(df, pd.DataFrame):
            ...
            # For list of dicts from fallback, columns are the keys of the first dict
        ...
        # Convert to list of dicts for universal iteration
        if HAS_PANDAS and isinstance(df, pd.DataFrame):
...
def load_excel(filepath, sheet_name=None):
    if not HAS_PANDAS:
        logging.error("Pandas is required to process Excel files.")
        return []
    try:
        if sheet_name:
...
def load_xml(filepath):
    if not HAS_PANDAS:
        logging.error("Pandas is required to process XML files.")
        return []
    try:
...
def load_pdf(filepath):
    if not HAS_PDFPLUMBER:
        logging.error("pdfplumber is required to process PDF files.")
        return []
    dfs = []
    try:

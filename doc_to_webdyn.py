...
def _run_cli():
    ...
    parser.add_argument('--template', action='store_true', help='Generate a template CSV file')
    ...
    if not args.input_file:
        parser.error("the following arguments are required: input_file (unless --template is used)")

    if not args.manufacturer or not args.model:
        logging.error("manufacturer and model are required unless using --template.")
        sys.exit(1)
    ...
    has_data, raw_it = peek_generator(raw)
    if not has_data: logging.error("No data extracted."); sys.exit(1)

    mapped = extractor.map_and_clean(raw_it, args.address_offset)
    has_registers, mapped_it = peek_generator(mapped)
    ...
    run_generator(config, input_data=mapped_it)

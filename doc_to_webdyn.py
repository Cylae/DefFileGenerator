...
def main():
    ...
    elif ext_name == '.pdf': raw = extractor.extract_from_pdf(args.input_file, [int(p.strip()) for p in args.pages.split(',')] if args.pages else None)
    ...
    # Apply offset during processing or to mapped data
    if args.address_offset != 0:
        for row in mapped:
            row['Address'] = generator.apply_address_offset(row['Address'], args.address_offset)

    # doc_to_webdyn.py follows memory: pass user-provided offset to Extractor (done above via manual apply)
    # and 0 to Generator.process_rows.

...
from DefFileGenerator.def_gen import Generator, generate_template, RE_TAG_CLEAN
...
def main():
    ...
    parser.add_argument('--address-offset', type=int, default=0, help='Shift addresses by offset')
    ...
    if not args.input_file or not args.manufacturer or not args.model:
        parser.error("input_file, --manufacturer, and --model are required unless using --template")
    ...
    mapped = extractor.map_and_clean(raw, address_offset=args.address_offset, args=args)
    if not mapped:
    ...
    output_file = args.output or f"{mfg_clean}_{model_clean}_definition.csv"

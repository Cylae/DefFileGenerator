...
def main():
    ...
    parser.add_argument('--template', action='store_true', help='Generate a sample definition CSV template')
    ...
    if args.template:
        out_temp = args.output or "template.csv"
        generate_template(out_temp)
        logging.info(f"Template generated at {out_temp}")
        return
    ...
    if ext_name in ['.xlsx', '.xlsm', '.xltx', '.xltm']:
        ...
        logging.error(f"Unsupported extension: {ext_name}")
        sys.exit(1)
    ...
    # Pass the address_offset to map_and_clean
    ...
    generator = Generator()
    # Pass 0 to process_rows since offset was already applied in map_and_clean

...
def main():
    ...
    generator = Generator()
    # We pass 0 to process_rows because the offset is already applied by the extractor
    processed = generator.process_rows(mapped, address_offset=0)

    mfg_clean = re.sub(r'[^a-zA-Z0-9]', '_', args.manufacturer).lower()
    model_clean = re.sub(r'[^a-zA-Z0-9]', '_', args.model).lower()

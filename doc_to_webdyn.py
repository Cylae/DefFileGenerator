...
from DefFileGenerator.def_gen import Generator, RE_TAG_CLEAN, generate_template
...
def main():
    ...
    generator = Generator()
    processed = generator.process_rows(mapped, address_offset=0) # Already offset during extraction

    if args.output:
        output_file = args.output
    else:
        m_clean = RE_TAG_CLEAN.sub('_', args.manufacturer).lower()
        mod_clean = RE_TAG_CLEAN.sub('_', args.model).lower()
        output_file = f"{m_clean}_{mod_clean}_definition.csv"

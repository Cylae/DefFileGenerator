...
def _run_cli():
    ...
    if args.template:
        run_generator(GeneratorConfig(output=args.output, template=True))
        return
    ...
    import itertools
    # raw is a generator of generators
    # We peek to see if there is any data
    try:
        first_gen = next(raw)
        raw = itertools.chain([first_gen], raw)
    except StopIteration:
        logging.error("No data extracted."); sys.exit(1)

    mapped = extractor.map_and_clean(raw, args.address_offset)

    # Peek mapped to see if there are registers
    try:
        first_row = next(mapped)
        mapped = itertools.chain([first_row], mapped)
    except StopIteration:

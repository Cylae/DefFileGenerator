...
def _run_cli():
    ...
    if not args.input_file:
        logging.error("the following arguments are required: input_file")
        sys.exit(1)
    ...
    is_empty, raw = Extractor.peek_generator(raw)
    if is_empty: logging.error("No data extracted."); sys.exit(1)

    mapped = extractor.map_and_clean(raw, args.address_offset)
    is_empty_mapped, mapped = Extractor.peek_generator(mapped)
    if is_empty_mapped: logging.error("No registers extracted."); sys.exit(1)

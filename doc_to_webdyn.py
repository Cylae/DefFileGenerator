...
def main():
    ...
    logging.info(f"Extracted data from {len(raw_data)} table(s)/sheet(s).")

    mapped_data = extractor.map_and_clean(raw_data)

    if not mapped_data:
        logging.error("No registers could be extracted/mapped from the tables.")
        sys.exit(1)

    logging.info(f"Successfully mapped {len(mapped_data)} registers.")

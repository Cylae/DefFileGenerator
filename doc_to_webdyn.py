...
def main():
    ...
    parser.add_argument('--template', action='store_true', help='Generate a sample input file')
    ...
    if args.template:
        output_file = args.output or "sample_registers.csv"
    ...
    if not args.input_file or not args.manufacturer or not args.model:
        parser.error("input_file, --manufacturer, and --model are required.")
    ...
    # Apply offset during extraction to be consistent with main.py 'run' behavior
    ...
    generator = Generator()
    # Pass 0 for address_offset since it was already applied by map_and_clean

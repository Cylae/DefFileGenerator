...
def main():
    ...
    generator = Generator()
    processed = generator.process_rows(mapped, 0) # Already offset during mapping

    mfg_tag = RE_TAG_CLEAN.sub('_', args.manufacturer).lower()
    model_tag = RE_TAG_CLEAN.sub('_', args.model).lower()
    output_file = args.output or f"{mfg_tag}_{model_tag}_definition.csv"

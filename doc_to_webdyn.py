...
def main():
    ...
    if not args.input_file or not args.manufacturer or not args.model:
        parser.error("the following arguments are required: input_file, --manufacturer, --model")
    ...
    # To prevent double-offsetting, apply to extractor and pass 0 to generator
    ...
    if args.output:
        ...
        m_part = RE_TAG_CLEAN.sub('_', args.manufacturer).lower()
        d_part = RE_TAG_CLEAN.sub('_', args.model).lower()
        output_file = f"{m_part}_{d_part}_definition.csv"

...
def _run_cli():
    ...
    if not args.input_file:
        logging.error("input_file is required unless using --template")
        sys.exit(1)
    ...
    mapped = extractor.map_and_clean(it, args.address_offset)

    has_registers, it_reg = peek_generator(mapped)
    ...
    run_generator(config, input_data=it_reg)

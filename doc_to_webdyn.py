...
def _run_cli():
    ...
    if not has_raw and not args.template:
        logging.error("No data extracted.")
        sys.exit(1)

    has_mapped, mapped_data = peek_generator(extractor.map_and_clean(raw, args.address_offset))
    if not has_mapped and not args.template:

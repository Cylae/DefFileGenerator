...
def main():
    ...
    parser.add_argument('--manufacturer', required=False) # Optional if template
    parser.add_argument('--model', required=False) # Optional if template
    ...
    if not args.manufacturer or not args.model:
        parser.error("--manufacturer and --model are required unless --template is used.")
    ...
    extractor = Extractor(mapping=mapping)
    ...
    # Note: Offset is applied here during processing.
    # To prevent double-offsetting, we pass args.address_offset here
    # and we SHOULD NOT pass it to any subsequent generator call if we were using main.py's run.

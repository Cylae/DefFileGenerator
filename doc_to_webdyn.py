...
def _run_cli():
    ...
    if not args.input_file:
        logging.error("the following arguments are required: input_file (unless --template is used)")
        sys.exit(1)
    ...
    ok_raw, raw = peek_generator(raw)
    if not ok_raw: logging.error("No data extracted."); sys.exit(1)

    mapped_gen = extractor.map_and_clean(raw, args.address_offset)
    ok_mapped, mapped = peek_generator(mapped_gen)
    if not ok_mapped: logging.error("No registers extracted."); sys.exit(1)

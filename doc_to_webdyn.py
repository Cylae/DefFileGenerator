...
def main():
    ...
    parser.add_argument('--address-offset', type=int, default=0, help='Shift all addresses by this value')
    parser.add_argument('--forced-write', default='', help='Value for the Forced Write header field')
    parser.add_argument('--mapping', help='JSON file for explicit column mapping')
    ...
    if args.template:
        out_file = args.output or "template_registers.csv"
    ...
    if args.pages:
        try:
            pdf_pages = [int(p.strip()) for p in args.pages.split(',')]
        except ValueError:
    ...
    elif ext == '.pdf': raw = extractor.extract_from_pdf(args.input_file, pdf_pages)
    ...
    # Apply offset during mapping stage (Extractor handles this)
    ...
    generator = Generator()
    # Pass 0 offset to process_rows since it was already applied in map_and_clean

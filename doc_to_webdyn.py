...
def _run_cli():
    ...
    if not args.input_file:
        parser.error("input_file is required unless --template is used.")

    if not args.manufacturer or not args.model:
        parser.error("--manufacturer and --model are required.")
    ...
    ok, raw = peek_generator(raw)
    if not ok: logging.error("No data extracted."); sys.exit(1)

    mapped_data = extractor.map_and_clean(raw, args.address_offset)
    ok, mapped_data = peek_generator(mapped_data)
    if not ok: logging.error("No registers extracted."); sys.exit(1)

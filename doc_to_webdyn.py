...
from DefFileGenerator.def_gen import run_generator, Generator
...
def main():
    ...
    parser.add_argument('--pages', help='PDF pages to process (e.g. 1,2,5)')
    parser.add_argument('--address-offset', type=int, default=0, help='Address offset to subtract from input addresses')
    ...
    all_extracted_rows = extractor.map_and_clean(tables)
    ...
    # Process and generate output using run_generator logic (but we already have extracted rows)
    # Actually, run_generator expects a file. We can either write to a temp file or
    # just use Generator class directly here to be cleaner.

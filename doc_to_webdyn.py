...
def _run_cli():
    ...
    # Manual validation for manufacturer and model (if not template)
    template_mode = getattr(args, 'forced_write', '') == 'TEMPLATE' or (hasattr(args, 'template') and args.template)
    ...
    elif ext is not None: logging.error(f"Unsupported extension: {ext}"); sys.exit(1)

    has_data, raw = peek_generator(raw)
    if not has_data and not template_mode:
        logging.error("No data extracted.")
        sys.exit(1)

    mapped_data = extractor.map_and_clean(raw, args.address_offset)
    has_registers, mapped_data = peek_generator(mapped_data)
    if not has_registers and not template_mode:
        logging.error("No registers extracted.")
        sys.exit(1)

    m_clean = re.sub(r'[^a-zA-Z0-9]', '_', args.manufacturer or 'mfg').lower()
    mod_clean = re.sub(r'[^a-zA-Z0-9]', '_', args.model or 'model').lower()
    output_file = args.output or f"{m_clean}_{mod_clean}_definition.csv"

...
def _run_cli():
    ...
    # Use peek_generator for O(1) memory efficiency
    has_data_source, raw_peeking = peek_generator(raw)
    if not has_data_source: logging.error("No data extracted."); sys.exit(1)

    mapped_gen = extractor.map_and_clean(raw_peeking, args.address_offset)
    has_registers, mapped = peek_generator(mapped_gen)

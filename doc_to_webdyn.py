...
def main():
    ...
    for df in dataframes:
        ...
        for _, row in df.iterrows():
            ...
            addr = str(addr_raw) if not is_na(addr_raw) else ''
    ...
    # Pre-normalize extracted rows using centralized logic
    for row in all_extracted_rows:
        row['Address'] = generator.normalize_address_val(row['Address'])
        row['Type'] = generator.normalize_type(row['Type'])
        row['Action'] = generator.normalize_action(row['Action'])

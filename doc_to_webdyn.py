...
def _run_cli(argv=None):
    ...
    parser.add_argument(
        "--template", action="store_true", help="Generate a template definition"
    )
    ...
    if not output_file:
        clean_m_name = re.sub(r"[^a-zA-Z0-9]", "_", m_name).lower()
        clean_m_model = re.sub(r"[^a-zA-Z0-9]", "_", m_model).lower()
        output_file = f"{clean_m_name}_{clean_m_model}_definition.csv"

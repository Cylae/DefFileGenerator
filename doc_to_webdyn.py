...
def main():
    ...
    parser.add_argument('--pages', help='Comma-separated list of pages to extract from PDF')
    parser.add_argument('--mapping', help='JSON file for explicit column mapping')
    parser.add_argument('--address-offset', type=int, default=0, help='Offset to apply to all addresses')
    parser.add_argument('--forced-write', default='', help='Value for the forced-write column in header')
    ...
    if args.template:
        output_file = args.output or "sample_template.csv"
    ...
    generator = Generator()
    # To prevent double-offsetting, we pass 0 here because address_offset was already applied in map_and_clean

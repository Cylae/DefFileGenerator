...
lazy_static! {
    ...
    static ref RE_NORM_TYPE: Regex = Regex::new(r"^(u|i|uint|int)(\d+)(_(w|b|wb))?$").unwrap();
}
...
impl Generator {
    ...
    fn normalize_type(&self, dtype: &str) -> String {
        ...
        let mut t_str = dtype.to_lowercase();
        if let Some(pos) = t_str.find('(') {
            t_str = t_str[..pos].trim().to_string();
        }
        t_str = t_str.chars().filter(|c| c.is_alphanumeric() || *c == '_').collect();
        t_str = t_str.replace("unsigned", "u").replace("signed", "i");

        let mut m = HashMap::new();
        m.insert("uint16", "U16"); m.insert("int16", "I16");
        m.insert("uint32", "U32"); m.insert("int32", "I32");
        m.insert("uint64", "U64"); m.insert("int64", "I64");
        m.insert("u16", "U16"); m.insert("i16", "I16");
        m.insert("u32", "U32"); m.insert("i32", "I32");
        m.insert("u64", "U64"); m.insert("i64", "I64");
        m.insert("f32", "F32"); m.insert("f64", "F64");
        m.insert("float32", "F32"); m.insert("float64", "F64");
        m.insert("float", "F32"); m.insert("double", "F64");
        m.insert("string", "STRING"); m.insert("bits", "BITS");
        m.insert("ip", "IP"); m.insert("ipv6", "IPV6"); m.insert("mac", "MAC");

        if let Some(val) = m.get(t_str.as_str()) {
            return val.to_string();
        }

        if let Some(caps) = RE_NORM_TYPE.captures(&t_str) {
            let prefix = if caps.get(1).unwrap().as_str().starts_with('u') { "U" } else { "I" };
            let bits = caps.get(2).unwrap().as_str();
            let suffix = caps.get(3).map_or("", |m| m.as_str()).to_uppercase();
            return format!("{}{}{}", prefix, bits, suffix);
        }
...
fn main() -> Result<(), Box<dyn Error>> {
    let args = Args::parse();
    let gen = Generator::new(args.address_offset);
    ...
    for (i, result) in rdr.deserialize().enumerate() {
        ...
        let dtype_norm = gen.normalize_type(&row.dtype);
        if !gen.validate_type(&dtype_norm) {
            eprintln!("Line {}: Invalid Type '{}'. Skipping row.", line_num, row.dtype);
            continue;
        }

        let mut dtype = dtype_norm;
        ...
        // Address Offset and Overlap detection
        let mut addr_parts: Vec<String> = address.split('_').map(|s| s.to_string()).collect();
        if let Ok(mut start_addr) = addr_parts[0].parse::<i32>() {
            if gen.address_offset != 0 {
                let raw_start = start_addr;
                start_addr -= gen.address_offset;
                if start_addr < 0 {
                    eprintln!("Line {}: Address {} with offset {} results in negative address {}", line_num, raw_start, gen.address_offset, start_addr);
                }
                addr_parts[0] = start_addr.to_string();
                address = addr_parts.join("_");
            }

            let start_addr_u32 = start_addr as u32;
            let reg_count = gen.get_register_count(&dtype, &address);
            let end_addr = if reg_count > 0 { start_addr_u32 + reg_count - 1 } else { start_addr_u32 };
            ...
            for &(u_start, u_end, u_line, ref u_name, ref u_type) in used_list.iter() {
                if std::cmp::max(start_addr_u32, u_start) <= std::cmp::min(end_addr, u_end) {
                    ...
                    if !is_overlap_allowed {
                         eprintln!("Line {}: Address overlap detected for '{}' (Addr: {}-{}). Overlaps with '{}' (Line {}, Addr: {}-{}) in register type {}.",
                            line_num, row.name, start_addr_u32, end_addr, u_name, u_line, u_start, u_end, info1);
                    }
                }
            }
            used_list.push((start_addr_u32, end_addr, line_num, row.name.clone(), dtype.clone()));
        }
...
mod tests {
    ...
    fn test_normalize_type() {
        ...
        assert_eq!(gen.normalize_type("float64"), "F64");
        assert_eq!(gen.normalize_type("double"), "F64");
        assert_eq!(gen.normalize_type("uint16 (0-65535)"), "U16");
    }
}
